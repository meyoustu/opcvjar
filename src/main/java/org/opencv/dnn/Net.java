 ...
 * are Layer instances, and edges specify relationships between layers inputs
 * and outputs.
 * <p>
 * Each network layer has unique integer id and unique string name inside its
 * network. LayerId can store either layer name or layer id.
 * <p>
 * This class supports reference counting of its instances, i. e. copies point
...
public class Net {
     ...
     * @return blob for first output of specified layer. By default runs forward
     *         pass for the whole network.
     */
     ...
     * @param numParam index of the layer parameter in the Layer::blobs array. SEE:
     *                 Layer::blobs
     ...
     * @param bin Binary file with trained weights. Networks imported from Intel's
     *            Model Optimizer are launched in Intel's Inference Engine backend.
     * @return automatically generated
     */
    ...
    //
    // C++: static Net cv::dnn::Net::readFromModelOptimizer(vector_uchar
    // bufferModelConfig, vector_uchar bufferWeights)
    //

    /**
     * Create a network from Intel's Model Optimizer in-memory buffers with
    ...
    //
    // C++: int64 cv::dnn::Net::getFLOPS(int layerId, vector_MatShape
    // netInputShapes)
    //
    ...
    /**
     * Returns overall time for inference and timings (in ticks) for layers. Indexes
     * in returned vector correspond to layers ids. Some layers can be fused with
     * others, in this case zero ticks count will be return for that skipped layers.
     *
     ...
     *               &lt;DFN&gt;layer_name&lt;/DFN&gt; is string name of the added
     *               layer. If this part is empty then the network input pseudo
     *               layer will be used; - the second optional part of the template
     *               &lt;DFN&gt;input_number&lt;/DFN&gt; is either number of the
     *               layer input, either label one. If this part is omitted then the
     *               first layer input will be used.
     *               <p>
     ...
     *
     * @param fusion true to enable the fusion, false to disable. The fusion is
     *               enabled by default.
     */
    ...
    //
    // C++: void cv::dnn::Net::forward(vector_Mat& outputBlobs, String outputName =
    // String())
    //
     ...
     * @param outputName  name for layer which output is needed to get If
     *                    {@code outputName} is empty, runs forward pass for the
     *                    whole network.
     */
     ...
     * Runs forward pass to compute output of layer with name {@code outputName}.
     *
     * @param outputBlobs contains all output blobs for specified layer. If
     *                    {@code outputName} is empty, runs forward pass for the
     *                    whole network.
     */
    ...
    //
    // C++: void cv::dnn::Net::forward(vector_Mat& outputBlobs, vector_String
    // outBlobNames)
    //
    ...
    //
    // C++: void cv::dnn::Net::forward(vector_vector_Mat& outputBlobs, vector_String
    // outBlobNames)
    //
    ...
    //
    // C++: void cv::dnn::Net::getLayersShapes(MatShape netInputShape, vector_int&
    ...
    //
    // C++: void cv::dnn::Net::getLayersShapes(vector_MatShape netInputShapes,
    ...
    //
    // C++: void cv::dnn::Net::getMemoryConsumption(MatShape netInputShape, size_t&
    ...
    //
    // C++: void cv::dnn::Net::getMemoryConsumption(int layerId, MatShape
    // netInputShape, size_t& weights, size_t& blobs)
    //
    ...
    //
    // C++: void cv::dnn::Net::getMemoryConsumption(int layerId, vector_MatShape
    // netInputShapes, size_t& weights, size_t& blobs)
    //
     ...
     *
     * @param scheduler Path to YAML file with scheduling directives. SEE:
     *                  setPreferableBackend
     *                  <p>
     *                  Schedule layers that support Halide backend. Then compile
     *                  them for specific target. For layers that not represented in
    ...
    //
    // C++: void cv::dnn::Net::setInput(Mat blob, String name = "", double
    // scalefactor = 1.0, Scalar mean = Scalar())
    //
     ...
     * @param mean        An optional mean subtraction values. SEE: connect(String,
     *                    String) to know format of the descriptor.
     *                    <p>
     *                    If scale or mean values are specified, a final input blob
     *                    is computed as: \(input(n,c,h,w) = scalefactor \times
     *                    (blob(n,c,h,w) - mean_c)\)
     */
     ...
     * @param blob        A new blob. Should have CV_32F or CV_8U depth.
     * @param name        A name of input layer.
     * @param scalefactor An optional normalization scale. SEE: connect(String,
     *                    String) to know format of the descriptor.
     *                    <p>
     *                    If scale or mean values are specified, a final input blob
     *                    is computed as: \(input(n,c,h,w) = scalefactor \times
     ...
     *
     * @param blob A new blob. Should have CV_32F or CV_8U depth.
     * @param name A name of input layer. SEE: connect(String, String) to know
     *             format of the descriptor.
     *             <p>
     *             If scale or mean values are specified, a final input blob is
     *             computed as: \(input(n,c,h,w) = scalefactor \times (blob(n,c,h,w)
     *             - mean_c)\)
     */
     ...
     * Sets the new input value for the network
     *
     * @param blob A new blob. Should have CV_32F or CV_8U depth. SEE:
     ...
     * This layer stores the user blobs only and don't make any computations. In
     * fact, this layer provides the only way to pass user data into the network. As
     * any other layer, this layer can label its outputs and this function provides
     ...
     * @param blob     the new value. SEE: Layer::blobs <b>Note:</b> If shape of the
     *                 new blob differs from the previous shape, then the following
     ...
     *                 DNN_BACKEND_HALIDE | DNN_BACKEND_CUDA |
     *                 |------------------------|--------------------|------------------------------|--------------------|-------------------|
     *                 | DNN_TARGET_CPU | + | + | + | | | DNN_TARGET_OPENCL | + | +
     *                 | + | | | DNN_TARGET_OPENCL_FP16 | + | + | | | |
     *                 DNN_TARGET_MYRIAD | | + | | | | DNN_TARGET_FPGA | | + | | | |
    ...
    // C++: static Net cv::dnn::Net::readFromModelOptimizer(vector_uchar
    // bufferModelConfig, vector_uchar bufferWeights)
    private static native long readFromModelOptimizer_1(long bufferModelConfig_mat_nativeObj,
    ...
    // C++: void cv::dnn::Net::getMemoryConsumption(MatShape netInputShape, size_t&
    // weights, size_t& blobs)
    private static native void getMemoryConsumption_0(long nativeObj, long netInputShape_mat_nativeObj,
            double[] weights_out, double[] blobs_out);

    // C++: void cv::dnn::Net::getMemoryConsumption(int layerId, MatShape
    // netInputShape, size_t& weights, size_t& blobs)
    private static native void getMemoryConsumption_1(long nativeObj, int layerId, long netInputShape_mat_nativeObj,
            double[] weights_out, double[] blobs_out);

    // C++: void cv::dnn::Net::getMemoryConsumption(int layerId, vector_MatShape
    // netInputShapes, size_t& weights, size_t& blobs)
    private static native void getMemoryConsumption_2(long nativeObj, int layerId, List<MatOfInt> netInputShapes,
            double[] weights_out, double[] blobs_out);
    ...
    // C++: void cv::dnn::Net::setInput(Mat blob, String name = "", double
    // scalefactor = 1.0, Scalar mean = Scalar())
    private static native void setInput_0(long nativeObj, long blob_nativeObj, String name, double scalefactor,
            double mean_val0, double mean_val1, double mean_val2, double mean_val3);
