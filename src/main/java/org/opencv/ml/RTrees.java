...
public class RTrees extends DTrees {
    ...
    /**
     * Returns the variable importance array. The method returns the variable
     * importance vector, computed at the training stage when CalculateVarImportance
     * is set to true. If this flag was set to false, the empty matrix is returned.
     *
     ...
     * StatModel::train to create and train the model, Algorithm::load to load the
     * pre-trained model.
     *
     * @return automatically generated
     */
    ...
    //
    // C++: static Ptr_RTrees cv::ml::RTrees::load(String filepath, String nodeName
    // = String())
    //
     ...
     * <p>
     * Use RTree::save to serialize and store an RTree to disk. Load the RTree from
     * this file again, by calling this function with the path to the file.
    ...
    /**
     * Returns the result of each individual tree in the forest. In case the model
     * is a regression problem, the method will return each of the trees' results
     * for each of the sample cases. If the model is a classifier, it will return a
     * Mat with samples + 1 rows, where the first row gives the class number and the
