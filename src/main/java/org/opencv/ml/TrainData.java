 ...
 * Ptr&lt;TrainData&gt; as parameter. In other words, you can create your own
 * class derived from TrainData and pass smart pointer to the instance of this
 * class into StatModel::train.
 * <p>
...
public class TrainData {
    ...
    //
    // C++: static Mat cv::ml::TrainData::getSubMatrix(Mat matrix, Mat idx, int
    // layout)
    //
     ...
     * {@code 0} to `&lt;number of classes&gt;-1`. The actual label value can be
     * retrieved then from the class label vector, see TrainData::getClassLabels.
     *
     * @return automatically generated
     */
    ...
    //
    // C++: Mat cv::ml::TrainData::getTrainSamples(int layout = ROW_SAMPLE, bool
    // compressSamples = true, bool compressVars = true)
    //

    /**
     * Returns matrix of train samples
     *
     * @param layout          The requested layout. If it's different from the
     *                        initial one, the matrix is transposed. See
     *                        ml::SampleTypes.
     ...
     *                        <p>
     *                        In current implementation the function tries to avoid
     *                        physical data copying and returns the matrix stored
     *                        inside TrainData (unless the transposition or
     ...
     *                        ml::SampleTypes.
     * @param compressSamples if true, the function returns only the training
     *                        samples (specified by sampleIdx) the active variables.
     *                        <p>
     *                        In current implementation the function tries to avoid
     *                        physical data copying and returns the matrix stored
     *                        inside TrainData (unless the transposition or
     ...
     *               the matrix is transposed. See ml::SampleTypes. sampleIdx) the
     *               active variables.
     *               <p>
     *               In current implementation the function tries to avoid physical
     *               data copying and returns the matrix stored inside TrainData
     *               (unless the transposition or compression is needed).
     * @return automatically generated
     */
    ...
    //
    // C++: static Ptr_TrainData cv::ml::TrainData::create(Mat samples, int layout,
    // Mat responses, Mat varIdx = Mat(), Mat sampleIdx = Mat(), Mat sampleWeights =
    // Mat(), Mat varType = Mat())
    //
     ...
     *                      should be stored as a single row or as a single column.
     *                      The matrix should have type CV_32F or CV_32S (in the
     *                      former case the responses are considered as ordered by
     *                      default; in the latter case - as categorical)
     * @param varIdx        vector specifying which variables to use for training.
     *                      It can be an integer vector (CV_32S) containing 0-based
     *                      variable indices or byte vector (CV_8U) containing a
     *                      mask of active variables.
     * @param sampleIdx     vector specifying which samples to use for training. It
     *                      can be an integer vector (CV_32S) containing 0-based
     *                      sample indices or byte vector (CV_8U) containing a mask
     *                      of training samples.
     ...
     *                      &lt;number_of_variables_in_responses&gt;`, containing
     *                      types of each input and output variable. See
     *                      ml::VariableTypes.
     * @return automatically generated
     */
    public static TrainData create(Mat samples, int layout, Mat responses, Mat varIdx, Mat sampleIdx, Mat sampleWeights,
            Mat varType) {
        return TrainData.__fromPtr__(create_0(samples.nativeObj, layout, responses.nativeObj, varIdx.nativeObj,
                sampleIdx.nativeObj, sampleWeights.nativeObj, varType.nativeObj));
    }
    ...
    public static TrainData create(Mat samples, int layout, Mat responses, Mat varIdx, Mat sampleIdx,
            Mat sampleWeights) {
        return TrainData.__fromPtr__(create_1(samples.nativeObj, layout, responses.nativeObj, varIdx.nativeObj,
                sampleIdx.nativeObj, sampleWeights.nativeObj));
    }
     ...
     * @param layout    see ml::SampleTypes.
     * @param responses matrix of responses. If the responses are scalar, they
     *                  should be stored as a single row or as a single column. The
     *                  matrix should have type CV_32F or CV_32S (in the former case
     *                  the responses are considered as ordered by default; in the
     *                  latter case - as categorical)
     * @param varIdx    vector specifying which variables to use for training. It
     *                  can be an integer vector (CV_32S) containing 0-based
     *                  variable indices or byte vector (CV_8U) containing a mask of
     *                  active variables.
     * @param sampleIdx vector specifying which samples to use for training. It can
     *                  be an integer vector (CV_32S) containing 0-based sample
     *                  indices or byte vector (CV_8U) containing a mask of training
     *                  samples. &lt;number_of_variables_in_responses&gt;`,
    ...
    public static TrainData create(Mat samples, int layout, Mat responses, Mat varIdx, Mat sampleIdx) {
        return TrainData.__fromPtr__(
                create_2(samples.nativeObj, layout, responses.nativeObj, varIdx.nativeObj, sampleIdx.nativeObj));
    }
     ...
     *                  can be an integer vector (CV_32S) containing 0-based
     *                  variable indices or byte vector (CV_8U) containing a mask of
     *                  active variables. vector (CV_32S) containing 0-based sample
     *                  indices or byte vector (CV_8U) containing a mask of training
     *                  samples. &lt;number_of_variables_in_responses&gt;`,
     *                  containing types of each input and output variable. See
     *                  ml::VariableTypes.
     ...
     *                  matrix should have type CV_32F or CV_32S (in the former case
     *                  the responses are considered as ordered by default; in the
     *                  latter case - as categorical) (CV_32S) containing 0-based
     *                  variable indices or byte vector (CV_8U) containing a mask of
     *                  active variables. vector (CV_32S) containing 0-based sample
     *                  indices or byte vector (CV_8U) containing a mask of training
     *                  samples. &lt;number_of_variables_in_responses&gt;`,
    ...
    //
    // C++: void cv::ml::TrainData::setTrainTestSplit(int count, bool shuffle =
    // true)
    //
    ...
    //
    // C++: void cv::ml::TrainData::setTrainTestSplitRatio(double ratio, bool
    // shuffle = true)
    //
     ...
     * and processed as well. SEE: TrainData::setTrainTestSplit
     *
    ...
    // Mat responses, Mat varIdx = Mat(), Mat sampleIdx = Mat(), Mat sampleWeights =
    // Mat(), Mat varType = Mat())
    private static native long create_0(long samples_nativeObj, int layout, long responses_nativeObj,
            long varIdx_nativeObj, long sampleIdx_nativeObj, long sampleWeights_nativeObj, long varType_nativeObj);

    private static native long create_1(long samples_nativeObj, int layout, long responses_nativeObj,
            long varIdx_nativeObj, long sampleIdx_nativeObj, long sampleWeights_nativeObj);

    private static native long create_2(long samples_nativeObj, int layout, long responses_nativeObj,
            long varIdx_nativeObj, long sampleIdx_nativeObj);

    private static native long create_3(long samples_nativeObj, int layout, long responses_nativeObj,
