...
public class StatModel extends Algorithm {
    ...
    //
    // C++: float cv::ml::StatModel::calcError(Ptr_TrainData data, bool test, Mat&
    // resp)
    //
     ...
     *             Please note that if you loaded a completely different dataset to
     *             evaluate already trained classifier, you will probably want not
     *             to set the test subset at all with
     *             TrainData::setTrainTestSplitRatio and specify test=false, so that
     *             the error is computed for the whole new set. Yes, this sounds a
     *             bit confusing.
     * @param resp the optional output responses.
     *             <p>
     *             The method uses StatModel::predict to compute the error. For
    ...
    //
    // C++: float cv::ml::StatModel::predict(Mat samples, Mat& results = Mat(), int
    // flags = 0)
    //
