...
public class KNearest extends StatModel {
     ...
     * <p>
     * Use KNearest::save to serialize and store an KNearest to disk. Load the
     * KNearest from this file again, by calling this function with the path to the
     * file.
     *
    ...
    //
    // C++: float cv::ml::KNearest::findNearest(Mat samples, int k, Mat& results,
    // Mat& neighborResponses = Mat(), Mat& dist = Mat())
    //

    /**
     * Finds the neighbors and predicts responses for input vectors.
     *
     * @param samples           Input samples stored by rows. It is a
     *                          single-precision floating-point matrix of
     *                          {@code &lt;number_of_samples&gt; * k} size.
     * @param k                 Number of used nearest neighbors. Should be greater
     *                          than 1.
     * @param results           Vector with results of prediction (regression or
     *                          classification) for each input sample. It is a
     *                          single-precision floating-point vector with
     *                          {@code &lt;number_of_samples&gt;} elements.
     ...
     *                          {@code &lt;number_of_samples&gt; * k} size.
     * @param dist              Optional output distances from the input vectors to
     *                          the corresponding neighbors. It is a
     *                          single-precision floating-point matrix of
     *                          {@code &lt;number_of_samples&gt; * k} size.
     *                          <p>
     *                          For each input vector (a row of the matrix samples),
     *                          the method finds the k nearest neighbors. In case of
     *                          regression, the predicted result is a mean value of
     *                          the particular vector's neighbor responses. In case
     *                          of classification, the class is determined by
     *                          voting.
     *                          <p>
     *                          For each input vector, the neighbors are sorted by
     *                          their distances to the vector.
     *                          <p>
     *                          In case of C++ interface you can use output pointers
     *                          to empty matrices and the function will allocate
     *                          memory itself.
     *                          <p>
     *                          If only a single input vector is passed, all output
     *                          matrices are optional and the predicted value is
    ...
    public float findNearest(Mat samples, int k, Mat results, Mat neighborResponses, Mat dist) {
        return findNearest_0(nativeObj, samples.nativeObj, k, results.nativeObj, neighborResponses.nativeObj,
                dist.nativeObj);
    }
     ...
     * @param neighborResponses Optional output values for corresponding neighbors.
     *                          It is a single- precision floating-point matrix of
     *                          {@code &lt;number_of_samples&gt; * k} size. is a
     *                          single-precision floating-point matrix of
     *                          {@code &lt;number_of_samples&gt; * k} size.
     *                          <p>
     *                          For each input vector (a row of the matrix samples),
     ...
     *                floating-point matrix of {@code &lt;number_of_samples&gt; * k}
     *                size.
     * @param k       Number of used nearest neighbors. Should be greater than 1.
     * @param results Vector with results of prediction (regression or
     *                classification) for each input sample. It is a
     *                single-precision floating-point vector with
     *                {@code &lt;number_of_samples&gt;} elements. precision
     *                floating-point matrix of {@code &lt;number_of_samples&gt; * k}
     *                size. is a single-precision floating-point matrix of
     *                {@code &lt;number_of_samples&gt; * k} size.
     *                <p>
     *                For each input vector (a row of the matrix samples), the
     *                method finds the k nearest neighbors. In case of regression,
     *                the predicted result is a mean value of the particular
     *                vector's neighbor responses. In case of classification, the
     *                class is determined by voting.
     *                <p>
     *                For each input vector, the neighbors are sorted by their
     *                distances to the vector.
     *                <p>
     *                In case of C++ interface you can use output pointers to empty
     *                matrices and the function will allocate memory itself.
     *                <p>
     *                If only a single input vector is passed, all output matrices
     *                are optional and the predicted value is returned by the
     *                method.
     *                <p>
     *                The function is parallelized with the TBB library.
     * @return automatically generated
