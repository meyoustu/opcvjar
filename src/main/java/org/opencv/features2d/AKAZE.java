 ...
 * <p>
 * AKAZE descriptors can only be used with KAZE or AKAZE keypoints. This class
 * is thread-safe.
 *
 ...
 *
 * <b>Note:</b> AKAZE implements T-API. When image is passed as UMat some parts
 * of the algorithm will use OpenCL.
 *
 * <b>Note:</b> [ANB13] Fast Explicit Diffusion for Accelerated Features in
 * Nonlinear Scale Spaces. Pablo F. Alcantarilla, Jesús Nuevo and Adrien
 * Bartoli. In British Machine Vision Conference (BMVC), Bristol, UK, September
 * 2013.
 */
public class AKAZE extends Feature2D {
    ...
    public static final int DESCRIPTOR_KAZE_UPRIGHT = 2, DESCRIPTOR_KAZE = 3, DESCRIPTOR_MLDB_UPRIGHT = 4,
            DESCRIPTOR_MLDB = 5;
    ...
    // = AKAZE::DESCRIPTOR_MLDB, int descriptor_size = 0, int descriptor_channels =
    // 3, float threshold = 0.001f, int nOctaves = 4, int nOctaveLayers = 4,
    ...
    public static AKAZE create(int descriptor_type, int descriptor_size, int descriptor_channels, float threshold,
        ...
        return AKAZE.__fromPtr__(create_0(descriptor_type, descriptor_size, descriptor_channels, threshold, nOctaves,
    ...
    public static AKAZE create(int descriptor_type, int descriptor_size, int descriptor_channels, float threshold,
        ...
        return AKAZE.__fromPtr__(
                create_1(descriptor_type, descriptor_size, descriptor_channels, threshold, nOctaves, nOctaveLayers));
    }
     ...
     * @param descriptor_type Type of the extracted descriptor: DESCRIPTOR_KAZE,
     *                        DESCRIPTOR_KAZE_UPRIGHT, DESCRIPTOR_MLDB or
     *                        DESCRIPTOR_MLDB_UPRIGHT. DIFF_CHARBONNIER
     * @return automatically generated
     */
    ...
    // 3, float threshold = 0.001f, int nOctaves = 4, int nOctaveLayers = 4,
    // KAZE_DiffusivityType diffusivity = KAZE::DIFF_PM_G2)
    private static native long create_0(int descriptor_type, int descriptor_size, int descriptor_channels,
            float threshold, int nOctaves, int nOctaveLayers, int diffusivity);

    private static native long create_1(int descriptor_type, int descriptor_size, int descriptor_channels,
            float threshold, int nOctaves, int nOctaveLayers);

    private static native long create_2(int descriptor_type, int descriptor_size, int descriptor_channels,
            float threshold, int nOctaves);

    private static native long create_3(int descriptor_type, int descriptor_size, int descriptor_channels,
            float threshold);
