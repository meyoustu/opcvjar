...
public class BFMatcher extends DescriptorMatcher {
    ...
    //
    // C++: cv::BFMatcher::BFMatcher(int normType = NORM_L2, bool crossCheck =
    // false)
    //
    ...
    //
    // C++: static Ptr_BFMatcher cv::BFMatcher::create(int normType = NORM_L2, bool
    // crossCheck = false)
    //
     ...
     *                   and L2 norms are preferable choices for SIFT and SURF
     ...
     *                   when it finds the k nearest neighbors for each query
     *                   descriptor. If crossCheck==true, then the knnMatch() method
     *                   with k=1 will only return pairs (i,j) such that for i-th
     *                   query descriptor the j-th descriptor in the matcher's
     *                   collection is the nearest and vice versa, i.e. the
     *                   BFMatcher will only return consistent pairs. Such technique
     *                   usually produces best results with minimal number of
     *                   outliers when there are enough matches. This is alternative
     ...
     *                 L2 norms are preferable choices for SIFT and SURF
     *                 descriptors, NORM_HAMMING should be used with ORB, BRISK and
     *                 BRIEF, NORM_HAMMING2 should be used with ORB when WTA_K==3 or
     *                 4 (see ORB::ORB constructor description). nearest neighbors
     *                 for each query descriptor. If crossCheck==true, then the
     *                 knnMatch() method with k=1 will only return pairs (i,j) such
     *                 that for i-th query descriptor the j-th descriptor in the
     *                 matcher's collection is the nearest and vice versa, i.e. the
     *                 BFMatcher will only return consistent pairs. Such technique
     *                 usually produces best results with minimal number of outliers
     *                 when there are enough matches. This is alternative to the
    ...
    /**
     * Brute-force matcher create method. preferable choices for SIFT and SURF
     * descriptors, NORM_HAMMING should be used with ORB, BRISK and BRIEF,
     * NORM_HAMMING2 should be used with ORB when WTA_K==3 or 4 (see ORB::ORB
     * constructor description). nearest neighbors for each query descriptor. If
