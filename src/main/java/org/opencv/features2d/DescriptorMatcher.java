...
public class DescriptorMatcher extends Algorithm {
    ...
    //
    // C++: Ptr_DescriptorMatcher cv::DescriptorMatcher::clone(bool emptyTrainData =
    // false)
    //
     ...
     *                       copy of the object, that is, copies both parameters and
     *                       train data. If emptyTrainData is true, the method
     *                       creates an object copy with the current parameters but
     *                       with empty train data.
     ...
     * Clones the matcher.
     * <p>
     * that is, copies both parameters and train data. If emptyTrainData is true,
     * the method creates an object copy with the current parameters but with empty
     * train data.
     *
     * @return automatically generated
     */
    ...
    // cv::DescriptorMatcher::create(DescriptorMatcher_MatcherType matcherType)
    //
    ...
    //
    // C++: static Ptr_DescriptorMatcher cv::DescriptorMatcher::create(String
     ...
     *                              <ul>
     *                              <li>{@code BruteForce} (it uses L2 )</li>
     *                              <li>{@code BruteForce-L1}</li>
     *                              <li>{@code BruteForce-Hamming}</li>
     *                              <li>{@code BruteForce-Hamming(2)}</li>
     *                              <li>{@code FlannBased}</li>
     *                              </ul>
     * @return automatically generated
     */
    ...
    //
    // C++: void cv::DescriptorMatcher::knnMatch(Mat queryDescriptors, Mat
    // trainDescriptors, vector_vector_DMatch& matches, int k, Mat mask = Mat(),
     ...
     *                         the train descriptors collection stored in the class
     *                         object.
     * @param mask             Mask specifying permissible matches between an input
     *                         query and train matrices of descriptors.
     * @param matches          Matches. Each matches[i] is k or less matches for the
     *                         same query descriptor.
     * @param k                Count of best matches found per each query descriptor
     *                         or less if a query descriptor has less than k
     *                         possible matches in total.
     * @param compactResult    Parameter used when the mask (or masks) is not empty.
     *                         If compactResult is false, the matches vector has the
     *                         same size as queryDescriptors rows. If compactResult
     *                         is true, the matches vector does not contain matches
     *                         for fully masked-out query descriptors.
     *                         <p>
     *                         These extended variants of DescriptorMatcher::match
     *                         methods find several best matches for each query
     *                         descriptor. The matches are returned in the distance
     *                         increasing order. See DescriptorMatcher::match for
    ...
    //
    // C++: void cv::DescriptorMatcher::knnMatch(Mat queryDescriptors,
    // vector_vector_DMatch& matches, int k, vector_Mat masks = vector_Mat(), bool
    // compactResult = false)
    //
     ...
     *                         matches between the input query descriptors and
     ...
     *                         matches between the input query descriptors and
     *                         stored train descriptors from the i-th image
     *                         trainDescCollection[i]. false, the matches vector has
     *                         the same size as queryDescriptors rows. If
     *                         compactResult is true, the matches vector does not
     *                         contain matches for fully masked-out query
     *                         descriptors.
     */
     ...
     * @param k                Count of best matches found per each query descriptor
     *                         or less if a query descriptor has less than k
     *                         possible matches in total. descriptors and stored
     *                         train descriptors from the i-th image
     *                         trainDescCollection[i]. false, the matches vector has
     *                         the same size as queryDescriptors rows. If
     *                         compactResult is true, the matches vector does not
     *                         contain matches for fully masked-out query
    ...
    //
    // C++: void cv::DescriptorMatcher::match(Mat queryDescriptors, Mat
    // trainDescriptors, vector_DMatch& matches, Mat mask = Mat())
    //
     ...
     *                         the train descriptors collection stored in the class
     *                         object.
     * @param matches          Matches. If a query descriptor is masked out in mask
     *                         , no match is added for this descriptor. So, matches
     *                         size may be smaller than the query descriptors count.
     * @param mask             Mask specifying permissible matches between an input
     *                         query and train matrices of descriptors.
     *                         <p>
     *                         In the first variant of this method, the train
     *                         descriptors are passed as an input argument. In the
     *                         second variant of the method, train descriptors
     *                         collection that was set by DescriptorMatcher::add is
     *                         used. Optional mask (or masks) can be passed to
     *                         specify which query and training descriptors can be
     *                         matched. Namely, queryDescriptors[i] can be matched
     *                         with trainDescriptors[j] only if
    ...
    //
    // C++: void cv::DescriptorMatcher::match(Mat queryDescriptors, vector_DMatch&
    // matches, vector_Mat masks = vector_Mat())
    //
     ...
     *                         , no match is added for this descriptor. So, matches
     *                         size may be smaller than the query descriptors count.
     *                         descriptors and stored train descriptors from the
    ...
    //
    // C++: void cv::DescriptorMatcher::radiusMatch(Mat queryDescriptors, Mat
    // trainDescriptors, vector_vector_DMatch& matches, float maxDistance, Mat mask
    // = Mat(), bool compactResult = false)
    //
     ...
     *                         query and train matrices of descriptors.
     *                         <p>
     *                         For each query descriptor, the methods find such
     *                         training descriptors that the distance between the
     *                         query descriptor and the training descriptor is equal
     *                         or smaller than maxDistance. Found matches are
     ...
     *                         the train descriptors collection stored in the class
     *                         object.
     * @param matches          Found matches. false, the matches vector has the same
     *                         size as queryDescriptors rows. If compactResult is
     *                         true, the matches vector does not contain matches for
     *                         fully masked-out query descriptors.
     * @param maxDistance      Threshold for the distance between matched
     *                         descriptors. Distance means here metric distance
     *                         (e.g. Hamming distance), not the distance between
     *                         coordinates (which is measured in Pixels)!
    ...
    public void radiusMatch(Mat queryDescriptors, Mat trainDescriptors, List<MatOfDMatch> matches, float maxDistance) {
        Mat matches_mat = new Mat();
        radiusMatch_2(nativeObj, queryDescriptors.nativeObj, trainDescriptors.nativeObj, matches_mat.nativeObj,
                maxDistance);
        Converters.Mat_to_vector_vector_DMatch(matches_mat, matches);
        matches_mat.release();
    }
    ...
    //
    // C++: void cv::DescriptorMatcher::radiusMatch(Mat queryDescriptors,
    // vector_vector_DMatch& matches, float maxDistance, vector_Mat masks =
    // vector_Mat(), bool compactResult = false)
    //
    ...
    public void radiusMatch(Mat queryDescriptors, List<MatOfDMatch> matches, float maxDistance, List<Mat> masks,
        ...
        Mat matches_mat = new Mat();
        Mat masks_mat = Converters.vector_Mat_to_Mat(masks);
        radiusMatch_3(nativeObj, queryDescriptors.nativeObj, matches_mat.nativeObj, maxDistance, masks_mat.nativeObj,
                compactResult);
        Converters.Mat_to_vector_vector_DMatch(matches_mat, matches);
        matches_mat.release();
    }
     ...
     *                         coordinates (which is measured in Pixels)!
     *                         descriptors and stored train descriptors from the
     *                         i-th image trainDescCollection[i]. false, the matches
     ...
     * matchers (for example, BruteForceMatcher) have an empty implementation of
     * this method. Other matchers really train their inner structures (for example,
    ...
    //
    // C++: void cv::DescriptorMatcher::write(Ptr_FileStorage fs, String name =
    // String())
    //
    ...
    private static native void knnMatch_4(long nativeObj, long queryDescriptors_nativeObj, long matches_mat_nativeObj,
            int k, long masks_mat_nativeObj);

    private static native void knnMatch_5(long nativeObj, long queryDescriptors_nativeObj, long matches_mat_nativeObj,
            int k);
    ...
    private static native void radiusMatch_5(long nativeObj, long queryDescriptors_nativeObj,
