 ...
 * <p>
 * 1. Compute descriptors for a given image and its keypoints set. 2. Find the
 * nearest visual words from the vocabulary for each keypoint descriptor. 3.
 * Compute the bag-of-words image descriptor as is a normalized histogram of
...
public class BOWImgDescriptorExtractor {
    ...
    //
    // C++:
    // cv::BOWImgDescriptorExtractor::BOWImgDescriptorExtractor(Ptr_DescriptorExtractor
    ...
    //
    // C++: void cv::BOWImgDescriptorExtractor::compute2(Mat image, vector_KeyPoint
    // keypoints, Mat& imgDescriptor)
    //

    /**
     * @param imgDescriptor Computed output image descriptor. pointIdxsOfClusters[i]
     *                      are keypoint indices that belong to the i -th cluster
     *                      (word of vocabulary) returned if it is non-zero.
     ...
     *                   BOWTrainer ). Each row of the vocabulary is a visual word
     *                   (cluster center).
     */
