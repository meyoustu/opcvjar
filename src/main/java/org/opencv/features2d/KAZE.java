 ...
 * described in CITE: ABD12 .
 *
 * <b>Note:</b> AKAZE descriptor can only be used with KAZE or AKAZE keypoints
 * .. [ABD12] KAZE Features. Pablo F. Alcantarilla, Adrien Bartoli and Andrew J.
 * Davison. In European Conference on Computer Vision (ECCV), Fiorenze, Italy,
 * October 2012.
 */
public class KAZE extends Feature2D {
    ...
    //
    // C++: static Ptr_KAZE cv::KAZE::create(bool extended = false, bool upright =
    // false, float threshold = 0.001f, int nOctaves = 4, int nOctaveLayers = 4,
     ...
     * @param extended Set to enable extraction of extended (128-byte) descriptor.
     * @param upright  Set to enable use of upright descriptors (non
     *                 rotation-invariant). DIFF_CHARBONNIER
     * @return automatically generated
     */
    ...
    // false, float threshold = 0.001f, int nOctaves = 4, int nOctaveLayers = 4,
    // KAZE_DiffusivityType diffusivity = KAZE::DIFF_PM_G2)
    private static native long create_0(boolean extended, boolean upright, float threshold, int nOctaves,
            int nOctaveLayers, int diffusivity);

    private static native long create_1(boolean extended, boolean upright, float threshold, int nOctaves,
            int nOctaveLayers);
