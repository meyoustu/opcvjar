...
public class Imgcodecs {
    ...
    public static final int IMWRITE_JPEG_QUALITY = 1, IMWRITE_JPEG_PROGRESSIVE = 2, IMWRITE_JPEG_OPTIMIZE = 3,
            ...
            IMWRITE_EXR_TYPE = (3 << 4) + 0, IMWRITE_WEBP_QUALITY = 64, IMWRITE_PAM_TUPLETYPE = 128,
            IMWRITE_TIFF_RESUNIT = 256, IMWRITE_TIFF_XDPI = 257, IMWRITE_TIFF_YDPI = 258,
            IMWRITE_TIFF_COMPRESSION = 259, IMWRITE_JPEG2000_COMPRESSION_X1000 = 272;
     ...
     * If the buffer is too short or contains invalid data, the function returns an
     * empty matrix ( Mat::data==NULL ).
     * <p>
     * See cv::imread for the list of supported formats and flags description.
     *
     * <b>Note:</b> In the case of color images, the decoded images will have the
     * channels stored in <b>B G R</b> order.
     *
     ...
     * <li>Portable image format - \*.pbm, \*.pgm, \*.ppm \*.pxm, \*.pnm (always
     * supported)</li>
     ...
     * <li>Raster and Vector geospatial data supported by GDAL (see the *Note*
     * section)</li>
     * </ul>
     *
     * <b>Note:</b>
     * <ul>
     * <li>The function determines the type of an image by the content, not by the
     * file extension.</li>
     * <li>In the case of color images, the decoded images will have the channels
     * stored in <b>B G R</b> order.</li>
     * <li>When using IMREAD_GRAYSCALE, the codec's internal grayscale conversion
     * will be used, if available. Results may differ to the output of cvtColor()
     * </li>
     * <li>On Microsoft Windows\* OS and MacOSX\*, the codecs shipped with an OpenCV
     ...
     * color management embedded into MacOSX.</li>
     ...
     * the OPENCV_BUILD_3RDPARTY_LIBS flag in CMake.</li>
     * <li>In the case you set *WITH_GDAL* flag to true in CMake and REF:
     * IMREAD_LOAD_GDAL to load the image, then the [GDAL](http://www.gdal.org)
     * driver will be used in order to decode the image, supporting the following
     * formats: [Raster](http://www.gdal.org/formats_list.html),
     * [Vector](http://www.gdal.org/ogr_formats.html).</li>
     * <li>If EXIF information is embedded in the image file, the EXIF orientation
     * will be taken into account and thus the image will be rotated accordingly
     * except if the flags REF: IMREAD_IGNORE_ORIENTATION or REF: IMREAD_UNCHANGED
     * are passed.</li>
     * <li>Use the IMREAD_UNCHANGED flag to keep the floating point values from PFM
     * image.</li>
     * <li>By default number of pixels must be less than 2^30. Limit can be set
     * using system variable OPENCV_IO_MAX_IMAGE_PIXELS</li>
     * </ul>
     *
    ...
    //
    // C++: bool cv::imencode(String ext, Mat img, vector_uchar& buf, vector_int
    ...
    //
    // C++: bool cv::imreadmulti(String filename, vector_Mat& mats, int flags =
    // IMREAD_ANYCOLOR)
    //
    ...
    //
    // C++: bool cv::imwrite(String filename, Mat img, vector_int params =
    // std::vector<int>())
    //
     ...
     * channel order) images can be saved using this function, with these
     * exceptions:
     *
     * <ul>
     * <li>16-bit unsigned (CV_16U) images can be saved in the case of PNG, JPEG
     * 2000, and TIFF formats</li>
     * <li>32-bit float (CV_32F) images can be saved in PFM, TIFF, OpenEXR, and
     * Radiance HDR formats; 3-channel (CV_32FC3) TIFF images will be saved using
     * the LogLuv high dynamic range encoding (4 bytes per pixel)</li>
     * <li>PNG images with an alpha channel can be saved using this function. To do
     ...
     * pixels should have alpha set to 255/65535 (see the code sample below).</li>
     * </ul>
     * <p>
     * If the format, depth or channel order is different, use Mat::convertTo and
     * cv::cvtColor to convert it before saving. Or, use the universal FileStorage
     * I/O functions to save the image to XML or YAML format.
     * <p>
     * The sample below shows how to create a BGRA image and save it to a PNG file.
     * It also demonstrates how to set custom compression parameters: INCLUDE:
     * snippets/imgcodecs_imwrite.cpp
     *
     * @param filename Name of the file.
     * @param img      Image to be saved.
     * @param params   Format-specific parameters encoded as pairs (paramId_1,
     *                 paramValue_1, paramId_2, paramValue_2, ... .) see
     *                 cv::ImwriteFlags
     * @return automatically generated
     */
