...
public class Photo {
    ...
    // C++: enum <unnamed>
    public static final int INPAINT_NS = 0, INPAINT_TELEA = 1, LDR_SIZE = 256, NORMAL_CLONE = 1, MIXED_CLONE = 2,
            MONOCHROME_TRANSFER = 3, RECURS_FILTER = 1, NORMCONV_FILTER = 2;

    //
    // C++: Ptr_AlignMTB cv::createAlignMTB(int max_bits = 6, int exclude_range = 4,
    // bool cut = true)
    //
     ...
     *                      dimension. Values of 5 and 6 are usually good enough (31
     *                      and 63 pixels shift respectively).
     * @param exclude_range range for exclusion bitmap that is constructed to
     *                      suppress noise around the median value.
     * @param cut           if true cuts images, otherwise fills the new regions
     *                      with zeros.
     ...
     *                 Values of 5 and 6 are usually good enough (31 and 63 pixels
     *                 shift respectively). median value.
     * @return automatically generated
     */
    ...
    //
    // C++: Ptr_CalibrateDebevec cv::createCalibrateDebevec(int samples = 70, float
     ...
     * @param samples number of pixel locations to use
     * @param lambda  smoothness term weight. Greater values produce smoother
     *                results, but can alter the response. rectangular grid.
     * @return automatically generated
     */
    ...
    //
    // C++: Ptr_CalibrateRobertson cv::createCalibrateRobertson(int max_iter = 30,
    ...
    //
    // C++: Ptr_MergeMertens cv::createMergeMertens(float contrast_weight = 1.0f,
     ...
     *              no correction, gamma equal to 2.2f is suitable for most
     *              displays. Generally gamma &gt; 1 brightens the image and gamma
     *              &lt; 1 darkens it.
     * @return automatically generated
     */
     ...
     * Creates simple linear mapper with gamma correction
     * <p>
     * equal to 2.2f is suitable for most displays. Generally gamma &gt; 1 brightens
     * the image and gamma &lt; 1 darkens it.
     *
     * @return automatically generated
     */
    ...
    //
    // C++: Ptr_TonemapDrago cv::createTonemapDrago(float gamma = 1.0f, float
     ...
     * @param saturation positive saturation enhancement value. 1.0 preserves
     *                   saturation, values greater than 1 increase saturation and
     *                   values less than 1 decrease it. results, default value is
     *                   0.85.
     * @return automatically generated
     */
     ...
     * Creates TonemapDrago object
     *
     * @param gamma gamma value for gamma correction. See createTonemap than 1
     *              increase saturation and values less than 1 decrease it. results,
     ...
     * Creates TonemapDrago object
     * <p>
     * than 1 increase saturation and values less than 1 decrease it. results,
     * default value is 0.85.
     *
     * @return automatically generated
     */
    ...
    //
    // C++: Ptr_TonemapMantiuk cv::createTonemapMantiuk(float gamma = 1.0f, float
     ...
     * @param gamma gamma value for gamma correction. See createTonemap
     * @param scale contrast scale factor. HVS response is multiplied by this
     *              parameter, thus compressing dynamic range. Values from 0.6 to
     *              0.9 produce best results.
     * @return automatically generated
     */
     ...
     * Creates TonemapMantiuk object
     *
     * @param gamma gamma value for gamma correction. See createTonemap dynamic
     *              range. Values from 0.6 to 0.9 produce best results.
     * @return automatically generated
     */
    ...
    //
    // C++: Ptr_TonemapReinhard cv::createTonemapReinhard(float gamma = 1.0f, float
     ...
     *                    only on pixel value, if 0 it's global, otherwise it's a
     *                    weighted mean of this two cases.
     * @param color_adapt chromatic adaptation in [0, 1] range. If 1 channels are
     *                    treated independently, if 0 adaptation level is the same
     *                    for each channel.
     * @return automatically generated
     */
     ...
     * @param light_adapt light adaptation in [0, 1] range. If 1 adaptation is based
     *                    only on pixel value, if 0 it's global, otherwise it's a
     *                    weighted mean of this two cases. if 0 adaptation level is
     ...
     * @param gamma     gamma value for gamma correction. See createTonemap
     * @param intensity result intensity in [-8, 8] range. Greater intensity
     *                  produces brighter results. value, if 0 it's global,
     *                  otherwise it's a weighted mean of this two cases. if 0
     ...
     * Creates TonemapReinhard object
     *
     * @param gamma gamma value for gamma correction. See createTonemap value, if 0
     *              it's global, otherwise it's a weighted mean of this two cases.
     ...
     * Creates TonemapReinhard object
     * <p>
     * value, if 0 it's global, otherwise it's a weighted mean of this two cases. if
     * 0 adaptation level is the same for each channel.
     *
     * @return automatically generated
     */
    ...
    //
    // C++: void cv::colorChange(Mat src, Mat mask, Mat& dst, float red_mul = 1.0f,
    // float green_mul = 1.0f, float blue_mul = 1.0f)
    //

    /**
     * Given an original color image, two differently colored versions of this image
     * can be mixed seamlessly.
     *
    ...
    //
    // C++: void cv::denoise_TVL1(vector_Mat observations, Mat result, double lambda
    // = 1.0, int niters = 30)
    //
     ...
     * blog entry CITE: MA13 , which also contained (slightly more general)
     ...
     * finally it was slightly adapted by later authors.
     * <p>
     * Although the thorough discussion and justification of the algorithm involved
     * may be found in CITE: ChambolleEtAl, it might make sense to skim over it
     * here, following CITE: MA13 . To begin with, we consider the 1-byte gray-level
     * images as the functions from the rectangular domain of pixels (it may be seen
     * as set \(\left\{(x,y)\in\mathbb{N}\times\mathbb{N}\mid 1\leq x\leq n,\;1\leq
     * y\leq m\right\}\) for some \(m,\;n\in\mathbb{N}\)) into
     * \(\{0,1,\dots,255\}\). We shall denote the noised images as \(f_i\) and with
     * this view, given some image \(x\) of the same size, we may measure how bad it
     * is by the formula
     * <p>
     * \(\left\|\left\|\nabla x\right\|\right\| +
     * \lambda\sum_i\left\|\left\|x-f_i\right\|\right\|\)
     * <p>
     * \(\|\|\cdot\|\|\) here denotes \(L_2\)-norm and as you see, the first addend
     * states that we want our image to be smooth (ideally, having zero gradient,
     * thus being constant) and the second states that we want our result to be
     * close to the observations we've got. If we treat \(x\) as a function, this is
     ...
     * @param result       Here the denoised image will be stored. There is no need
     *                     to do pre-allocation of storage space, as it will be
     *                     automatically allocated, if necessary.
     * @param lambda       Corresponds to \(\lambda\) in the formulas above. As it
     *                     is enlarged, the smooth (blurred) images are treated more
     *                     favorably than detailed (but maybe more noised) ones.
     *                     Roughly speaking, as it becomes smaller, the result will
     *                     be more blur but more sever outliers will be removed.
     * @param niters       Number of iterations that the algorithm will run. Of
     *                     course, as more iterations as better, but it is hard to
     *                     quantitatively refine this statement, so just use the
     *                     default and increase it if the results are poor.
     */
     ...
     *                     Roughly speaking, as it becomes smaller, the result will
     *                     be more blur but more sever outliers will be removed.
     *                     better, but it is hard to quantitatively refine this
     *                     statement, so just use the default and increase it if the
     *                     results are poor.
     */
     ...
     * @param result       Here the denoised image will be stored. There is no need
     *                     to do pre-allocation of storage space, as it will be
     *                     automatically allocated, if necessary. (blurred) images
     *                     are treated more favorably than detailed (but maybe more
     *                     noised) ones. Roughly speaking, as it becomes smaller,
     *                     the result will be more blur but more sever outliers will
     *                     be removed. better, but it is hard to quantitatively
     *                     refine this statement, so just use the default and
    ...
    //
    // C++: void cv::edgePreservingFilter(Mat src, Mat& dst, int flags = 1, float
    ...
    //
    // C++: void cv::fastNlMeansDenoising(Mat src, Mat& dst, float h = 3, int
     ...
     * noise
     *
     * @param src                Input 8-bit 1-channel, 2-channel, 3-channel or
     *                           4-channel image.
     * @param dst                Output image with the same size and type as src .
     * @param templateWindowSize Size in pixels of the template patch that is used
     *                           to compute weights. Should be odd. Recommended
     *                           value 7 pixels
     * @param searchWindowSize   Size in pixels of the window that is used to
     *                           compute weighted average for given pixel. Should be
     *                           odd. Affect performance linearly: greater
     *                           searchWindowsSize - greater denoising time.
     *                           Recommended value 21 pixels
     * @param h                  Parameter regulating filter strength. Big h value
     *                           perfectly removes noise but also removes image
     *                           details, smaller h value preserves details but also
     *                           preserves some noise
     *                           <p>
     *                           This function expected to be applied to grayscale
     *                           images. For colored images look at
     *                           fastNlMeansDenoisingColored. Advanced usage of this
     *                           functions can be manual denoising of colored image
     *                           in different colorspaces. Such approach is used in
     *                           fastNlMeansDenoisingColored by converting image to
     *                           CIELAB colorspace and then separately denoise L and
     ...
     * @param templateWindowSize Size in pixels of the template patch that is used
     *                           to compute weights. Should be odd. Recommended
     *                           value 7 pixels given pixel. Should be odd. Affect
     ...
     * @param dst Output image with the same size and type as src . Should be odd.
     *            Recommended value 7 pixels given pixel. Should be odd. Affect
     *            performance linearly: greater searchWindowsSize - greater
     ...
     *            <p>
     *            This function expected to be applied to grayscale images. For
     *            colored images look at fastNlMeansDenoisingColored. Advanced usage
     *            of this functions can be manual denoising of colored image in
     *            different colorspaces. Such approach is used in
     *            fastNlMeansDenoisingColored by converting image to CIELAB
     *            colorspace and then separately denoise L and AB components with
     ...
     *            Recommended value 7 pixels given pixel. Should be odd. Affect
     *            performance linearly: greater searchWindowsSize - greater
     *            denoising time. Recommended value 21 pixels removes image details,
    ...
    //
    // C++: void cv::fastNlMeansDenoising(Mat src, Mat& dst, vector_float h, int
    // templateWindowSize = 7, int searchWindowSize = 21, int normType = NORM_L2)
    //
     ...
     * noise
     *
     * @param src                Input 8-bit or 16-bit (only with NORM_L1)
     *                           1-channel, 2-channel, 3-channel or 4-channel image.
     * @param dst                Output image with the same size and type as src .
     * @param templateWindowSize Size in pixels of the template patch that is used
     *                           to compute weights. Should be odd. Recommended
     *                           value 7 pixels
     ...
     *                           either one parameter applied to all channels or one
     *                           per channel in dst. Big h value perfectly removes
     *                           noise but also removes image details, smaller h
     *                           value preserves details but also preserves some
     *                           noise
     * @param normType           Type of norm used for weight calculation. Can be
     *                           either NORM_L2 or NORM_L1
     *                           <p>
     *                           This function expected to be applied to grayscale
     *                           images. For colored images look at
     *                           fastNlMeansDenoisingColored. Advanced usage of this
    ...
    public static void fastNlMeansDenoising(Mat src, Mat dst, MatOfFloat h, int templateWindowSize,
        ...
        Mat h_mat = h;
        fastNlMeansDenoising_4(src.nativeObj, dst.nativeObj, h_mat.nativeObj, templateWindowSize, searchWindowSize,
                normType);
    }
     ...
     * noise
     *
     * @param src Input 8-bit or 16-bit (only with NORM_L1) 1-channel, 2-channel,
     ...
     *            parameter applied to all channels or one per channel in dst. Big h
    ...
    //
    // C++: void cv::fastNlMeansDenoisingColored(Mat src, Mat& dst, float h = 3,
     ...
     *                           searchWindowsSize - greater denoising time.
     *                           Recommended value 21 pixels
     * @param h                  Parameter regulating filter strength for luminance
     *                           component. Bigger h value perfectly removes noise
     *                           but also removes image details, smaller h value
     *                           preserves details but also preserves some noise
     * @param hColor             The same as h but for color components. For most
     *                           images value equals 10 will be enough to remove
     *                           colored noise and do not distort colors
     *                           <p>
     *                           The function converts image to CIELAB colorspace
     *                           and then separately denoise L and AB components
     *                           with given h parameters using fastNlMeansDenoising
     *                           function.
     */
     ...
     *
     * @param src    Input 8-bit 3-channel image.
     * @param dst    Output image with the same size and type as src . Should be
     *               odd. Recommended value 7 pixels given pixel. Should be odd.
     ...
     *               distort colors
     *               <p>
     *               The function converts image to CIELAB colorspace and then
     *               separately denoise L and AB components with given h parameters
     *               using fastNlMeansDenoising function.
     */
     ...
     *            noise will be enough to remove colored noise and do not distort
     *            colors
     *            <p>
     *            The function converts image to CIELAB colorspace and then
     *            separately denoise L and AB components with given h parameters
     ...
     *            Recommended value 7 pixels given pixel. Should be odd. Affect
     *            performance linearly: greater searchWindowsSize - greater
     *            denoising time. Recommended value 21 pixels removes noise but also
     *            removes image details, smaller h value preserves details but also
     *            preserves some noise will be enough to remove colored noise and do
     *            not distort colors
     *            <p>
     *            The function converts image to CIELAB colorspace and then
     *            separately denoise L and AB components with given h parameters
    ...
    //
    // C++: void cv::fastNlMeansDenoisingColoredMulti(vector_Mat srcImgs, Mat& dst,
     ...
     * sequences
     *
     * @param srcImgs            Input 8-bit 3-channel images sequence. All images
     ...
     *                           imgToDenoiseIndex - temporalWindowSize / 2 from
     *                           srcImgs will be used to denoise
     ...
     *                           <p>
     *                           The function converts images to CIELAB colorspace
     *                           and then separately denoise L and AB components
     *                           with given h parameters using
     ...
     *                           srcImgs[imgToDenoiseIndex] image.
     * @param dst                Output image with the same size and type as srcImgs
     *                           images. Should be odd. Recommended value 7 pixels
     *                           given pixel. Should be odd. Affect performance
     *                           linearly: greater searchWindowsSize - greater
     *                           denoising time. Recommended value 21 pixels
     * @param h                  Parameter regulating filter strength for luminance
     *                           component. Bigger h value perfectly removes noise
     *                           but also removes image details, smaller h value
    ...
    public static void fastNlMeansDenoisingColoredMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
        ...
        Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
        fastNlMeansDenoisingColoredMulti_2(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
                h, hColor);
    }
     ...
     */
    public static void fastNlMeansDenoisingColoredMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
            int temporalWindowSize, float h) {
        Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
        fastNlMeansDenoisingColoredMulti_3(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
                h);
    }
     ...
     *                           given pixel. Should be odd. Affect performance
     *                           linearly: greater searchWindowsSize - greater
     *                           denoising time. Recommended value 21 pixels removes
     *                           noise but also removes image details, smaller h
     *                           value preserves details but also preserves some
     *                           noise.
     *                           <p>
    ...
    //
    // C++: void cv::fastNlMeansDenoisingMulti(vector_Mat srcImgs, Mat& dst, int
    // imgToDenoiseIndex, int temporalWindowSize, float h = 3, int
    // templateWindowSize = 7, int searchWindowSize = 21)
    //

    /**
     * Modification of fastNlMeansDenoising function for images sequence where
     ...
     *                           4-channel images sequence. All images should have
     ...
     *                           searchWindowsSize - greater denoising time.
     *                           Recommended value 21 pixels
     * @param h                  Parameter regulating filter strength. Bigger h
     *                           value perfectly removes noise but also removes
     *                           image details, smaller h value preserves details
     ...
     */
    public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
            int temporalWindowSize, float h, int templateWindowSize) {
        Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
        fastNlMeansDenoisingMulti_1(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize, h,
                templateWindowSize);
    }
     ...
     * &lt;http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.131.6394&gt;
     *
     * @param srcImgs            Input 8-bit or 16-bit (only with NORM_L1)
     *                           1-channel, 2-channel, 3-channel or 4-channel images
     *                           sequence. All images should have the same type and
     *                           size.
     * @param imgToDenoiseIndex  Target image to denoise index in srcImgs sequence
     * @param temporalWindowSize Number of surrounding images to use for target
     *                           image denoising. Should be odd. Images from
    ...
    public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
        ...
        Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
        Mat h_mat = h;
        fastNlMeansDenoisingMulti_4(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
     ...
     */
    public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
            int temporalWindowSize, MatOfFloat h, int templateWindowSize, int searchWindowSize) {
        Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
        Mat h_mat = h;
        fastNlMeansDenoisingMulti_5(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
                h_mat.nativeObj, templateWindowSize, searchWindowSize);
    }
     ...
     */
    public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
            int temporalWindowSize, MatOfFloat h, int templateWindowSize) {
        Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
        Mat h_mat = h;
        fastNlMeansDenoisingMulti_6(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
                h_mat.nativeObj, templateWindowSize);
    }
    ...
    public static void fastNlMeansDenoisingMulti(List<Mat> srcImgs, Mat dst, int imgToDenoiseIndex,
        ...
        Mat srcImgs_mat = Converters.vector_Mat_to_Mat(srcImgs);
        Mat h_mat = h;
        fastNlMeansDenoisingMulti_7(srcImgs_mat.nativeObj, dst.nativeObj, imgToDenoiseIndex, temporalWindowSize,
                h_mat.nativeObj);
    }

    //
    // C++: void cv::illuminationChange(Mat src, Mat mask, Mat& dst, float alpha =
     ...
     *              <p>
     *              This is useful to highlight under-exposed foreground objects or
    ...
    //
    // C++: void cv::inpaint(Mat src, Mat inpaintMask, Mat& dst, double
     ...
     *
     * @param src           Input 8-bit, 16-bit unsigned or 32-bit float 1-channel
     *                      or 8-bit 3-channel image.
     ...
     *                      cv::INPAINT_TELEA
     *                      <p>
     *                      The function reconstructs the selected image area from
     *                      the pixel near the area boundary. The function may be
     *                      used to remove dust and scratches from a scanned photo,
     *                      or to remove undesirable objects from still images or
     *                      video. See
     *                      &lt;http://en.wikipedia.org/wiki/Inpainting&gt; for more
     *                      details.
     *
     *                      <b>Note:</b>
     *                      <ul>
     *                      <li>An example using the inpainting technique can be
     *                      found at opencv_source_code/samples/cpp/inpaint.cpp</li>
     *                      <li>(Python) An example using the inpainting technique
     *                      can be found at
     *                      opencv_source_code/samples/python/inpaint.py</li>
     *                      </ul>
     */
    ...
    //
    // C++: void cv::pencilSketch(Mat src, Mat& dst1, Mat& dst2, float sigma_s = 60,
    ...
    //
    // C++: void cv::seamlessClone(Mat src, Mat dst, Mat mask, Point p, Mat& blend,
    // int flags)
    //
     ...
     * @param flags Cloning method that could be cv::NORMAL_CLONE, cv::MIXED_CLONE
     *              or cv::MONOCHROME_TRANSFER
     */
    ...
    //
    // C++: void cv::stylization(Mat src, Mat& dst, float sigma_s = 60, float
    // sigma_r = 0.45f)
    //
    ...
    //
    // C++: void cv::textureFlattening(Mat src, Mat mask, Mat& dst, float
     ...
     * its contents a flat aspect. Here Canny Edge %Detector is used.
     *
     * @param src            Input 8-bit 3-channel image.
     * @param mask           Input 8-bit 1 or 3-channel image.
     * @param dst            Output image with the same size and type as src.
     ...
     *
     *                       <b>Note:</b> The algorithm assumes that the color of
     *                       the source image is close to that of the destination.
     *                       This assumption means that when the colors don't match,
     *                       the source image color gets tinted toward the color of
     ...
     * @param low_threshold %Range from 0 to 100.
     *
     *                      <b>Note:</b> The algorithm assumes that the color of the
     *                      source image is close to that of the destination. This
     *                      assumption means that when the colors don't match, the
     *                      source image color gets tinted toward the color of the
     *                      destination image.
     */
     ...
     * @param dst  Output image with the same size and type as src.
     *
     *             <b>Note:</b> The algorithm assumes that the color of the source
     *             image is close to that of the destination. This assumption means
     *             that when the colors don't match, the source image color gets
     *             tinted toward the color of the destination image.
     */
    ...
    private static native void colorChange_1(long src_nativeObj, long mask_nativeObj, long dst_nativeObj, float red_mul,
            float green_mul);

    private static native void colorChange_2(long src_nativeObj, long mask_nativeObj, long dst_nativeObj,
            float red_mul);
    ...
    private static native void fastNlMeansDenoising_1(long src_nativeObj, long dst_nativeObj, float h,
    ...
    // C++: void cv::fastNlMeansDenoising(Mat src, Mat& dst, vector_float h, int
    // templateWindowSize = 7, int searchWindowSize = 21, int normType = NORM_L2)
    private static native void fastNlMeansDenoising_4(long src_nativeObj, long dst_nativeObj, long h_mat_nativeObj,
            int templateWindowSize, int searchWindowSize, int normType);

    private static native void fastNlMeansDenoising_5(long src_nativeObj, long dst_nativeObj, long h_mat_nativeObj,
            int templateWindowSize, int searchWindowSize);

    private static native void fastNlMeansDenoising_6(long src_nativeObj, long dst_nativeObj, long h_mat_nativeObj,
            int templateWindowSize);
    ...
    private static native void fastNlMeansDenoisingColored_1(long src_nativeObj, long dst_nativeObj, float h,
            float hColor, int templateWindowSize);

    private static native void fastNlMeansDenoisingColored_2(long src_nativeObj, long dst_nativeObj, float h,
            float hColor);
    ...
    private static native void fastNlMeansDenoisingColoredMulti_2(long srcImgs_mat_nativeObj, long dst_nativeObj,
            int imgToDenoiseIndex, int temporalWindowSize, float h, float hColor);

    private static native void fastNlMeansDenoisingColoredMulti_3(long srcImgs_mat_nativeObj, long dst_nativeObj,
            int imgToDenoiseIndex, int temporalWindowSize, float h);

    private static native void fastNlMeansDenoisingColoredMulti_4(long srcImgs_mat_nativeObj, long dst_nativeObj,
    ...
    private static native void fastNlMeansDenoisingMulti_1(long srcImgs_mat_nativeObj, long dst_nativeObj,
            int imgToDenoiseIndex, int temporalWindowSize, float h, int templateWindowSize);
    ...
    // imgToDenoiseIndex, int temporalWindowSize, vector_float h, int
    // templateWindowSize = 7, int searchWindowSize = 21, int normType = NORM_L2)
    private static native void fastNlMeansDenoisingMulti_4(long srcImgs_mat_nativeObj, long dst_nativeObj,
            int imgToDenoiseIndex, int temporalWindowSize, long h_mat_nativeObj, int templateWindowSize,
            int searchWindowSize, int normType);

    private static native void fastNlMeansDenoisingMulti_5(long srcImgs_mat_nativeObj, long dst_nativeObj,
            int imgToDenoiseIndex, int temporalWindowSize, long h_mat_nativeObj, int templateWindowSize,
            int searchWindowSize);

    private static native void fastNlMeansDenoisingMulti_6(long srcImgs_mat_nativeObj, long dst_nativeObj,
            int imgToDenoiseIndex, int temporalWindowSize, long h_mat_nativeObj, int templateWindowSize);

    private static native void fastNlMeansDenoisingMulti_7(long srcImgs_mat_nativeObj, long dst_nativeObj,
    ...
    private static native void pencilSketch_2(long src_nativeObj, long dst1_nativeObj, long dst2_nativeObj,
            float sigma_s);
    ...
    private static native void textureFlattening_2(long src_nativeObj, long mask_nativeObj, long dst_nativeObj,
            float low_threshold);
