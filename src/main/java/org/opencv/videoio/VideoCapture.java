 ...
 * <p>
 * Here is how the class can be used: INCLUDE:
 * samples/cpp/videocapture_basic.cpp
 *
 * <b>Note:</b> In REF: videoio_c "C API" the black-box structure
 * {@code CvCapture} is used instead of %VideoCapture. <b>Note:</b>
 * <ul>
 * <li>(C++) A basic sample on using the %VideoCapture interface can be found at
 * {@code OPENCV_SOURCE_CODE/samples/cpp/videocapture_starter.cpp}</li>
 * <li>(Python) A basic sample on using the %VideoCapture interface can be found
 * at {@code OPENCV_SOURCE_CODE/samples/python/video.py}</li>
 * <li>(Python) A multi threaded video processing sample can be found at
 * {@code OPENCV_SOURCE_CODE/samples/python/video_threaded.py}</li>
 * <li>(Python) %VideoCapture sample showcasing some features of the
 * Video4Linux2 backend {@code OPENCV_SOURCE_CODE/samples/python/video_v4l2.py}
 * </li>
 * </ul>
 */
public class VideoCapture {
    ...
    /**
     * Default constructor <b>Note:</b> In REF: videoio_c "C API", when you finished
     * working with video, release CvCapture structure with cvReleaseCapture(), or
     * use Ptr&lt;CvCapture&gt; that calls cvReleaseCapture() automatically in the
     * destructor.
     */
    public VideoCapture() {
    ...
    //
    // C++: explicit cv::VideoCapture::VideoCapture(String filename, int
    ...
    //
    // C++: explicit cv::VideoCapture::VideoCapture(int index, int apiPreference =
    // CAP_ANY)
    //
     ...
     *         and returns true (non-zero) in the case of success.
     *         <p>
     *         The primary use of the function is in multi-camera environments,
     *         especially when the cameras do not have hardware synchronization.
     *         That is, you call VideoCapture::grab() for each camera and after that
     *         call the slower method VideoCapture::retrieve() to decode and get
     *         frame from each camera. This way the overhead on demosaicing or
     *         motion jpeg decompression etc. is eliminated and the retrieved frames
     *         from different cameras will be closer in time.
     *         <p>
     *         Also, when a connected camera is multi-head (for example, a stereo
     *         camera or a Kinect device), the correct way of retrieving data from
     *         it is to call VideoCapture::grab() first and then call
     *         VideoCapture::retrieve() one or more times with different values of
     ...
     *         <p>
     *         The method first calls VideoCapture::release to close the already
     ...
     *         returns the just grabbed frame. If no frames has been grabbed (camera
     *         has been disconnected, or there are no more frames in video file),
     *         the method returns false and the function returns empty image (with
     *         %cv::Mat, test it with Mat::empty()).
     *
     *         <b>Note:</b> In REF: videoio_c "C API", functions cvRetrieveFrame()
     *         and cv.RetrieveFrame() return image stored inside the video capturing
     *         structure. It is not allowed to modify or release the image! You can
     *         copy the frame using cvCloneImage and then do whatever you want with
     *         the copy.
     */
     ...
     * @return {@code false} if no frames has been grabbed
     *         <p>
     *         The method decodes and returns the just grabbed frame. If no frames
     *         has been grabbed (camera has been disconnected, or there are no more
     *         frames in video file), the method returns false and the function
     *         returns an empty image (with %cv::Mat, test it with Mat::empty()).
     *         <p>
     *         SEE: read()
     *
     *         <b>Note:</b> In REF: videoio_c "C API", functions cvRetrieveFrame()
     ...
     *               cv::CAP_PROP_POS_MSEC, cv::CAP_PROP_POS_FRAMES, ...) or one
     *               from REF: videoio_flags_others
     * @param value  Value of the property.
     * @return {@code true} if the property is supported by backend used by the
     *         VideoCapture instance. <b>Note:</b> Even if it returns {@code true}
     *         this doesn't ensure that the property value has been accepted by the
     *         capture device. See note in VideoCapture::get()
     */
     ...
     *               cv::CAP_PROP_POS_MSEC, cv::CAP_PROP_POS_FRAMES, ...) or one
     *               from REF: videoio_flags_others
     * @return Value for the specified property. Value 0 is returned when querying a
     *         property that is not supported by the backend used by the
     *         VideoCapture instance.
     *
     *         <b>Note:</b> Reading / writing properties involves many layers. Some
     *         unexpected result might happens along this chain. <code>
     * VideoCapture -&gt; API Backend -&gt; Operating System -&gt; Device Driver -&gt; Device Hardware
     * </code> The returned value might be different from what really used by the
     *         device or it could be encoded using device dependent rules (eg. steps
     *         or percentage). Effective behaviour depends from device driver and
     *         API Backend
     */
