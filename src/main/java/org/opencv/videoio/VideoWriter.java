...
public class VideoWriter {
    ...
    //
    // C++: cv::VideoWriter::VideoWriter(String filename, int apiPreference, int
    // fourcc, double fps, Size frameSize, bool isColor = true)
    //

    /**
     * The {@code apiPreference} parameter allows to specify API backends to use.
    ...
    //
    // C++: cv::VideoWriter::VideoWriter(String filename, int fourcc, double fps,
     ...
     *                  example, VideoWriter::fourcc('P','I','M','1') is a MPEG-1
     *                  codec, VideoWriter::fourcc('M','J','P','G') is a motion-jpeg
     *                  codec etc. List of codes can be obtained at [Video Codecs by
     *                  FOURCC](http://www.fourcc.org/codecs.php) page. FFMPEG
     *                  backend with MP4 container natively uses other values as
     *                  fourcc code: see
     *                  [ObjectType](http://www.mp4ra.org/codecs.html), so you may
     ...
     *                  flag is currently supported on Windows only).
     *
     *                  <b>Tips</b>:
     *                  <ul>
     *                  <li>With some backends {@code fourcc=-1} pops up the codec
     *                  selection dialog from the system.</li>
     *                  <li>To save image sequence use a proper filename (eg.
     *                  {@code img_%02d.jpg}) and {@code fourcc=0} OR {@code fps=0}.
     *                  Use uncompressed image format (eg. {@code img_%02d.BMP}) to
     *                  save raw frames.</li>
     *                  <li>Most codecs are lossy. If you want lossless video file
     *                  you need to use a lossless codecs (eg. FFMPEG FFV1, Huffman
     *                  HFYU, Lagarith LAGS, etc...)</li>
     *                  <li>If FFMPEG is enabled, using {@code codec=0; fps=0;} you
     *                  can create an uncompressed (raw) video file.</li>
     *                  </ul>
     */
     ...
     *                  conversion.
     * @param fps       Framerate of the created video stream.
     * @param frameSize Size of the video frames. will work with grayscale frames
    ...
    //
    // C++: bool cv::VideoWriter::open(String filename, int apiPreference, int
    // fourcc, double fps, Size frameSize, bool isColor = true)
    //
    ...
    //
    // C++: bool cv::VideoWriter::open(String filename, int fourcc, double fps, Size
     ...
     *         <p>
     *         The method first calls VideoWriter::release to close the already
     *         opened file.
     */
     ...
     *               cv::VIDEOWRITER_PROP_QUALITY) or one of REF:
     *               videoio_flags_others
     * @param value  Value of the property.
     * @return {@code true} if the property is supported by the backend used by the
     ...
     *         property that is not supported by the backend used by the VideoWriter
     *         instance.
     */
     ...
     *         in the constructor VideoWriter::VideoWriter or VideoWriter::open.
     */
     ...
     * <p>
     * The method is automatically called by subsequent VideoWriter::open and by the
     ...
     *              format.
     *              <p>
     *              The function/method writes the specified image to video file. It
     *              must have the same size as has been specified when opening the
     *              video writer.
     */
