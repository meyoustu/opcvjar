...
public class Imgproc {
    ...
    public static final int INTER_NEAREST = 0, INTER_LINEAR = 1, INTER_CUBIC = 2, INTER_AREA = 3, INTER_LANCZOS4 = 4,
            INTER_LINEAR_EXACT = 5, INTER_MAX = 7, WARP_FILL_OUTLIERS = 8, WARP_INVERSE_MAP = 16;
    ...
    //
    // C++: Mat cv::getGaborKernel(Size ksize, double sigma, double theta, double
     ...
     * <p>
     * The function computes and returns the \(\texttt{ksize} \times 1\) matrix of
     ...
     *
     * @param ksize Aperture size. It should be odd ( \(\texttt{ksize} \mod 2 = 1\)
     *              ) and positive.
     ...
     * @param ktype Type of filter coefficients. It can be CV_32F or CV_64F . SEE:
     *              sepFilter2D, getDerivKernels, getStructuringElement,
     *              GaussianBlur
     * @return automatically generated
     */
     ...
     * @param sigma Gaussian standard deviation. If it is non-positive, it is
     *              computed from ksize as
     *              {@code sigma = 0.3*((ksize-1)*0.5 - 1) + 0.8}. SEE: sepFilter2D,
    ...
    //
    // C++: Mat cv::getPerspectiveTransform(Mat src, Mat dst, int solveMethod =
    // DECOMP_LU)
    //
     ...
     * points.
     * <p>
     * The function calculates the \(3 \times 3\) matrix of a perspective transform
     * so that:
     * <p>
     * \(\begin{bmatrix} t_i x'_i \\ t_i y'_i \\ t_i \end{bmatrix} =
     * \texttt{map_matrix} \cdot \begin{bmatrix} x_i \\ y_i \\ 1 \end{bmatrix}\)
     * <p>
     * where
     * <p>
     ...
     * @param dst         Coordinates of the corresponding quadrangle vertices in
     ...
     * <p>
     * \(\begin{bmatrix} \alpha &amp; \beta &amp; (1- \alpha ) \cdot
     * \texttt{center.x} - \beta \cdot \texttt{center.y} \\ - \beta &amp; \alpha
     * &amp; \beta \cdot \texttt{center.x} + (1- \alpha ) \cdot \texttt{center.y}
     * \end{bmatrix}\)
     * <p>
     * where
     * <p>
     * \(\begin{array}{l} \alpha = \texttt{scale} \cdot \cos \texttt{angle} , \\
     * \beta = \texttt{scale} \cdot \sin \texttt{angle} \end{array}\)
     * <p>
    ...
    //
    // C++: Mat cv::getStructuringElement(int shape, Size ksize, Point anchor =
    // Point(-1,-1))
    //
     ...
     * @param anchor Anchor position within the element. The default value \((-1,
     *               -1)\) means that the anchor is at the center. Note that only
     *               the shape of a cross-shaped element depends on the anchor
     *               position. In other cases the anchor just regulates how much the
     ...
     *
     * @param shape Element shape that could be one of #MorphShapes
     * @param ksize Size of the structuring element. anchor is at the center. Note
     *              that only the shape of a cross-shaped element depends on the
     *              anchor position. In other cases the anchor just regulates how
     *              much the result of the morphological operation is shifted.
     * @return automatically generated
     */
     ...
     *                    array) or an array ( \(1 \times N\) or \(N \times 1\) ) of
     *                    2D points (Point or Point2f ).
     * @param binaryImage If it is true, all non-zero image pixels are treated as
     *                    1's. The parameter is used for images only.
     * @return moments.
     *
     *         <b>Note:</b> Only applicable to contour moments calculations from
     *         Python bindings: Note that the numpy type for the input array should
     *         be either np.int32 or np.float32.
     *         <p>
     ...
     *              or an array ( \(1 \times N\) or \(N \times 1\) ) of 2D points
     *              (Point or Point2f ). used for images only.
     * @return moments.
     *
     *         <b>Note:</b> Only applicable to contour moments calculations from
     *         Python bindings: Note that the numpy type for the input array should
    ...
    //
    // C++: Point2d cv::phaseCorrelate(Mat src1, Mat src2, Mat window = Mat(),
    // double* response = 0)
    //
     ...
     * possible edge effects. This window is cached until the array size changes to
     * speed up processing time.</li>
     * <li>Next it computes the forward DFTs of each source array: \(\mathbf{G}_a =
     * \mathcal{F}\{src_1\}, \; \mathbf{G}_b = \mathcal{F}\{src_2\}\) where
     * \(\mathcal{F}\) is the forward DFT.</li>
     * <li>It then computes the cross-power spectrum of each frequency domain array:
     * \(R = \frac{ \mathbf{G}_a \mathbf{G}_b^*}{|\mathbf{G}_a \mathbf{G}_b^*|}\)
     * </li>
     * <li>Next the cross-correlation is converted back into the time domain via the
     * inverse DFT: \(r = \mathcal{F}^{-1}\{R\}\)</li>
     * <li>Finally, it computes the peak location and computes a 5x5 weighted
     * centroid around the peak to achieve sub-pixel accuracy. \((\Delta x, \Delta
     * y) = \texttt{weightedCentroid} \{\arg \max_{(x, y)}\{r\}\}\)</li>
     ...
     * smaller when there are multiple peaks.</li>
     * </ul>
     *
    ...
    //
    // C++: Ptr_CLAHE cv::createCLAHE(double clipLimit = 40.0, Size tileGridSize =
    // Size(8, 8))
    //
     ...
     * @param tileGridSize Size of grid for histogram equalization. Input image will
     *                     be divided into equally sized rectangular tiles.
     *                     tileGridSize defines the number of tiles in row and
     *                     column.
     * @return automatically generated
     */
     ...
     * Creates a smart pointer to a cv::CLAHE class and initializes it.
     *
     * @param clipLimit Threshold for contrast limiting. equally sized rectangular
     *                  tiles. tileGridSize defines the number of tiles in row and
     *                  column.
     * @return automatically generated
     */
     ...
     * Creates a smart pointer to a cv::CLAHE class and initializes it.
     * <p>
     * equally sized rectangular tiles. tileGridSize defines the number of tiles in
     * row and column.
     *
     * @return automatically generated
     */
    ...
    /**
     * Creates a smart pointer to a cv::GeneralizedHoughBallard class and
     * initializes it.
     *
     * @return automatically generated
     */
    ...
    /**
     * Creates a smart pointer to a cv::GeneralizedHoughGuil class and initializes
     * it.
     *
     * @return automatically generated
     */
    ...
    //
    // C++: Ptr_LineSegmentDetector cv::createLineSegmentDetector(int _refine =
    // LSD_REFINE_STD, double _scale = 0.8, double _sigma_scale = 0.6, double _quant
    // = 2.0, double _ang_th = 22.5, double _log_eps = 0, double _density_th = 0.7,
    // int _n_bins = 1024)
    //
     ...
     */
    public static LineSegmentDetector createLineSegmentDetector(int _refine, double _scale, double _sigma_scale,
            double _quant, double _ang_th, double _log_eps, double _density_th) {
        return LineSegmentDetector.__fromPtr__(
                createLineSegmentDetector_1(_refine, _scale, _sigma_scale, _quant, _ang_th, _log_eps, _density_th));
    }
    ...
    public static LineSegmentDetector createLineSegmentDetector(int _refine, double _scale, double _sigma_scale,
        ...
        return LineSegmentDetector
                .__fromPtr__(createLineSegmentDetector_2(_refine, _scale, _sigma_scale, _quant, _ang_th, _log_eps));
    }
     ...
     */
    public static LineSegmentDetector createLineSegmentDetector(int _refine, double _scale, double _sigma_scale,
            double _quant, double _ang_th) {
        return LineSegmentDetector
                .__fromPtr__(createLineSegmentDetector_3(_refine, _scale, _sigma_scale, _quant, _ang_th));
    }
     ...
     * @param _sigma_scale Sigma for Gaussian filter. It is computed as sigma =
     *                     _sigma_scale/_scale.
     * @param _quant       Bound to the quantization error on the gradient norm. is
     *                     chosen.
     *
     *                     <b>Note:</b> Implementation has been removed due original
     *                     code license conflict
     ...
     *                Range (0..1]. is chosen.
     *
     *                <b>Note:</b> Implementation has been removed due original code
     *                license conflict
     * @return automatically generated
     */
     ...
     *
     * @param _refine The way found lines will be refined, see
     *                #LineSegmentDetectorModes is chosen.
     *
     *                <b>Note:</b> Implementation has been removed due original code
     *                license conflict
     * @return automatically generated
     ...
     * is chosen.
     *
     * <b>Note:</b> Implementation has been removed due original code license
     * conflict
     *
     * @return automatically generated
     */
     ...
     * ellipse is inscribed. The first algorithm described by CITE: Fitzgibbon95 is
     ...
     * Fits an ellipse around a set of 2D points.
     * <p>
     * The function calculates the ellipse that fits a set of 2D points. It returns
     * the rotated rectangle in which the ellipse is inscribed. The Approximate Mean
     * Square (AMS) proposed by CITE: Taubin1991 is used.
     * <p>
     * For an ellipse, this basis set is \( \chi= \left(x^2, x y, y^2, x, y,
     * 1\right) \), which is a set of six free coefficients \(
     ...
     * and minor axes lengths \( (a,b) \), the position \( (x_0,y_0) \), and the
     * orientation \( \theta \). This is because the basis set includes lines,
     * quadratics, parabolic and hyperbolic functions as well as elliptical
     * functions as possible fits. If the fit is found to be a parabolic or
     * hyperbolic function then the standard #fitEllipse method is used. The AMS
     * method restricts the fit to parabolic, hyperbolic and elliptical curves by
     * imposing the condition that \( A^T ( D_x^T D_x + D_y^T D_y) A = 1 \) where
     * the matrices \( Dx \) and \( Dy \) are the partial derivatives of the design
     * matrix \( D \) with respect to x and y. The matrices are formed row by row
     * applying the following to each of the points in the set: \(align*}{
     * D(i,:)&amp;=\left\{x_i^2, x_i y_i, y_i^2, x_i, y_i, 1\right\} &amp;
     * D_x(i,:)&amp;=\left\{2 x_i,y_i,0,1,0,0\right\} &amp;
     * D_y(i,:)&amp;=\left\{0,x_i,2 y_i,0,1,0\right\} \) The AMS method minimizes
     * the cost function \(equation*}{ \epsilon ^2=\frac{ A^T D^T D A }{ A^T (D_x^T
     * D_x + D_y^T D_y) A^T } \)
     * <p>
     ...
     * <p>
     * The function calculates the ellipse that fits a set of 2D points. It returns
     * the rotated rectangle in which the ellipse is inscribed. The Direct least
     * square (Direct) method by CITE: Fitzgibbon1999 is used.
     * <p>
     * For an ellipse, this basis set is \( \chi= \left(x^2, x y, y^2, x, y,
     * 1\right) \), which is a set of six free coefficients \(
     * A^T=\left\{A_{\text{xx}},A_{\text{xy}},A_{\text{yy}},A_x,A_y,A_0\right\} \).
     ...
     * orientation \( \theta \). This is because the basis set includes lines,
     * quadratics, parabolic and hyperbolic functions as well as elliptical
     * functions as possible fits. The Direct method confines the fit to ellipses by
     * ensuring that \( 4 A_{xx} A_{yy}- A_{xy}^2 &gt; 0 \). The condition imposed
     * is that \( 4 A_{xx} A_{yy}- A_{xy}^2=1 \) which satisfies the inequality and
     * as the coefficients can be arbitrarily scaled is not overly restrictive.
     * <p>
     * \(equation*}{ \epsilon ^2= A^T D^T D A \quad \text{with} \quad A^T C A =1
     * \quad \text{and} \quad C=\left(\begin{matrix} 0 &amp; 0 &amp; 2 &amp; 0 &amp;
     ...
     * <p>
     * The system produces only one positive eigenvalue \( \lambda\) which is chosen
     * as the solution with its eigenvector \(\mathbf{u}\). These are used to find
     * the coefficients
     * <p>
     * \(equation*}{ A = \sqrt{\frac{1}{\mathbf{u}^T C \mathbf{u}}} \mathbf{u} \)
     ...
     * that the returned RotatedRect can contain negative indices when data is close
     ...
     *                 signed area value, depending on the contour orientation
     *                 (clockwise or counter-clockwise). Using this feature you can
     *                 determine orientation of a contour by taking the sign of an
     *                 area. By default, the parameter is false, which means that
     ...
     *
     * @param contour Input vector of 2D points (contour vertices), stored in
     *                std::vector or Mat. depending on the contour orientation
     *                (clockwise or counter-clockwise). Using this feature you can
     *                determine orientation of a contour by taking the sign of an
     *                area. By default, the parameter is false, which means that the
    ...
    //
    // C++: double cv::getFontScaleFromHeight(int fontFace, int pixelHeight, int
    // thickness = 1)
    //
    ...
    //
    // C++: double cv::matchShapes(Mat contour1, Mat contour2, int method, double
    // parameter)
    //
    ...
    /**
     * Finds a triangle of minimum area enclosing a 2D point set and returns its
     * area.
     * <p>
     ...
     * the image below. 2D points are depicted in red* and the enclosing triangle in
     * *yellow*.
     * <p>
     * ![Sample output of the minimum enclosing triangle
     * function](pics/minenclosingtriangle.png)
     * <p>
     * The implementation of the algorithm is based on O'Rourke's CITE: ORourke86
     * and Klee and Laskowski's CITE: KleeLaskowski85 papers. O'Rourke provides a
     * \(\theta(n)\) algorithm for finding the minimal enclosing triangle of a 2D
     * convex polygon with n vertices. Since the #minEnclosingTriangle function
     ...
     * #convexHull function is \(O(n log(n))\) which is higher than \(\theta(n)\).
     * Thus the overall complexity of the function is \(O(n log(n))\).
     *
     * @param points   Input vector of 2D points with depth CV_32S or CV_32F, stored
    ...
    //
    // C++: double cv::pointPolygonTest(vector_Point2f contour, Point2f pt, bool
    // measureDist)
    //
     ...
     *                    the point to the nearest contour edge. Otherwise, the
     *                    function only checks if the point is inside a contour or
     *                    not.
     * @return automatically generated
     */
    ...
    //
    // C++: double cv::threshold(Mat src, Mat& dst, double thresh, double maxval,
    // int type)
    //
     ...
     *
     * <b>Note:</b> Currently, the Otsu's and Triangle methods are implemented only
    ...
    //
    // C++: float cv::intersectConvexConvex(Mat _p1, Mat _p2, Mat& _p12, bool
    // handleNested = true)
    //
     ...
     * @param handleNested When true, an intersection is found if one of the
     *                     polygons is fully enclosed in the other. When false, no
     *                     intersection is found. If the polygons share a side or
     *                     the vertex of one polygon lies on an edge of the other,
     *                     they are not considered nested and an intersection will
     *                     be found regardless of the value of handleNested.
     * @return Absolute value of area of intersecting polygon
     *
     *         <b>Note:</b> intersectConvexConvex doesn't confirm that both polygons
     *         are convex and will return invalid results if they aren't.
     */
     ...
     * @param _p1  First polygon
     * @param _p2  Second polygon
     * @param _p12 Output polygon describing the intersecting area When false, no
     *             intersection is found. If the polygons share a side or the vertex
     *             of one polygon lies on an edge of the other, they are not
     *             considered nested and an intersection will be found regardless of
    ...
    //
    // C++: float cv::wrapperEMD(Mat signature1, Mat signature2, int distType, Mat
    // cost = Mat(), Ptr_float& lowerBound = Ptr<float>(), Mat& flow = Mat())
    //
     ...
     * distance between the two weighted point configurations. One of the
     * applications described in CITE: RubnerSept98, CITE: Rubner2000 is
     ...
     * algorithm, thus the complexity is exponential in the worst case, though, on
     * average it is much faster. In the case of a real metric the lower boundary
     * can be calculated even faster (using linear-time algorithm) and it can be
     * used to determine roughly whether the two signatures are far enough so that
     * they cannot relate to the same object.
     *
     * @param signature1 First signature, a \(\texttt{size1}\times \texttt{dims}+1\)
     *                   floating-point matrix. Each row stores the point weight
     *                   followed by the point coordinates. The matrix is allowed to
     *                   have a single column (weights only) if the user-defined
     *                   cost matrix is used. The weights must be non-negative and
     *                   have at least one non-zero value.
     * @param signature2 Second signature of the same format as signature1 , though
     *                   the number of rows may be different. The total weights may
     *                   be different. In this case an extra "dummy" point is added
     *                   to either signature1 or signature2. The weights must be
     *                   non-negative and have at least one non-zero value.
     * @param distType   Used metric. See #DistanceTypes.
     * @param cost       User-defined \(\texttt{size1}\times \texttt{size2}\) cost
     *                   matrix. Also, if a cost matrix is used, lower boundary
     *                   lowerBound cannot be calculated because it needs a metric
     *                   function. signatures that is a distance between mass
     *                   centers. The lower boundary may not be calculated if the
     *                   user-defined cost matrix is used, the total weights of
     *                   point configurations are not equal, or if the signatures
     *                   consist of weights only (the signature matrices have a
     *                   single column). You <b>must</b> initialize \*lowerBound .
     *                   If the calculated distance between mass centers is greater
     *                   or equal to \*lowerBound (it means that the signatures are
     *                   far enough), the function does not calculate EMD. In any
     *                   case \*lowerBound is set to the calculated distance between
     *                   mass centers on return. Thus, if you want to calculate both
     *                   distance between mass centers and EMD, \*lowerBound should
     *                   be set to 0.
     * @param flow       Resultant \(\texttt{size1} \times \texttt{size2}\) flow
     *                   matrix: \(\texttt{flow}_{i,j}\) is a flow from \(i\) -th
     *                   point of signature1 to \(j\) -th point of signature2 .
     * @return automatically generated
     */
     ...
     *                   mass centers on return. Thus, if you want to calculate both
     *                   distance between mass centers and EMD, \*lowerBound should
     *                   be set to 0. a flow from \(i\) -th point of signature1 to
     *                   \(j\) -th point of signature2 .
     * @return automatically generated
     */
     ...
     *                   to either signature1 or signature2. The weights must be
     *                   non-negative and have at least one non-zero value.
     * @param distType   Used metric. See #DistanceTypes. is used, lower boundary
     *                   lowerBound cannot be calculated because it needs a metric
     *                   function. signatures that is a distance between mass
     *                   centers. The lower boundary may not be calculated if the
     *                   user-defined cost matrix is used, the total weights of
    ...
    //
    // C++: int cv::connectedComponents(Mat image, Mat& labels, int connectivity,
     ...
     * [0, N-1] where 0 represents the background label. ltype specifies the output
     * label image type, an important consideration based on the total number of
     * labels or alternatively the total number of pixels in the source image.
     * ccltype specifies the connected components labeling algorithm to use,
    ...
    //
    // C++: int cv::connectedComponentsWithStats(Mat image, Mat& labels, Mat& stats,
    // Mat& centroids, int connectivity, int ltype, int ccltype)
    //
     ...
     * #ConnectedComponentsAlgorithmsTypes for details. Note that SAUF algorithm
     * forces a row major ordering of labels while BBDT does not. This function uses
     * parallel version of both Grana and Wu's algorithms (statistics included) if
     ...
     * @param image        the 8-bit single-channel image to be labeled
     * @param labels       destination labeled image
     * @param stats        statistics output for each label, including the
     *                     background label, see below for available statistics.
     *                     Statistics are accessed via stats(label, COLUMN) where
     *                     COLUMN is one of #ConnectedComponentsTypes. The data type
     *                     is CV_32S.
     * @param centroids    centroid output for each label, including the background
     *                     label. Centroids are accessed via centroids(label, 0) for
     *                     x and centroids(label, 1) for y. The data type CV_64F.
     * @param connectivity 8 or 4 for 8-way or 4-way connectivity respectively
     * @param ltype        output image label type. Currently CV_32S and CV_16U are
     *                     supported.
     ...
     *                  label, see below for available statistics. Statistics are
     *                  accessed via stats(label, COLUMN) where COLUMN is one of
     *                  #ConnectedComponentsTypes. The data type is CV_32S.
     * @param centroids centroid output for each label, including the background
     *                  label. Centroids are accessed via centroids(label, 0) for x
     *                  and centroids(label, 1) for y. The data type CV_64F.
     * @return automatically generated
     */
    ...
    //
    // C++: int cv::floodFill(Mat& image, Mat& mask, Point seedPoint, Scalar newVal,
    // Rect* rect = 0, Scalar loDiff = Scalar(), Scalar upDiff = Scalar(), int flags
    // = 4)
    //
     ...
     *
     * <ul>
     * <li>in case of a grayscale image and floating range \(\texttt{src} (x',y')-
     * \texttt{loDiff} \leq \texttt{src} (x,y) \leq \texttt{src} (x',y')+
     * \texttt{upDiff}\)</li>
     * </ul>
     *
     *
     * <ul>
     * <li>in case of a grayscale image and fixed range \(\texttt{src} (
     * \texttt{seedPoint} .x, \texttt{seedPoint} .y)- \texttt{loDiff} \leq
     * \texttt{src} (x,y) \leq \texttt{src} ( \texttt{seedPoint} .x,
     * \texttt{seedPoint} .y)+ \texttt{upDiff}\)</li>
     * </ul>
     *
     *
     * <ul>
     * <li>in case of a color image and floating range \(\texttt{src} (x',y')_r-
     * \texttt{loDiff} _r \leq \texttt{src} (x,y)_r \leq \texttt{src} (x',y')_r+
     * \texttt{upDiff} _r,\) \(\texttt{src} (x',y')_g- \texttt{loDiff} _g \leq
     * \texttt{src} (x,y)_g \leq \texttt{src} (x',y')_g+ \texttt{upDiff} _g\) and
     * \(\texttt{src} (x',y')_b- \texttt{loDiff} _b \leq \texttt{src} (x,y)_b \leq
     * \texttt{src} (x',y')_b+ \texttt{upDiff} _b\)</li>
     * </ul>
     *
     *
     * <ul>
     * <li>in case of a color image and fixed range \(\texttt{src} (
     * \texttt{seedPoint} .x, \texttt{seedPoint} .y)_r- \texttt{loDiff} _r \leq
     * \texttt{src} (x,y)_r \leq \texttt{src} ( \texttt{seedPoint} .x,
     * \texttt{seedPoint} .y)_r+ \texttt{upDiff} _r,\) \(\texttt{src} (
     * \texttt{seedPoint} .x, \texttt{seedPoint} .y)_g- \texttt{loDiff} _g \leq
     * \texttt{src} (x,y)_g \leq \texttt{src} ( \texttt{seedPoint} .x,
     * \texttt{seedPoint} .y)_g+ \texttt{upDiff} _g\) and \(\texttt{src} (
     * \texttt{seedPoint} .x, \texttt{seedPoint} .y)_b- \texttt{loDiff} _b \leq
     * \texttt{src} (x,y)_b \leq \texttt{src} ( \texttt{seedPoint} .x,
     * \texttt{seedPoint} .y)_b+ \texttt{upDiff} _b\)</li>
     * </ul>
     * <p>
     * <p>
     ...
     *                  image. It is modified by the function unless the
     *                  #FLOODFILL_MASK_ONLY flag is set in the second variant of
     *                  the function. See the details below.
     * @param mask      Operation mask that should be a single-channel 8-bit image,
     *                  2 pixels wider and 2 pixels taller than image. Since this is
     *                  both an input and output parameter, you must take
     *                  responsibility of initializing it. Flood-filling cannot go
     *                  across non-zero pixels in the input mask. For example, an
     *                  edge detector output can be used as a mask to stop filling
     *                  at edges. On output, pixels in the mask corresponding to
     *                  filled pixels in the image are set to 1 or to the a value
     *                  specified in flags as described below. Additionally, the
     *                  function fills the border of the mask with ones to simplify
     *                  internal processing. It is therefore possible to use the
     *                  same mask in multiple calls to the function to make sure the
     *                  filled areas do not overlap.
     * @param seedPoint Starting point.
     * @param newVal    New value of the repainted domain pixels.
     * @param loDiff    Maximal lower brightness/color difference between the
     *                  currently observed pixel and one of its neighbors belonging
     *                  to the component, or a seed pixel being added to the
     *                  component.
     ...
     *                  value. The default value of 4 means that only the four
     *                  nearest neighbor pixels (those that share an edge) are
     *                  considered. A connectivity value of 8 means that the eight
     *                  nearest neighbor pixels (those that share a corner) will be
     *                  considered. The next 8 bits (8-16) contain a value between 1
     *                  and 255 with which to fill the mask (the default value is
     *                  1). For example, 4 | ( 255 &lt;&lt; 8 ) will consider 4
     *                  nearest neighbours and fill the mask with a value of 255.
     *                  The following additional options occupy higher bits and
     *                  therefore may be further combined with the connectivity and
     *                  mask fill values using bit-wise or (|), see #FloodFillFlags.
     *
     *                  <b>Note:</b> Since the mask is larger than the filled image,
     *                  a pixel \((x, y)\) in image corresponds to the pixel \((x+1,
     *                  y+1)\) in the mask .
     *                  <p>
     *                  SEE: findContours
     * @return automatically generated
     ...
     *                  component.
     * @param rect      Optional output parameter set by the function to the minimum
     *                  bounding rectangle of the repainted domain. 4 means that
     *                  only the four nearest neighbor pixels (those that share an
     *                  edge) are considered. A connectivity value of 8 means that
     *                  the eight nearest neighbor pixels (those that share a
     *                  corner) will be considered. The next 8 bits (8-16) contain a
     *                  value between 1 and 255 with which to fill the mask (the
     *                  default value is 1). For example, 4 | ( 255 &lt;&lt; 8 )
     *                  will consider 4 nearest neighbours and fill the mask with a
     *                  value of 255. The following additional options occupy higher
     *                  bits and therefore may be further combined with the
     *                  connectivity and mask fill values using bit-wise or (|), see
     *                  #FloodFillFlags.
     *
     *                  <b>Note:</b> Since the mask is larger than the filled image,
     *                  a pixel \((x, y)\) in image corresponds to the pixel \((x+1,
     ...
     *                  currently observed pixel and one of its neighbors belonging
     *                  to the component, or a seed pixel being added to the
     *                  component. one of its neighbors belonging to the component,
     ...
     *                  filled areas do not overlap.
     * @param seedPoint Starting point.
     * @param newVal    New value of the repainted domain pixels. one of its
     *                  neighbors belonging to the component, or a seed pixel being
     *                  added to the component. one of its neighbors belonging to
     ...
     *                  added to the component. one of its neighbors belonging to
     *                  the component, or a seed pixel being added to the component.
     *                  repainted domain. 4 means that only the four nearest
     *                  neighbor pixels (those that share an edge) are considered. A
     *                  connectivity value of 8 means that the eight nearest
     *                  neighbor pixels (those that share a corner) will be
     *                  considered. The next 8 bits (8-16) contain a value between 1
     *                  and 255 with which to fill the mask (the default value is
     *                  1). For example, 4 | ( 255 &lt;&lt; 8 ) will consider 4
     *                  nearest neighbours and fill the mask with a value of 255.
    ...
    //
    // C++: int cv::rotatedRectangleIntersection(RotatedRect rect1, RotatedRect
     ...
     *                           intersecting region. It returns at most 8 vertices.
     *                           Stored as std::vector&lt;cv::Point2f&gt; or cv::Mat
     *                           as Mx1 of type CV_32FC2.
    ...
    //
    // C++: void cv::Canny(Mat dx, Mat dy, Mat& edges, double threshold1, double
    // threshold2, bool L2gradient = false)
    //
     ...
     *                   \(=\sqrt{(dI/dx)^2 + (dI/dy)^2}\) should be used to
     *                   calculate the image gradient magnitude ( L2gradient=true ),
     *                   or whether the default \(L_1\) norm \(=|dI/dx|+|dI/dy|\) is
    ...
    //
    // C++: void cv::Canny(Mat image, Mat& edges, double threshold1, double
     ...
     * threshold2 is used for edge linking. The largest value is used to find
     * initial segments of strong edges. See
     * &lt;http://en.wikipedia.org/wiki/Canny_edge_detector&gt;
     *
     * @param image        8-bit input image.
     * @param edges        output edge map; single channels 8-bit image, which has
     ...
     *                     \(=\sqrt{(dI/dx)^2 + (dI/dy)^2}\) should be used to
     *                     calculate the image gradient magnitude ( L2gradient=true
     *                     ), or whether the default \(L_1\) norm
     *                     \(=|dI/dx|+|dI/dy|\) is enough ( L2gradient=false ).
     */
     ...
     * @param threshold1   first threshold for the hysteresis procedure.
     * @param threshold2   second threshold for the hysteresis procedure.
     * @param apertureSize aperture size for the Sobel operator. \(=\sqrt{(dI/dx)^2
     *                     + (dI/dy)^2}\) should be used to calculate the image
     *                     gradient magnitude ( L2gradient=true ), or whether the
    ...
    //
    // C++: void cv::GaussianBlur(Mat src, Mat& dst, Size ksize, double sigmaX,
     ...
     *                   which are processed independently, but the depth should be
     *                   CV_8U, CV_16U, CV_16S, CV_32F or CV_64F.
     * @param dst        output image of the same size and type as src.
     * @param ksize      Gaussian kernel size. ksize.width and ksize.height can
     *                   differ but they both must be positive and odd. Or, they can
     ...
     *                   sigmaY is zero, it is set to be equal to sigmaX, if both
     *                   sigmas are zeros, they are computed from ksize.width and
     *                   ksize.height, respectively (see #getGaussianKernel for
     *                   details); to fully control the result regardless of
     *                   possible future modifications of all this semantics, it is
     *                   recommended to specify all of ksize, sigmaX, and sigmaY.
     * @param borderType pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported.
     *                   <p>
     *                   SEE: sepFilter2D, filter2D, blur, boxFilter,
     ...
     *               <p>
     *               SEE: sepFilter2D, filter2D, blur, boxFilter, bilateralFilter,
     *               medianBlur
     */
     ...
     *               but they both must be positive and odd. Or, they can be zero's
     *               and then they are computed from sigma.
     * @param sigmaX Gaussian kernel standard deviation in X direction. equal to
     *               sigmaX, if both sigmas are zeros, they are computed from
     *               ksize.width and ksize.height, respectively (see
     *               #getGaussianKernel for details); to fully control the result
     *               regardless of possible future modifications of all this
     *               semantics, it is recommended to specify all of ksize, sigmaX,
     *               and sigmaY.
     *               <p>
     *               SEE: sepFilter2D, filter2D, blur, boxFilter, bilateralFilter,
     *               medianBlur
    ...
    //
    // C++: void cv::HoughCircles(Mat image, Mat& circles, int method, double dp,
     ...
     *
     * <b>Note:</b> Usually the function detects the centers of circles well.
     * However, it may fail to find correct radii. You can assist to the function by
     * specifying the radius range ( minRadius and maxRadius ) if you know it. Or,
     * in the case of #HOUGH_GRADIENT method you may set maxRadius to a negative
     * number to return centers only without radius search, and find the correct
     * radius using an additional procedure.
     * <p>
     * It also helps to smooth image a bit unless it's already soft. For example,
     * GaussianBlur() with 7x7 kernel and 1.5x1.5 sigma or similar blurring may
     * help.
     *
     * @param image     8-bit, single-channel, grayscale input image.
     * @param circles   Output vector of found circles. Each vector is encoded as 3
     *                  or 4 element floating-point vector \((x, y, radius)\) or
     *                  \((x, y, radius, votes)\) .
     ...
     *                  same resolution as the input image. If dp=2 , the
     *                  accumulator has half as big width and height. For
     *                  #HOUGH_GRADIENT_ALT the recommended value is dp=1.5, unless
     *                  some small very circles need to be detected.
     ...
     *                  and #HOUGH_GRADIENT_ALT, it is the higher threshold of the
     *                  two passed to the Canny edge detector (the lower one is
     *                  twice smaller). Note that #HOUGH_GRADIENT_ALT uses #Scharr
     *                  algorithm to compute image derivatives, so the threshold
     *                  value shough normally be higher, such as 300 or normally
     *                  exposed and contrasty images.
     * @param param2    Second method-specific parameter. In case of
     *                  #HOUGH_GRADIENT, it is the accumulator threshold for the
     *                  circle centers at the detection stage. The smaller it is,
     *                  the more false circles may be detected. Circles,
     *                  corresponding to the larger accumulator values, will be
     *                  returned first. In the case of #HOUGH_GRADIENT_ALT
     *                  algorithm, this is the circle "perfectness" measure. The
     *                  closer it to 1, the better shaped circles algorithm selects.
     *                  In most cases 0.9 should be fine. If you want get better
     *                  detection of small circles, you may decrease it to 0.85, 0.8
     *                  or even less. But then also try to limit the search range
     *                  [minRadius, maxRadius] to avoid many false circles.
     * @param minRadius Minimum circle radius.
     * @param maxRadius Maximum circle radius. If &lt;= 0, uses the maximum image
     *                  dimension. If &lt; 0, #HOUGH_GRADIENT returns centers
     *                  without finding the radius. #HOUGH_GRADIENT_ALT always
     ...
     *                  or even less. But then also try to limit the search range
     *                  [minRadius, maxRadius] to avoid many false circles.
     * @param minRadius Minimum circle radius. centers without finding the radius.
     ...
     *
     * @param image   8-bit, single-channel, grayscale input image.
     * @param circles Output vector of found circles. Each vector is encoded as 3 or
     *                4 element floating-point vector \((x, y, radius)\) or \((x, y,
     *                radius, votes)\) .
     * @param method  Detection method, see #HoughModes. The available methods are
     *                #HOUGH_GRADIENT and #HOUGH_GRADIENT_ALT.
     * @param dp      Inverse ratio of the accumulator resolution to the image
     ...
     *                has half as big width and height. For #HOUGH_GRADIENT_ALT the
     ...
     * @param param1  First method-specific parameter. In case of #HOUGH_GRADIENT
     *                and #HOUGH_GRADIENT_ALT, it is the higher threshold of the two
     ...
     *                contrasty images.
     * @param param2  Second method-specific parameter. In case of #HOUGH_GRADIENT,
     ...
     *                #HOUGH_GRADIENT_ALT algorithm, this is the circle
     *                "perfectness" measure. The closer it to 1, the better shaped
     *                circles algorithm selects. In most cases 0.9 should be fine.
     *                If you want get better detection of small circles, you may
     *                decrease it to 0.85, 0.8 or even less. But then also try to
     *                limit the search range [minRadius, maxRadius] to avoid many
     *                false circles. centers without finding the radius.
     *                #HOUGH_GRADIENT_ALT always computes circle radiuses.
     *                <p>
     *                SEE: fitEllipse, minEnclosingCircle
     */
     ...
     *                to compute image derivatives, so the threshold value shough
     *                normally be higher, such as 300 or normally exposed and
     *                contrasty images. accumulator threshold for the circle centers
     *                at the detection stage. The smaller it is, the more false
     *                circles may be detected. Circles, corresponding to the larger
     *                accumulator values, will be returned first. In the case of
     *                #HOUGH_GRADIENT_ALT algorithm, this is the circle
     *                "perfectness" measure. The closer it to 1, the better shaped
     *                circles algorithm selects. In most cases 0.9 should be fine.
     *                If you want get better detection of small circles, you may
     ...
     *                If the parameter is too small, multiple neighbor circles may
     *                be falsely detected in addition to a true one. If it is too
     *                large, some circles may be missed. it is the higher threshold
     *                of the two passed to the Canny edge detector (the lower one is
     *                twice smaller). Note that #HOUGH_GRADIENT_ALT uses #Scharr
     *                algorithm to compute image derivatives, so the threshold value
     *                shough normally be higher, such as 300 or normally exposed and
     *                contrasty images. accumulator threshold for the circle centers
     *                at the detection stage. The smaller it is, the more false
     *                circles may be detected. Circles, corresponding to the larger
     *                accumulator values, will be returned first. In the case of
    ...
    //
    // C++: void cv::HoughLines(Mat image, Mat& lines, double rho, double theta, int
     ...
     * &lt;http://homepages.inf.ed.ac.uk/rbf/HIPR2/hough.htm&gt; for a good
     ...
     *                  element vector \((\rho, \theta)\) or \((\rho, \theta,
     *                  \textrm{votes})\) . \(\rho\) is the distance from the
     *                  coordinate origin \((0,0)\) (top-left corner of the image).
     *                  \(\theta\) is the line rotation angle in radians ( \(0 \sim
     *                  \textrm{vertical line}, \pi/2 \sim \textrm{horizontal
     ...
     *                  returned that get enough votes ( \(&gt;\texttt{threshold}\)
     *                  ).
     ...
     *                  rho/srn . If both srn=0 and stn=0 , the classical Hough
     *                  transform is used. Otherwise, both these parameters should
     *                  be positive.
     ...
     *                  distance resolution theta.
     * @param min_theta For standard and multi-scale Hough transform, minimum angle
     *                  to check for lines. Must fall between 0 and max_theta. Must
     *                  fall between min_theta and CV_PI.
     */
    public static void HoughLines(Mat image, Mat lines, double rho, double theta, int threshold, double srn, double stn,
            double min_theta) {
     ...
     *                  be positive.
     * @param stn       For the multi-scale Hough transform, it is a divisor for the
     *                  distance resolution theta. Must fall between 0 and
     ...
     *                  rho/srn . If both srn=0 and stn=0 , the classical Hough
     *                  transform is used. Otherwise, both these parameters should
     *                  be positive. Must fall between 0 and max_theta. Must fall
     ...
     * @param threshold Accumulator threshold parameter. Only those lines are
     *                  returned that get enough votes ( \(&gt;\texttt{threshold}\)
     *                  ). The coarse accumulator distance resolution is rho and the
     *                  accurate accumulator resolution is rho/srn . If both srn=0
     *                  and stn=0 , the classical Hough transform is used.
     *                  Otherwise, both these parameters should be positive. Must
     *                  fall between 0 and max_theta. Must fall between min_theta
     *                  and CV_PI.
     */
    ...
    //
    // C++: void cv::HoughLinesP(Mat image, Mat& lines, double rho, double theta,
     ...
     * <p>
     * See the line detection example below: INCLUDE:
     * snippets/imgproc_HoughLinesP.cpp This is a sample picture the function
     * parameters have been tuned for:
     * <p>
     ...
     *
     * @param image         8-bit, single-channel binary source image. The image may
     *                      be modified by the function.
     * @param lines         Output vector of lines. Each line is represented by a
     *                      4-element vector \((x_1, y_1, x_2, y_2)\) , where
     *                      \((x_1,y_1)\) and \((x_2, y_2)\) are the ending points
     ...
     * @param lines     Output vector of lines. Each line is represented by a
     *                  4-element vector \((x_1, y_1, x_2, y_2)\) , where
     *                  \((x_1,y_1)\) and \((x_2, y_2)\) are the ending points of
    ...
    //
    // C++: void cv::HoughLinesPointSet(Mat _point, Mat& _lines, int lines_max, int
    // threshold, double min_rho, double max_rho, double rho_step, double min_theta,
     ...
     * transform. INCLUDE: snippets/imgproc_HoughLinesPointSet.cpp
     *
     * @param _point     Input vector of points. Each vector must be encoded as a
     *                   Point vector \((x,y)\). Type must be CV_32FC2 or CV_32SC2.
     * @param _lines     Output vector of found lines. Each vector is encoded as a
     *                   vector&lt;Vec3d&gt; \((votes, rho, theta)\). The larger the
     *                   value of 'votes', the higher the reliability of the Hough
     *                   line.
     ...
     */
    public static void HoughLinesPointSet(Mat _point, Mat _lines, int lines_max, int threshold, double min_rho,
            double max_rho, double rho_step, double min_theta, double max_theta, double theta_step) {
        HoughLinesPointSet_0(_point.nativeObj, _lines.nativeObj, lines_max, threshold, min_rho, max_rho, rho_step,
    ...
    //
    // C++: void cv::Laplacian(Mat src, Mat& dst, int ddepth, int ksize = 1, double
     ...
     * <p>
     * \(\texttt{dst} = \Delta \texttt{src} = \frac{\partial^2
     * \texttt{src}}{\partial x^2} + \frac{\partial^2 \texttt{src}}{\partial y^2}\)
     * <p>
     * This is done when {@code ksize &gt; 1}. When {@code ksize == 1}, the
     * Laplacian is computed by filtering the image with the following \(3 \times
     * 3\) aperture:
     * <p>
     ...
     * @param ksize      Aperture size used to compute the second-derivative
     *                   filters. See #getDerivKernels for details. The size must be
     *                   positive and odd.
     * @param scale      Optional scale factor for the computed Laplacian values. By
     *                   default, no scaling is applied. See #getDerivKernels for
     *                   details.
     * @param delta      Optional delta value that is added to the results prior to
     *                   storing them in dst .
     * @param borderType Pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported. SEE: Sobel, Scharr
     */
     ...
     *               See #getDerivKernels for details. The size must be positive and
     *               odd.
     * @param scale  Optional scale factor for the computed Laplacian values. By
     *               default, no scaling is applied. See #getDerivKernels for
     *               details.
     * @param delta  Optional delta value that is added to the results prior to
     *               storing them in dst . SEE: Sobel, Scharr
     */
     ...
     * @param ksize  Aperture size used to compute the second-derivative filters.
     *               See #getDerivKernels for details. The size must be positive and
     *               odd. applied. See #getDerivKernels for details. SEE: Sobel,
     *               Scharr
     */
     ...
     * @param dst    Destination image of the same size and the same number of
     *               channels as src .
     * @param ddepth Desired depth of the destination image. details. The size must
     *               be positive and odd. applied. See #getDerivKernels for details.
     *               SEE: Sobel, Scharr
     */
    ...
    //
    // C++: void cv::Scharr(Mat src, Mat& dst, int ddepth, int dx, int dy, double
    // scale = 1, double delta = 0, int borderType = BORDER_DEFAULT)
    //
     ...
     * is equivalent to
     * <p>
     * \(\texttt{Sobel(src, dst, ddepth, dx, dy, FILTER_SCHARR, scale, delta,
     * borderType)} .\)
     *
     * @param src        input image.
     * @param dst        output image of the same size and the same number of
     *                   channels as src.
     * @param ddepth     output image depth, see REF: filter_depths "combinations"
     * @param dx         order of the derivative x.
     * @param dy         order of the derivative y.
     * @param scale      optional scale factor for the computed derivative values;
     *                   by default, no scaling is applied (see #getDerivKernels for
     *                   details).
     * @param delta      optional delta value that is added to the results prior to
     *                   storing them in dst.
     * @param borderType pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported. SEE: cartToPolar
     */
     ...
     *               details).
     * @param delta  optional delta value that is added to the results prior to
     *               storing them in dst. SEE: cartToPolar
     */
    public static void Scharr(Mat src, Mat dst, int ddepth, int dx, int dy, double scale, double delta) {
     ...
     * @param ddepth output image depth, see REF: filter_depths "combinations"
     * @param dx     order of the derivative x.
     * @param dy     order of the derivative y. applied (see #getDerivKernels for
     *               details). SEE: cartToPolar
     */
    ...
    //
    // C++: void cv::Sobel(Mat src, Mat& dst, int ddepth, int dx, int dy, int ksize
     ...
     * separable kernel is used to calculate the derivative. When \(\texttt{ksize =
     * 1}\), the \(3 \times 1\) or \(1 \times 3\) kernel is used (that is, no
     * Gaussian smoothing is done). {@code ksize = 1} can only be used for the first
     * or the second x- or y- derivatives.
     * <p>
     * There is also the special value {@code ksize = #FILTER_SCHARR (-1)} that
     ...
     * appropriate kernel:
     * <p>
     * \(\texttt{dst} = \frac{\partial^{xorder+yorder} \texttt{src}}{\partial
     ...
     *                   channels as src .
     * @param ddepth     output image depth, see REF: filter_depths "combinations";
     *                   in the case of 8-bit input images it will result in
     ...
     *                   storing them in dst.
     * @param borderType pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported. SEE: Scharr, Laplacian, sepFilter2D,
     ...
     * @param dst    output image of the same size and the same number of channels
     *               as src .
     * @param ddepth output image depth, see REF: filter_depths "combinations"; in
     ...
     *               details).
     * @param delta  optional delta value that is added to the results prior to
     *               storing them in dst. SEE: Scharr, Laplacian, sepFilter2D,
     *               filter2D, GaussianBlur, cartToPolar
     */
    public static void Sobel(Mat src, Mat dst, int ddepth, int dx, int dy, int ksize, double scale, double delta) {
     ...
     * @param scale  optional scale factor for the computed derivative values; by
     *               default, no scaling is applied (see #getDerivKernels for
     *               details). SEE: Scharr, Laplacian, sepFilter2D, filter2D,
     *               GaussianBlur, cartToPolar
     */
     ...
     * @param dy     order of the derivative y.
     * @param ksize  size of the extended Sobel kernel; it must be 1, 3, 5, or 7.
     *               applied (see #getDerivKernels for details). SEE: Scharr,
     ...
     * <p>
     * \(\texttt{dst} (x,y) \leftarrow \texttt{dst} (x,y) + \texttt{src} (x,y) \quad
    ...
    //
    // C++: void cv::accumulateProduct(Mat src1, Mat src2, Mat& dst, Mat mask =
    // Mat())
    //
     ...
     * accumulator dst :
     * <p>
     * \(\texttt{dst} (x,y) \leftarrow \texttt{dst} (x,y) + \texttt{src1} (x,y)
    ...
    //
    // C++: void cv::accumulateWeighted(Mat src, Mat& dst, double alpha, Mat mask =
    // Mat())
    //
     ...
     * <p>
     * \(\texttt{dst} (x,y) \leftarrow (1- \texttt{alpha} ) \cdot \texttt{dst} (x,y)
     ...
     * about earlier images). The function supports multi-channel images. Each
    ...
    //
    // C++: void cv::adaptiveThreshold(Mat src, Mat& dst, double maxValue, int
     ...
     * &gt; T(x,y)\)}{0}{otherwise}\)</li>
     * <li><b>THRESH_BINARY_INV</b> \(dst(x,y) = \fork{0}{if \(src(x,y) &gt;
     * T(x,y)\)}{\texttt{maxValue}}{otherwise}\) where \(T(x,y)\) is a threshold
     * calculated individually for each pixel (see adaptiveMethod parameter).</li>
     * </ul>
     * <p>
     ...
     * @param C              Constant subtracted from the mean or weighted mean (see
     *                       the details below). Normally, it is positive but may be
     ...
     *                  CV_8UC3.
     * @param dst       The result is the colormapped source image. Note:
     *                  Mat::create is called on dst.
     * @param userColor The colormap to apply of type CV_8UC1 or CV_8UC3 and size
     *                  256
     */
    ...
    //
    // C++: void cv::approxPolyDP(vector_Point2f curve, vector_Point2f& approxCurve,
    ...
    //
    // C++: void cv::arrowedLine(Mat& img, Point pt1, Point pt2, Scalar color, int
    ...
    //
    // C++: void cv::bilateralFilter(Mat src, Mat& dst, int d, double sigmaColor,
     ...
     * <p>
     * _Filter size_: Large filters (d &gt; 5) are very slow, so it is recommended
     * to use d=5 for real-time applications, and perhaps d=9 for offline
     * applications that need heavy noise filtering.
     * <p>
     * This filter does not work inplace.
     *
     * @param src        Source 8-bit or floating-point, 1-channel or 3-channel
     *                   image.
     ...
     *                   parameter means that farther pixels will influence each
     *                   other as long as their colors are close enough (see
     *                   sigmaColor ). When d&gt;0, it specifies the neighborhood
     *                   size regardless of sigmaSpace. Otherwise, d is proportional
     *                   to sigmaSpace.
    ...
    //
    // C++: void cv::blur(Mat src, Mat& dst, Size ksize, Point anchor =
     ...
     * <p>
     * \(\texttt{K} = \frac{1}{\texttt{ksize.width*ksize.height}} \begin{bmatrix} 1
     * &amp; 1 &amp; 1 &amp; \cdots &amp; 1 &amp; 1 \\ 1 &amp; 1 &amp; 1 &amp;
     * \cdots &amp; 1 &amp; 1 \\ \hdotsfor{6} \\ 1 &amp; 1 &amp; 1 &amp; \cdots
     * &amp; 1 &amp; 1 \\ \end{bmatrix}\)
     * <p>
     * The call {@code blur(src, dst, ksize, anchor, borderType)} is equivalent to
     ...
     * @param borderType border mode used to extrapolate pixels outside of the
     *                   image, see #BorderTypes. #BORDER_WRAP is not supported.
     ...
     * @param dst    output image of the same size and type as src.
     * @param ksize  blurring kernel size.
     * @param anchor anchor point; default value Point(-1,-1) means that the anchor
     *               is at the kernel center. SEE: boxFilter, bilateralFilter,
     ...
     *              CV_16S, CV_32F or CV_64F.
     * @param dst   output image of the same size and type as src.
     * @param ksize blurring kernel size. center. SEE: boxFilter, bilateralFilter,
     *              GaussianBlur, medianBlur
     */
    ...
    //
    // C++: void cv::boxFilter(Mat src, Mat& dst, int ddepth, Size ksize, Point
     ...
     * The function smooths an image using the kernel:
     * <p>
     * \(\texttt{K} = \alpha \begin{bmatrix} 1 &amp; 1 &amp; 1 &amp; \cdots &amp; 1
     * &amp; 1 \\ 1 &amp; 1 &amp; 1 &amp; \cdots &amp; 1 &amp; 1 \\ \hdotsfor{6} \\
     ...
     * @param borderType border mode used to extrapolate pixels outside of the
     *                   image, see #BorderTypes. #BORDER_WRAP is not supported.
     *                   SEE: blur, bilateralFilter, GaussianBlur, medianBlur,
     *                   integral
     */
     ...
     *                  anchor is at the kernel center.
     * @param normalize flag, specifying whether the kernel is normalized by its
     *                  area or not. SEE: blur, bilateralFilter, GaussianBlur,
     *                  medianBlur, integral
     */
    public static void boxFilter(Mat src, Mat dst, int ddepth, Size ksize, Point anchor, boolean normalize) {
     ...
     * @param ksize  blurring kernel size.
     * @param anchor anchor point; default value Point(-1,-1) means that the anchor
     *               is at the kernel center. SEE: blur, bilateralFilter,
     *               GaussianBlur, medianBlur, integral
     */
     ...
     * @param dst    output image of the same size and type as src.
     * @param ddepth the output image depth (-1 to use src.depth()).
     * @param ksize  blurring kernel size. center. SEE: blur, bilateralFilter,
     *               GaussianBlur, medianBlur, integral
     */
    ...
    //
    // C++: void cv::calcBackProject(vector_Mat images, vector_int channels, Mat
    ...
    //
    // C++: void cv::calcHist(vector_Mat images, vector_int channels, Mat mask, Mat&
    ...
    //
    // C++: void cv::circle(Mat& img, Point center, int radius, Scalar color, int
     ...
     *                  values, like #FILLED, mean that a filled circle is to be
     *                  drawn.
    ...
    //
    // C++: void cv::convertMaps(Mat map1, Mat map2, Mat& dstmap1, Mat& dstmap2, int
     ...
     *
     * <ul>
     * <li>\(\texttt{(CV_32FC1, CV_32FC1)} \rightarrow \texttt{(CV_16SC2,
     * CV_16UC1)}\). This is the most frequently used conversion operation, in which
     * the original floating-point maps (see remap ) are converted to a more compact
     * and much faster fixed-point representation. The first output array contains
     * the rounded coordinates and the second array (created only when
     * nninterpolation=false ) contains indices in the interpolation tables.</li>
     * </ul>
     *
     * <ul>
     * <li>\(\texttt{(CV_32FC2)} \rightarrow \texttt{(CV_16SC2, CV_16UC1)}\). The
     * same as above but the original maps are stored in one 2-channel matrix.</li>
     * </ul>
     *
     * <ul>
     * <li>Reverse conversion. Obviously, the reconstructed floating-point maps will
     * not be exactly the same as the originals.</li>
     * </ul>
     *
     * @param map1            The first input map of type CV_16SC2, CV_32FC1, or
     *                        CV_32FC2 .
     * @param map2            The second input map of type CV_16UC1, CV_32FC1, or
     *                        none (empty matrix), respectively.
     * @param dstmap1         The first output map that has the type dstmap1type and
     ...
     *                        for the nearest-neighbor or for a more complex
     *                        interpolation.
     *                        <p>
     ...
     * @param dstmap2     The second output map.
     * @param dstmap1type Type of the first output map that should be CV_16SC2,
     *                    CV_32FC1, or CV_32FC2 . nearest-neighbor or for a more
    ...
    //
    // C++: void cv::convexHull(vector_Point points, vector_int& hull, bool
     ...
     *
     * @param points    Input 2D point set, stored in std::vector or Mat.
     * @param hull      Output convex hull. It is either an integer vector of
     *                  indices or vector of points. In the first case, the hull
     *                  elements are 0-based indices of the convex hull points in
     *                  the original array (since the set of convex hull points is a
     *                  subset of the original point set). In the second case, hull
     *                  elements are the convex hull points themselves.
     * @param clockwise Orientation flag. If it is true, the output convex hull is
     *                  oriented clockwise. Otherwise, it is oriented
     *                  counter-clockwise. The assumed coordinate system has its X
     *                  axis pointing to the right, and its Y axis pointing upwards.
     *                  returns convex hull points. Otherwise, it returns indices of
     *                  the convex hull points. When the output array is
     *                  std::vector, the flag is ignored, and the output depends on
     *                  the type of the vector: std::vector&lt;int&gt; implies
     *                  returnPoints=false, std::vector&lt;Point&gt; implies
     *                  returnPoints=true.
     *
     *                  <b>Note:</b> {@code points} and {@code hull} should be
     *                  different arrays, inplace processing isn't supported.
     *                  <p>
     *                  Check REF: tutorial_hull "the corresponding tutorial" for
     *                  more details.
     *                  <p>
     *                  useful links:
     *                  <p>
     ...
     *               or vector of points. In the first case, the hull elements are
     *               0-based indices of the convex hull points in the original array
     *               (since the set of convex hull points is a subset of the
     *               original point set). In the second case, hull elements are the
     *               convex hull points themselves. Otherwise, it is oriented
     *               counter-clockwise. The assumed coordinate system has its X axis
     *               pointing to the right, and its Y axis pointing upwards. returns
     *               convex hull points. Otherwise, it returns indices of the convex
     *               hull points. When the output array is std::vector, the flag is
     *               ignored, and the output depends on the type of the vector:
     *               std::vector&lt;int&gt; implies returnPoints=false,
     *               std::vector&lt;Point&gt; implies returnPoints=true.
     *
     *               <b>Note:</b> {@code points} and {@code hull} should be
     *               different arrays, inplace processing isn't supported.
     *               <p>
     *               Check REF: tutorial_hull "the corresponding tutorial" for more
     *               details.
     *               <p>
     *               useful links:
     *               <p>
    ...
    //
    // C++: void cv::convexityDefects(vector_Point contour, vector_int convexhull,
     ...
     *                         contain indices of the contour points that make the
     *                         hull.
     * @param convexityDefects The output vector of convexity defects. In C++ and
     *                         the new Python/Java interface each convexity defect
     *                         is represented as 4-element integer vector (a.k.a.
     *                         #Vec4i): (start_index, end_index, farthest_pt_index,
     *                         fixpt_depth), where indices are 0-based indices in
     *                         the original contour of the convexity defect
     *                         beginning, end and the farthest point, and
     *                         fixpt_depth is fixed-point approximation (with 8
     *                         fractional bits) of the distance between the farthest
     *                         contour point and the hull. That is, to get the
     *                         floating-point value of the depth will be
     *                         fixpt_depth/256.0.
     */
    ...
    //
    // C++: void cv::cornerEigenValsAndVecs(Mat src, Mat& dst, int blockSize, int
     ...
     * blockSize \(\times\) blockSize neighborhood \(S(p)\) . It calculates the
     * covariation matrix of derivatives over the neighborhood as:
     * <p>
     * \(M = \begin{bmatrix} \sum _{S(p)}(dI/dx)^2 &amp; \sum _{S(p)}dI/dx dI/dy \\
     * \sum _{S(p)}dI/dx dI/dy &amp; \sum _{S(p)}(dI/dy)^2 \end{bmatrix}\)
     * <p>
    ...
    //
    // C++: void cv::cornerHarris(Mat src, Mat& dst, int blockSize, int ksize,
     ...
     * calculates a \(2\times2\) gradient covariance matrix \(M^{(x,y)}\) over a
     * \(\texttt{blockSize} \times \texttt{blockSize}\) neighborhood. Then, it
     * computes the following characteristic:
     * <p>
     * \(\texttt{dst} (x,y) = \mathrm{det} M^{(x,y)} - k \cdot \left ( \mathrm{tr}
     * M^{(x,y)} \right )^2\)
     * <p>
     * Corners in the image can be found as the local maxima of this response map.
     *
     * @param src        Input single-channel 8-bit or floating-point image.
     * @param dst        Image to store the Harris detector responses. It has the
    ...
    //
    // C++: void cv::cornerMinEigenVal(Mat src, Mat& dst, int blockSize, int ksize =
    ...
    //
    // C++: void cv::cornerSubPix(Mat image, Mat& corners, Size winSize, Size
     ...
     * vector from the center \(q\) to a point \(p\) located within a neighborhood
     * of \(q\) is orthogonal to the image gradient at \(p\) subject to image and
     * measurement noise. Consider the expression:
     * <p>
     * \(\epsilon _i = {DI_{p_i}}^T \cdot (q - p_i)\)
     * <p>
     * where \({DI_{p_i}}\) is an image gradient at one of the points \(p_i\) in a
     * neighborhood of \(q\) . The value of \(q\) is to be found so that
     * \(\epsilon_i\) is minimized. A system of equations may be set up with
     * \(\epsilon_i\) set to zero:
     * <p>
     * \(\sum _i(DI_{p_i} \cdot {DI_{p_i}}^T) \cdot q - \sum _i(DI_{p_i} \cdot
     * {DI_{p_i}}^T \cdot p_i)\)
     * <p>
     * where the gradients are summed within a neighborhood ("search window") of
     * \(q\) . Calling the first gradient term \(G\) and the second gradient term
     * \(b\) gives:
     * <p>
     * \(q = G^{-1} \cdot b\)
     * <p>
     ...
     *                 winSize=Size(5,5) , then a \((5*2+1) \times (5*2+1) = 11
     *                 \times 11\) search window is used.
     * @param zeroZone Half of the size of the dead region in the middle of the
     *                 search zone over which the summation in the formula below is
     *                 not done. It is used sometimes to avoid possible
     *                 singularities of the autocorrelation matrix. The value of
     *                 (-1,-1) indicates that there is no such a size.
     * @param criteria Criteria for termination of the iterative process of corner
     *                 refinement. That is, the process of corner position
     *                 refinement stops either after criteria.maxCount iterations or
     *                 when the corner position moves by less than criteria.epsilon
     *                 on some iteration.
     */
     ...
     * \(\rightarrow\) L\*u\*v\* transformation. For example, if you have a 32-bit
     ...
     * </code> If you use #cvtColor with 8-bit images, the conversion will have some
     ...
     *             single-precision floating-point.
     * @param dst  output image of the same size and depth as src.
     * @param code color space conversion code (see #ColorConversionCodes). channels
     ...
     *              </ul>
     *              <p>
     *              #COLOR_BayerBG2BGR_VNG , #COLOR_BayerGB2BGR_VNG ,
     ...
     *              </ul>
     *              <p>
     *              #COLOR_BayerBG2BGR_EA , #COLOR_BayerGB2BGR_EA ,
     *              #COLOR_BayerRG2BGR_EA , #COLOR_BayerGR2BGR_EA
     *
     *              <ul>
     ...
     * @param src  input image: 8-bit unsigned or 16-bit unsigned.
     * @param dst  output image of the same size and depth as src.
     * @param code Color space conversion code (see the description below). channels
     *             is derived automatically from src and code.
     *             <p>
     *             :
     *
    ...
    //
    // C++: void cv::dilate(Mat src, Mat& dst, Mat kernel, Point anchor =
    // Point(-1,-1), int iterations = 1, int borderType = BORDER_CONSTANT, Scalar
     ...
     * taken: \(\texttt{dst} (x,y) = \max _{(x',y'): \, \texttt{element} (x',y')
     ...
     *
     * @param src         input image; the number of channels can be arbitrary, but
     *                    the depth should be one of CV_8U, CV_16U, CV_16S, CV_32F
     *                    or CV_64F.
     * @param dst         output image of the same size and type as src.
     * @param kernel      structuring element used for dilation; if elemenat=Mat(),
     *                    a 3 x 3 rectangular structuring element is used. Kernel
     ...
     * @param borderType  pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                    is not suported.
     * @param borderValue border value in case of a constant border SEE: erode,
     ...
     *
     * @param src        input image; the number of channels can be arbitrary, but
     *                   the depth should be one of CV_8U, CV_16U, CV_16S, CV_32F or
     *                   CV_64F.
     * @param dst        output image of the same size and type as src.
     * @param kernel     structuring element used for dilation; if elemenat=Mat(), a
     *                   3 x 3 rectangular structuring element is used. Kernel can
     *                   be created using #getStructuringElement
     * @param anchor     position of the anchor within the element; default value
     *                   (-1, -1) means that the anchor is at the element center.
     * @param iterations number of times dilation is applied.
     * @param borderType pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not suported. SEE: erode, morphologyEx,
     ...
     * @param anchor     position of the anchor within the element; default value
     *                   (-1, -1) means that the anchor is at the element center.
     * @param iterations number of times dilation is applied. SEE: erode,
     *                   morphologyEx, getStructuringElement
     */
     ...
     * @param dst    output image of the same size and type as src.
     * @param kernel structuring element used for dilation; if elemenat=Mat(), a 3 x
     *               3 rectangular structuring element is used. Kernel can be
     *               created using #getStructuringElement
     * @param anchor position of the anchor within the element; default value (-1,
     *               -1) means that the anchor is at the element center. SEE: erode,
     *               morphologyEx, getStructuringElement
     */
     ...
     * @param kernel structuring element used for dilation; if elemenat=Mat(), a 3 x
     *               3 rectangular structuring element is used. Kernel can be
     *               created using #getStructuringElement anchor is at the element
     *               center. SEE: erode, morphologyEx, getStructuringElement
     */
    ...
    //
    // C++: void cv::distanceTransform(Mat src, Mat& dst, Mat& labels, int
     ...
     * function runs the algorithm described in CITE: Felzenszwalb04 . This
     * algorithm is parallelized with the TBB library.
     * <p>
     * In other cases, the algorithm CITE: Borgefors86 is used. This means that for
     * a pixel the function finds the shortest path to the nearest zero pixel
     * consisting of basic shifts: horizontal, vertical, diagonal, or knight's move
     * (the latest is available for a \(5\times 5\) mask). The overall distance is
     ...
     * same cost (denoted as a ), all the diagonal shifts must have the same cost
     * (denoted as {@code b}), and all knight's moves must have the same cost
     * (denoted as {@code c}). For the #DIST_C and #DIST_L1 types, the distance is
     * calculated precisely, whereas for #DIST_L2 (Euclidean distance) the distance
     * can be calculated only with a relative error (a \(5\times 5\) mask gives more
     * accurate results). For {@code a},{@code b}, and {@code c}, OpenCV uses the
     ...
     * mask is used. For a more accurate distance estimation #DIST_L2, a \(5\times
     * 5\) mask or the precise algorithm is used. Note that both the precise and the
     * approximate algorithms are linear on the number of pixels.
     * <p>
     * This variant of the function does not only compute the minimum distance for
     * each pixel \((x, y)\) but also identifies the nearest connected component
     * consisting of zero pixels (labelType==#DIST_LABEL_CCOMP) or the nearest zero
     * pixel (labelType==#DIST_LABEL_PIXEL). Index of the component/pixel is stored
     * in {@code labels(x, y)}. When labelType==#DIST_LABEL_CCOMP, the function
     * automatically finds connected components of zero pixels in the input image
     * and marks them with distinct labels. When labelType==#DIST_LABEL_CCOMP, the
     ...
     * i.e. maskSize=#DIST_MASK_PRECISE is not supported yet.
     *
     ...
     *                     It has the type CV_32SC1 and the same size as src.
     * @param distanceType Type of distance, see #DistanceTypes
     * @param maskSize     Size of the distance transform mask, see
     *                     #DistanceTransformMasks. #DIST_MASK_PRECISE is not
     *                     supported by this variant. In case of the #DIST_L1 or
     *                     #DIST_C distance type, the parameter is forced to 3
     *                     because a \(3\times 3\) mask gives the same result as
     *                     \(5\times 5\) or any larger aperture.
    ...
    //
    // C++: void cv::distanceTransform(Mat src, Mat& dst, int distanceType, int
     ...
     * @param distanceType Type of distance, see #DistanceTypes
     * @param maskSize     Size of the distance transform mask, see
     *                     #DistanceTransformMasks. In case of the #DIST_L1 or
     *                     #DIST_C distance type, the parameter is forced to 3
     *                     because a \(3\times 3\) mask gives the same result as
     *                     \(5\times 5\) or any larger aperture.
     * @param dstType      Type of output image. It can be CV_8U or CV_32F. Type
     *                     CV_8U can be used only for the first variant of the
     *                     function and distanceType == #DIST_L1.
     */
     ...
     *                     #DIST_C distance type, the parameter is forced to 3
     *                     because a \(3\times 3\) mask gives the same result as
     *                     \(5\times 5\) or any larger aperture. the first variant
     *                     of the function and distanceType == #DIST_L1.
     */
    ...
    //
    // C++: void cv::drawContours(Mat& image, vector_vector_Point contours, int
     ...
     * <p>
     * The function draws contour outlines in the image if \(\texttt{thickness} \ge
     * 0\) or fills the area bounded by the contours if \(\texttt{thickness}&lt;0\)
     * . The example below shows how to retrieve connected components from the
     ...
     *                   negative (for example, thickness=#FILLED ), the contour
     *                   interiors are drawn.
     ...
     *                   specified contour is drawn. If it is 1, the function draws
     *                   the contour(s) and all the nested contours. If it is 2, the
     *                   function draws the contours, all the nested contours, all
     ...
     *                   <b>Note:</b> When thickness=#FILLED, the function is
     *                   designed to handle connected components with holes
     *                   correctly even when no hierarchy date is provided. This is
     *                   done by analyzing all the outlines together using even-odd
     *                   rule. This may give incorrect results if you have a joint
     *                   collection of separately retrieved contours. In order to
     *                   solve this problem, you need to call #drawContours
     *                   separately for each sub-group of contours, or iterate over
     ...
     *                   the nested-to-nested contours, and so on. This parameter is
     *                   only taken into account when there is hierarchy available.
     *                   \(\texttt{offset}=(dx,dy)\) . <b>Note:</b> When
     *                   thickness=#FILLED, the function is designed to handle
     *                   connected components with holes correctly even when no
     *                   hierarchy date is provided. This is done by analyzing all
     *                   the outlines together using even-odd rule. This may give
     *                   incorrect results if you have a joint collection of
     *                   separately retrieved contours. In order to solve this
     *                   problem, you need to call #drawContours separately for each
     *                   sub-group of contours, or iterate over the collection using
     ...
     *                   nested contours. If it is 2, the function draws the
     *                   contours, all the nested contours, all the nested-to-nested
     *                   contours, and so on. This parameter is only taken into
     *                   account when there is hierarchy available.
     *                   \(\texttt{offset}=(dx,dy)\) . <b>Note:</b> When
     *                   thickness=#FILLED, the function is designed to handle
     *                   connected components with holes correctly even when no
     *                   hierarchy date is provided. This is done by analyzing all
     ...
     *                   negative (for example, thickness=#FILLED ), the contour
     *                   interiors are drawn.
     * @param lineType   Line connectivity. See #LineTypes some of the contours (see
     *                   maxLevel ). If it is 1, the function draws the contour(s)
     *                   and all the nested contours. If it is 2, the function draws
     *                   the contours, all the nested contours, all the
     *                   nested-to-nested contours, and so on. This parameter is
     *                   only taken into account when there is hierarchy available.
     *                   \(\texttt{offset}=(dx,dy)\) . <b>Note:</b> When
     *                   thickness=#FILLED, the function is designed to handle
     *                   connected components with holes correctly even when no
     ...
     * @param thickness  Thickness of lines the contours are drawn with. If it is
     *                   negative (for example, thickness=#FILLED ), the contour
     *                   interiors are drawn. some of the contours (see maxLevel ).
     *                   If it is 1, the function draws the contour(s) and all the
     *                   nested contours. If it is 2, the function draws the
     *                   contours, all the nested contours, all the nested-to-nested
     *                   contours, and so on. This parameter is only taken into
     ...
     * @param contourIdx Parameter indicating a contour to draw. If it is negative,
     *                   all the contours are drawn.
     * @param color      Color of the contours. thickness=#FILLED ), the contour
     *                   interiors are drawn. some of the contours (see maxLevel ).
     *                   If it is 1, the function draws the contour(s) and all the
     *                   nested contours. If it is 2, the function draws the
     *                   contours, all the nested contours, all the nested-to-nested
    ...
    //
    // C++: void cv::drawMarker(Mat& img, Point position, Scalar color, int
     ...
     * For the moment several marker types are supported, see #MarkerTypes for more
     * information.
     *
     * @param img        Image.
    ...
    //
    // C++: void cv::ellipse(Mat& img, Point center, Size axes, double angle, double
    // startAngle, double endAngle, Scalar color, int thickness = 1, int lineType =
    // LINE_8, int shift = 0)
    //
     ...
     * filled ellipse, an elliptic arc, or a filled ellipse sector. The drawing code
     * uses general parametric form. A piecewise-linear curve is used to approximate
     * the elliptic arc boundary. If you need more control of the ellipse rendering,
     * you can retrieve the curve using #ellipse2Poly and then render it with
     * #polylines or fill it with #fillPoly. If you use the first variant of the
     * function and want to draw the whole ellipse, not an arc, pass
     * {@code startAngle=0} and {@code endAngle=360}. If {@code startAngle} is
     * greater than {@code endAngle}, they are swapped. The figure below explains
     ...
     *                   Otherwise, this indicates that a filled ellipse sector is
     *                   to be drawn.
    ...
    //
    // C++: void cv::ellipse(Mat& img, RotatedRect box, Scalar color, int thickness
    // = 1, int lineType = LINE_8)
    //

    /**
     * @param img       Image.
     * @param box       Alternative ellipse representation via RotatedRect. This
     *                  means that the function draws an ellipse inscribed in the
     *                  rotated rectangle.
     * @param color     Ellipse color.
     * @param thickness Thickness of the ellipse arc outline, if positive.
     *                  Otherwise, this indicates that a filled ellipse sector is to
     *                  be drawn.
     * @param lineType  Type of the ellipse boundary. See #LineTypes
     */
     ...
     *              that the function draws an ellipse inscribed in the rotated
     *              rectangle.
     * @param color Ellipse color. a filled ellipse sector is to be drawn.
     */
    ...
    //
    // C++: void cv::ellipse2Poly(Point center, Size axes, int angle, int arcStart,
     ...
     * approximates the specified elliptic arc. It is used by #ellipse. If
     * {@code arcStart} is greater than {@code arcEnd}, they are swapped.
     *
     ...
     * <li>Compute the integral of the histogram: \(H'_i = \sum _{0 \le j &lt; i}
     * H(j)\)</li>
    ...
    //
    // C++: void cv::erode(Mat src, Mat& dst, Mat kernel, Point anchor =
    // Point(-1,-1), int iterations = 1, int borderType = BORDER_CONSTANT, Scalar
    // borderValue = morphologyDefaultBorderValue())
    //
     ...
     *                    {@code element=Mat()}, a {@code 3 x 3} rectangular
     ...
     * @param borderType  pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                    is not supported.
     * @param borderValue border value in case of a constant border SEE: dilate,
     *                    morphologyEx, getStructuringElement
     */
     ...
     * @param iterations number of times erosion is applied.
     * @param borderType pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported. SEE: dilate, morphologyEx,
     *                   getStructuringElement
     */
    public static void erode(Mat src, Mat dst, Mat kernel, Point anchor, int iterations, int borderType) {
     ...
     * @param anchor     position of the anchor within the element; default value
     *                   (-1, -1) means that the anchor is at the element center.
     * @param iterations number of times erosion is applied. SEE: dilate,
     *                   morphologyEx, getStructuringElement
     */
     ...
     *               depth should be one of CV_8U, CV_16U, CV_16S, CV_32F or CV_64F.
     * @param dst    output image of the same size and type as src.
     * @param kernel structuring element used for erosion; if {@code element=Mat()},
     *               a {@code 3 x 3} rectangular structuring element is used. Kernel
     *               can be created using #getStructuringElement.
     * @param anchor position of the anchor within the element; default value (-1,
     *               -1) means that the anchor is at the element center. SEE:
     ...
     * @param kernel structuring element used for erosion; if {@code element=Mat()},
     *               a {@code 3 x 3} rectangular structuring element is used. Kernel
     *               can be created using #getStructuringElement. anchor is at the
     *               element center. SEE: dilate, morphologyEx,
     *               getStructuringElement
     */
    ...
    //
    // C++: void cv::fillConvexPoly(Mat& img, vector_Point points, Scalar color, int
    ...
    //
    // C++: void cv::fillPoly(Mat& img, vector_vector_Point pts, Scalar color, int
     ...
     *
     * @param img   Image.
     * @param pts   Array of polygons where each polygon is represented as an array
     *              of points.
     * @param color Polygon color.
     */
    ...
    //
    // C++: void cv::filter2D(Mat src, Mat& dst, int ddepth, Mat kernel, Point
     ...
     * (x',y')* \texttt{src} (x+x'- \texttt{anchor.x} ,y+y'- \texttt{anchor.y} )\)
     * <p>
     ...
     * kernels (~{@code 11 x 11} or larger) and the direct algorithm for small
     * kernels.
     *
     * @param src        input image.
     * @param dst        output image of the same size and the same number of
     ...
     *                   single-channel floating point matrix; if you want to apply
     *                   different kernels to different channels, split the image
     *                   into separate color planes using split and process them
     *                   individually.
     * @param anchor     anchor of the kernel that indicates the relative position
     *                   of a filtered point within the kernel; the anchor should
     *                   lie within the kernel; default value (-1,-1) means that the
     *                   anchor is at the kernel center.
     * @param delta      optional value added to the filtered pixels before storing
     *                   them in dst.
     * @param borderType pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported. SEE: sepFilter2D, dft, matchTemplate
     */
     ...
     * @param kernel convolution kernel (or rather a correlation kernel), a
     *               single-channel floating point matrix; if you want to apply
     *               different kernels to different channels, split the image into
     *               separate color planes using split and process them
     *               individually.
     ...
     *               the kernel; default value (-1,-1) means that the anchor is at
     *               the kernel center.
     * @param delta  optional value added to the filtered pixels before storing them
     *               in dst. SEE: sepFilter2D, dft, matchTemplate
     */
     ...
     *               filtered point within the kernel; the anchor should lie within
     *               the kernel; default value (-1,-1) means that the anchor is at
     *               the kernel center. SEE: sepFilter2D, dft, matchTemplate
     */
     ...
     *               different kernels to different channels, split the image into
     *               separate color planes using split and process them
     *               individually. the kernel; the anchor should lie within the
     *               kernel; default value (-1,-1) means that the anchor is at the
    ...
    //
    // C++: void cv::findContours(Mat image, vector_vector_Point& contours, Mat&
     ...
     * CITE: Suzuki85 . The contours are a useful tool for shape analysis and object
     ...
     *                  treated as 1's. Zero pixels remain 0's, so the image is
     *                  treated as binary . You can use #compare, #inRange,
     *                  #threshold , #adaptiveThreshold, #Canny, and others to
     *                  create a binary image out of a grayscale or color one. If
     *                  mode equals to #RETR_CCOMP or #RETR_FLOODFILL, the input can
     ...
     *                  containing information about the image topology. It has as
     *                  many elements as the number of contours. For each i-th
     *                  contour contours[i], the elements hierarchy[i][0] ,
     *                  hierarchy[i][1] , hierarchy[i][2] , and hierarchy[i][3] are
     *                  set to 0-based indices in contours of the next and previous
     *                  contours at the same hierarchical level, the first child
     *                  contour and the parent contour, respectively. If for the
     *                  contour i there are no next, previous, parent, or nested
     *                  contours, the corresponding elements of hierarchy[i] will be
     *                  negative.
     ...
     *                  ROI and then they should be analyzed in the whole image
     *                  context.
     */
     ...
     * @param mode      Contour retrieval mode, see #RetrievalModes
     * @param method    Contour approximation method, see #ContourApproximationModes
     *                  contours are extracted from the image ROI and then they
    ...
    //
    // C++: void cv::fitLine(Mat points, Mat& line, int distType, double param,
     ...
     * \(\sum_i \rho(r_i)\) where \(r_i\) is a distance between the \(i^{th}\)
     * point, the line and \(\rho(r)\) is a distance function, one of the following:
     * <ul>
     * <li>DIST_L2 \(\rho (r) = r^2/2 \quad \text{(the simplest and the fastest
     * least-squares method)}\)</li>
     * <li>DIST_L1 \(\rho (r) = r\)</li>
     * <li>DIST_L12 \(\rho (r) = 2 \cdot ( \sqrt{1 + \frac{r^2}{2}} - 1)\)</li>
     * <li>DIST_FAIR \(\rho \left (r \right ) = C^2 \cdot \left ( \frac{r}{C} -
     * \log{\left(1 + \frac{r}{C}\right)} \right ) \quad \text{where} \quad
     * C=1.3998\)</li>
     * <li>DIST_WELSCH \(\rho \left (r \right ) = \frac{C^2}{2} \cdot \left ( 1 -
     * \exp{\left(-\left(\frac{r}{C}\right)^2\right)} \right ) \quad \text{where}
     * \quad C=2.9846\)</li>
     * <li>DIST_HUBER \(\rho (r) = \fork{r^2/2}{if \(r &lt; C\)}{C \cdot
     * (r-C/2)}{otherwise} \quad \text{where} \quad C=1.345\)</li>
     * </ul>
     * <p>
     ...
     *                 vector of 4 elements (like Vec4f) - (vx, vy, x0, y0), where
     *                 (vx, vy) is a normalized vector collinear to the line and
     *                 (x0, y0) is a point on the line. In case of 3D fitting, it
     *                 should be a vector of 6 elements (like Vec6f) - (vx, vy, vz,
     *                 x0, y0, z0), where (vx, vy, vz) is a normalized vector
     *                 collinear to the line and (x0, y0, z0) is a point on the
     *                 line.
     * @param distType Distance used by the M-estimator, see #DistanceTypes
     * @param param    Numerical parameter ( C ) for some types of distances. If it
     *                 is 0, an optimal value is chosen.
    ...
    //
    // C++: void cv::getDerivKernels(Mat& kx, Mat& ky, int dx, int dy, int ksize,
     ...
     * derivatives. When {@code ksize=FILTER_SCHARR}, the Scharr \(3 \times 3\)
     ...
     *                  coefficients or not. Theoretically, the coefficients should
     *                  have the denominator \(=2^{ksize*2-dx-dy-2}\). If you are
     *                  going to filter floating-point images, you are likely to use
     *                  the normalized kernels. But if you compute derivatives of an
     *                  8-bit image, store the results in a 16-bit image, and wish
     *                  to preserve all the fractional bits, you may want to set
     *                  normalize=false .
     * @param ktype     Type of filter coefficients. It can be CV_32f or CV_64F .
     */
     ...
     *              \(=2^{ksize*2-dx-dy-2}\). If you are going to filter
     *              floating-point images, you are likely to use the normalized
    ...
    //
    // C++: void cv::getRectSubPix(Mat image, Size patchSize, Point2f center, Mat&
     ...
     * <p>
     * \(patch(x, y) = src(x + \texttt{center.x} - ( \texttt{dst.cols} -1)*0.5, y +
     ...
     * independently. Also the image should be a single channel or three channel
     * image. While the center of the rectangle must be inside the image, parts of
     * the rectangle may be outside.
     *
     * @param image     Source image.
     * @param patchSize Size of the extracted patch.
     * @param center    Floating point coordinates of the center of the extracted
     *                  rectangle within the source image. The center must be inside
     *                  the image.
    ...
    //
    // C++: void cv::goodFeaturesToTrack(Mat image, vector_Point& corners, int
    // maxCorners, double qualityLevel, double minDistance, Mat mask, int blockSize,
    // int gradientSize, bool useHarrisDetector = false, double k = 0.04)
    //
    ...
    //
    // C++: void cv::goodFeaturesToTrack(Mat image, vector_Point& corners, int
    // maxCorners, double qualityLevel, double minDistance, Mat mask = Mat(), int
     ...
     * specified image region, as described in CITE: Shi94
     *
     * <ul>
     * <li>Function calculates the corner quality measure at every source image
     * pixel using the #cornerMinEigenVal or #cornerHarris .</li>
     * <li>Function performs a non-maximum suppression (the local maximums in *3 x
     * 3* neighborhood are retained).</li>
     * <li>The corners with the minimal eigenvalue less than \(\texttt{qualityLevel}
     * \cdot \max_{x,y} qualityMeasureMap(x,y)\) are rejected.</li>
     * <li>The remaining corners are sorted by the quality measure in the descending
     * order.</li>
     * <li>Function throws away each corner for which there is a stronger corner at
     * a distance less than maxDistance.</li>
     * </ul>
     * <p>
     * The function can be used to initialize a point-based tracker of an object.
     *
     * <b>Note:</b> If the function is called with different values A and B of the
     ...
     * @param maxCorners        Maximum number of corners to return. If there are
     *                          more corners than are found, the strongest of them
     *                          is returned. {@code maxCorners &lt;= 0} implies that
     *                          no limit on the maximum is set and all detected
     *                          corners are returned.
     * @param qualityLevel      Parameter characterizing the minimal accepted
     *                          quality of image corners. The parameter value is
     *                          multiplied by the best corner quality measure, which
     *                          is the minimal eigenvalue (see #cornerMinEigenVal )
     *                          or the Harris function response (see #cornerHarris
     *                          ). The corners with the quality measure less than
     *                          the product are rejected. For example, if the best
     *                          corner has the quality measure = 1500, and the
     *                          qualityLevel=0.01 , then all the corners with the
     *                          quality measure less than 15 are rejected.
     * @param minDistance       Minimum possible Euclidean distance between the
     *                          returned corners.
     * @param mask              Optional region of interest. If the image is not
     *                          empty (it needs to have the type CV_8UC1 and the
     *                          same size as image ), it specifies the region in
     ...
     *                          <p>
     *                          SEE: cornerMinEigenVal, cornerHarris,
     *                          calcOpticalFlowPyrLK, estimateRigidTransform,
     */
    public static void goodFeaturesToTrack(Mat image, MatOfPoint corners, int maxCorners, double qualityLevel,
     ...
     *                     corners than are found, the strongest of them is
     *                     returned. {@code maxCorners &lt;= 0} implies that no
     *                     limit on the maximum is set and all detected corners are
     *                     returned.
     ...
     *                     function response (see #cornerHarris ). The corners with
     *                     the quality measure less than the product are rejected.
     *                     For example, if the best corner has the quality measure =
     *                     1500, and the qualityLevel=0.01 , then all the corners
     ...
     *                     corners.
     * @param mask         Optional region of interest. If the image is not empty
     *                     (it needs to have the type CV_8UC1 and the same size as
     *                     image ), it specifies the region in which the corners are
     *                     detected.
     * @param blockSize    Size of an average block for computing a derivative
     *                     covariation matrix over each pixel neighborhood. See
     ...
     *                     (it needs to have the type CV_8UC1 and the same size as
     *                     image ), it specifies the region in which the corners are
     *                     detected. pixel neighborhood. See cornerEigenValsAndVecs
     ...
     *                     with the quality measure less than 15 are rejected.
     * @param minDistance  Minimum possible Euclidean distance between the returned
     *                     corners. CV_8UC1 and the same size as image ), it
     *                     specifies the region in which the corners are detected.
     *                     pixel neighborhood. See cornerEigenValsAndVecs . or
     *                     #cornerMinEigenVal.
     *                     <p>
     *                     SEE: cornerMinEigenVal, cornerHarris,
     *                     calcOpticalFlowPyrLK, estimateRigidTransform,
    ...
    //
    // C++: void cv::grabCut(Mat img, Mat& mask, Rect rect, Mat& bgdModel, Mat&
     ...
     *                  initialized by the function when mode is set to
     *                  #GC_INIT_WITH_RECT. Its elements may have one of the
     *                  #GrabCutClasses.
     * @param rect      ROI containing a segmented object. The pixels outside of the
     *                  ROI are marked as "obvious background". The parameter is
     *                  only used when mode==#GC_INIT_WITH_RECT .
     ...
     *                  returning the result. Note that the result can be refined
     *                  with further calls with mode==#GC_INIT_WITH_MASK or
     *                  mode==GC_EVAL .
    ...
    //
    // C++: void cv::integral(Mat src, Mat& sum, Mat& sqsum, Mat& tilted, int sdepth
    // = -1, int sqdepth = -1)
    //
     ...
     * <p>
     * \(\texttt{tilted} (X,Y) = \sum _{y&lt;Y,abs(x-X+1) \leq Y-y-1} \texttt{image}
     * (x,y)\)
     * <p>
     ...
     * <p>
     * \(\sum _{x_1 \leq x &lt; x_2, \, y_1 \leq y &lt; y_2} \texttt{image} (x,y) =
     ...
     * . The selected pixels in the original image are shown, as well as the
     * relative pixels in the integral images sum and tilted .
     * <p>
     * ![integral calculation example](pics/integral.png)
     *
     * @param src     input image as \(W \times H\), 8-bit or floating-point (32f or
     *                64f).
     * @param sum     integral image as \((W+1)\times (H+1)\) , 32-bit integer or
     *                floating-point (32f or 64f).
     * @param sqsum   integral image for squared pixel values; it is \((W+1)\times
     *                (H+1)\), double-precision floating-point (64f) array.
     * @param tilted  integral for the image rotated by 45 degrees; it is
     *                \((W+1)\times (H+1)\) array with the same data type as sum.
    ...
    //
    // C++: void cv::integral(Mat src, Mat& sum, Mat& sqsum, int sdepth = -1, int
    // sqdepth = -1)
    //
     ...
     * <p>
     * The function computes an inverse affine transformation represented by \(2
     * \times 3\) matrix M:
     * <p>
     * \(\begin{bmatrix} a_{11} &amp; a_{12} &amp; b_1 \\ a_{21} &amp; a_{22} &amp;
     * b_2 \end{bmatrix}\)
     * <p>
    ...
    //
    // C++: void cv::line(Mat& img, Point pt1, Point pt2, Scalar color, int
    // thickness = 1, int lineType = LINE_8, int shift = 0)
    //
     ...
     * with integer coordinates, the 8-connected or 4-connected Bresenham algorithm
     * is used. Thick lines are drawn with rounding endings. Antialiased lines are
    ...
    //
    // C++: void cv::linearPolar(Mat src, Mat& dst, Point2f center, double
    // maxRadius, int flags)
    //
    ...
    // */
    // @Deprecated
    // public static void linearPolar(Mat src, Mat dst, Point center, double
    // maxRadius, int flags) {
    // linearPolar_0(src.nativeObj, dst.nativeObj, center.x, center.y, maxRadius,
    // flags);
    // }

    //
    // C++: void cv::logPolar(Mat src, Mat& dst, Point2f center, double M, int
    // flags)
    //
    ...
    // */
    // @Deprecated
    // public static void logPolar(Mat src, Mat dst, Point center, double M, int
    // flags) {
    // logPolar_0(src.nativeObj, dst.nativeObj, center.x, center.y, M, flags);
    // }

    //
    // C++: void cv::matchTemplate(Mat image, Mat templ, Mat& result, int method,
    // Mat mask = Mat())
    //
     ...
     * \(w \times h\) against templ using the specified method and stores the
     * comparison results in result . Here are the formulae for the available
     * comparison methods ( \(I\) denotes image, \(T\) template, \(R\) result ). The
     * summation is done over template and/or the image patch: \(x' = 0...w-1, y' =
     * 0...h-1\)
     * <p>
     ...
     * over all of the channels and separate mean values are used for each channel.
     * That is, the function can take a color template and a color image. The result
     * will still be a single-channel image, which is easier to analyze.
     *
     ...
     *               floating-point. If image is \(W \times H\) and templ is \(w
     *               \times h\) , then result is \((W-w+1) \times (H-h+1)\) .
     ...
     *               size with templ. It is not set by default. Currently, only the
     *               #TM_SQDIFF and #TM_CCORR_NORMED methods are supported.
     */
     ...
     *               \times h\) , then result is \((W-w+1) \times (H-h+1)\) .
     * @param method Parameter specifying the comparison method, see
     *               #TemplateMatchModes not set by default. Currently, only the
     *               #TM_SQDIFF and #TM_CCORR_NORMED methods are supported.
     */
     ...
     * supported.
     *
     * <b>Note:</b> The median filter uses #BORDER_REPLICATE internally to cope with
     ...
     *              example: 3, 5, 7 ... SEE: bilateralFilter, blur, boxFilter,
     *              GaussianBlur
     */
    ...
    //
    // C++: void cv::minEnclosingCircle(vector_Point2f points, Point2f& center,
    // float& radius)
    //
    ...
    //
    // C++: void cv::morphologyEx(Mat src, Mat& dst, int op, Mat kernel, Point
     ...
     *                    CV_64F.
     * @param dst         Destination image of the same size and type as source
     *                    image.
     ...
     *                    value has a special meaning. SEE: dilate, erode,
     *                    getStructuringElement <b>Note:</b> The number of
     *                    iterations is the number of times erosion or dilatation
     *                    operation will be applied. For instance, an opening
     *                    operation (#MORPH_OPEN) with two iterations is equivalent
     *                    to apply successively: erode -&gt; erode -&gt; dilate
     *                    -&gt; dilate (and not erode -&gt; dilate -&gt; erode -&gt;
     *                    dilate).
     */
     ...
     * @param iterations Number of times erosion and dilation are applied.
     * @param borderType Pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported. meaning. SEE: dilate, erode,
     *                   getStructuringElement <b>Note:</b> The number of iterations
     *                   is the number of times erosion or dilatation operation will
     *                   be applied. For instance, an opening operation
     *                   (#MORPH_OPEN) with two iterations is equivalent to apply
     *                   successively: erode -&gt; erode -&gt; dilate -&gt; dilate
     ...
     * @param anchor     Anchor position with the kernel. Negative values mean that
     *                   the anchor is at the kernel center.
     * @param iterations Number of times erosion and dilation are applied. meaning.
     *                   SEE: dilate, erode, getStructuringElement <b>Note:</b> The
     *                   number of iterations is the number of times erosion or
     *                   dilatation operation will be applied. For instance, an
     *                   opening operation (#MORPH_OPEN) with two iterations is
     *                   equivalent to apply successively: erode -&gt; erode -&gt;
     *                   dilate -&gt; dilate (and not erode -&gt; dilate -&gt; erode
     *                   -&gt; dilate).
     */
    public static void morphologyEx(Mat src, Mat dst, int op, Mat kernel, Point anchor, int iterations) {
     ...
     *               anchor is at the kernel center. meaning. SEE: dilate, erode,
     *               getStructuringElement <b>Note:</b> The number of iterations is
     *               the number of times erosion or dilatation operation will be
     *               applied. For instance, an opening operation (#MORPH_OPEN) with
     *               two iterations is equivalent to apply successively: erode -&gt;
     *               erode -&gt; dilate -&gt; dilate (and not erode -&gt; dilate
     *               -&gt; erode -&gt; dilate).
     */
     ...
     * @param op     Type of a morphological operation, see #MorphTypes
     * @param kernel Structuring element. It can be created using
     *               #getStructuringElement. kernel center. meaning. SEE: dilate,
     *               erode, getStructuringElement <b>Note:</b> The number of
     *               iterations is the number of times erosion or dilatation
     *               operation will be applied. For instance, an opening operation
     *               (#MORPH_OPEN) with two iterations is equivalent to apply
     *               successively: erode -&gt; erode -&gt; dilate -&gt; dilate (and
     *               not erode -&gt; dilate -&gt; erode -&gt; dilate).
     */
    ...
    //
    // C++: void cv::polylines(Mat& img, vector_vector_Point pts, bool isClosed,
     ...
     *                  not. If they are closed, the function draws a line from the
     ...
     * @param shift     Number of fractional bits in the vertex coordinates.
     *                  <p>
     *                  The function cv::polylines draws one or more polygonal
     *                  curves.
     */
    ...
    //
    // C++: void cv::preCornerDetect(Mat src, Mat& dst, int ksize, int borderType =
    // BORDER_DEFAULT)
    //
     ...
     * source image
     * <p>
     * \(\texttt{dst} = (D_x \texttt{src} )^2 \cdot D_{yy} \texttt{src} + (D_y
     * \texttt{src} )^2 \cdot D_{xx} \texttt{src} - 2 D_x \texttt{src} \cdot D_y
     * \texttt{src} \cdot D_{xy} \texttt{src}\)
     * <p>
     * where \(D_x\),\(D_y\) are the first image derivatives, \(D_{xx}\),\(D_{yy}\)
     * are the second image derivatives, and \(D_{xy}\) is the mixed derivative.
     * <p>
    ...
    //
    // C++: void cv::putText(Mat& img, String text, Point org, int fontFace, double
     ...
     *                         bottom-left corner. Otherwise, it is at the top-left
     *                         corner.
     */
     ...
     *                  base size.
     * @param color     Text color.
     * @param thickness Thickness of the lines used to draw a text. it is at the
     *                  top-left corner.
     */
    public static void putText(Mat img, String text, Point org, int fontFace, double fontScale, Scalar color,
            int thickness) {
    ...
    //
    // C++: void cv::pyrDown(Mat src, Mat& dst, Size dstsize = Size(), int
     ...
     * {@code Size((src.cols+1)/2, (src.rows+1)/2)}, but in any case, the following
     ...
     * <p>
     * \(\frac{1}{256} \begin{bmatrix} 1 &amp; 4 &amp; 6 &amp; 4 &amp; 1 \\ 4 &amp;
     * 16 &amp; 24 &amp; 16 &amp; 4 \\ 6 &amp; 24 &amp; 36 &amp; 24 &amp; 6 \\ 4
     ...
     *
     * @param src        input image.
     * @param dst        output image; it has the specified size and the same type
     *                   as src.
    ...
    //
    // C++: void cv::pyrMeanShiftFiltering(Mat src, Mat& dst, double sp, double sr,
     ...
     * <p>
     * \((x,y): X- \texttt{sp} \le x \le X+ \texttt{sp} , Y- \texttt{sp} \le y \le
     * Y+ \texttt{sp} , ||(R,G,B)-(r,g,b)|| \le \texttt{sr}\)
     * <p>
    ...
    //
    // C++: void cv::pyrUp(Mat src, Mat& dst, Size dstsize = Size(), int borderType
    // = BORDER_DEFAULT)
    //
     ...
     * <p>
     * By default, size of the output image is computed as
     * {@code Size(src.cols\*2, (src.rows\*2)}, but in any case, the following
     * conditions should be satisfied:
     * <p>
     * \(\begin{array}{l} | \texttt{dstsize.width} -src.cols*2| \leq (
     * \texttt{dstsize.width} \mod 2) \\ | \texttt{dstsize.height} -src.rows*2| \leq
     * ( \texttt{dstsize.height} \mod 2) \end{array}\)
     * <p>
    ...
    //
    // C++: void cv::rectangle(Mat& img, Point pt1, Point pt2, Scalar color, int
    // thickness = 1, int lineType = LINE_8, int shift = 0)
    //
     ...
     *                  values, like #FILLED, mean that the function has to draw a
     *                  filled rectangle.
     * @param lineType  Type of the line. See #LineTypes
     * @param shift     Number of fractional bits in the point coordinates.
     */
     ...
     * @param pt1   Vertex of the rectangle.
     * @param pt2   Vertex of the rectangle opposite to pt1 .
     * @param color Rectangle color or brightness (grayscale image). mean that the
    ...
    //
    // C++: void cv::rectangle(Mat& img, Rect rec, Scalar color, int thickness = 1,
    // int lineType = LINE_8, int shift = 0)
    //

    /**
     * use {@code rec} parameter as alternative specification of the drawn
     * rectangle: `r.tl() and r.br()-Point(1,1)` are opposite corners
     *
    ...
    //
    // C++: void cv::remap(Mat src, Mat& dst, Mat map1, Mat map2, int interpolation,
     ...
     * available interpolation methods. \(map_x\) and \(map_y\) can be encoded as
     * separate floating-point maps in \(map_1\) and \(map_2\) respectively, or
     * interleaved floating-point maps of \((x,y)\) in \(map_1\), or fixed-point
     ...
     * faster (\~2x) remapping operations. In the converted case, \(map_1\) contains
     * pairs (cvFloor(x), cvFloor(y)) and \(map_2\) contains indices in a table of
     * interpolation coefficients.
     * <p>
     * This function cannot operate in-place.
     *
     * @param src           Source image.
     * @param dst           Destination image. It has the same size as map1 and the
     *                      same type as src .
     * @param map1          The first map of either (x,y) points or just x values
     *                      having the type CV_16SC2 , CV_32FC1, or CV_32FC2. See
     *                      convertMaps for details on converting a floating point
     ...
     *                      borderMode=#BORDER_TRANSPARENT, it means that the pixels
     *                      in the destination image that corresponds to the
     *                      "outliers" in the source image are not modified by the
     *                      function.
     * @param borderValue   Value used in case of a constant border. By default, it
     *                      is 0. <b>Note:</b> Due to current implementation
     *                      limitations the size of an input and output images
     ...
     *                      in the destination image that corresponds to the
     *                      "outliers" in the source image are not modified by the
     *                      function. <b>Note:</b> Due to current implementation
     *                      limitations the size of an input and output images
     *                      should be less than 32767x32767.
     */
    public static void remap(Mat src, Mat dst, Mat map1, Mat map2, int interpolation, int borderMode) {
    ...
    //
    // C++: void cv::resize(Mat src, Mat& dst, Size dsize, double fx = 0, double fy
     ...
     * Instead, the size and type are derived from the
     * {@code src},{@code dsize},{@code fx}, and {@code fy}. If you want to resize
     * src so that it fits the pre-created dst, you may call the function as
     * follows: <code>
     * // explicitly specify dsize=dst.size(); fx and fy will be computed from that.
     * resize(src, dst, dst.size(), 0, 0, interpolation);
     * </code> If you want to decimate the image by factor of 2 in each direction,
     * you can call the function this way: <code>
     * // specify fx and fy and let the function compute the destination image size.
     * resize(src, dst, Size(), 0.5, 0.5, interpolation);
     * </code> To shrink an image, it will generally look best with #INTER_AREA
     * interpolation, whereas to enlarge an image, it will generally look best with
     * c#INTER_CUBIC (slow) or #INTER_LINEAR (faster but still looks OK).
     *
     * @param src           input image.
     * @param dst           output image; it has the size dsize (when it is
     *                      non-zero) or the size computed from src.size(), fx, and
     *                      fy; the type of dst is the same as of src.
     * @param dsize         output image size; if it equals zero, it is computed as:
     *                      \(\texttt{dsize = Size(round(fx*src.cols),
     *                      round(fy*src.rows))}\) Either dsize or both fx and fy
     *                      must be non-zero.
     * @param fx            scale factor along the horizontal axis; when it equals
     *                      0, it is computed as
     ...
     *
     * @param src   input image.
     * @param dst   output image; it has the size dsize (when it is non-zero) or the
     *              size computed from src.size(), fx, and fy; the type of dst is
     *              the same as of src.
     * @param dsize output image size; if it equals zero, it is computed as:
     *              \(\texttt{dsize = Size(round(fx*src.cols),
     *              round(fy*src.rows))}\) Either dsize or both fx and fy must be
     *              non-zero.
     * @param fx    scale factor along the horizontal axis; when it equals 0, it is
     *              computed as \(\texttt{(double)dsize.width/src.cols}\)
     * @param fy    scale factor along the vertical axis; when it equals 0, it is
     *              computed as \(\texttt{(double)dsize.height/src.rows}\)
     *              <p>
     *              SEE: warpAffine, warpPerspective, remap
     */
    ...
    //
    // C++: void cv::sepFilter2D(Mat src, Mat& dst, int ddepth, Mat kernelX, Mat
     ...
     * every row of src is filtered with the 1D kernel kernelX. Then, every column
     * of the result is filtered with the 1D kernel kernelY. The final result
     ...
     * @param borderType Pixel extrapolation method, see #BorderTypes. #BORDER_WRAP
     *                   is not supported. SEE: filter2D, Sobel, GaussianBlur,
     *                   boxFilter, blur
     */
     ...
     * @param anchor  Anchor position within the kernel. The default value
     *                \((-1,-1)\) means that the anchor is at the kernel center.
     * @param delta   Value added to the filtered results before storing them. SEE:
     ...
     * @param ddepth  Destination image depth, see REF: filter_depths "combinations"
     * @param kernelX Coefficients for filtering each row.
     * @param kernelY Coefficients for filtering each column. is at the kernel
     *                center. SEE: filter2D, Sobel, GaussianBlur, boxFilter, blur
     */
    ...
    //
    // C++: void cv::spatialGradient(Mat src, Mat& dx, Mat& dy, int ksize = 3, int
    // borderType = BORDER_DEFAULT)
    //

    /**
     * Calculates the first order image derivative in both x and y using a Sobel
     * operator
     * <p>
     ...
     * @param borderType pixel extrapolation method, see #BorderTypes. Only
     *                   #BORDER_DEFAULT=#BORDER_REFLECT_101 and #BORDER_REPLICATE
     *                   are supported.
     *                   <p>
     *                   SEE: Sobel
     */
     ...
     * @param dx    output image with first-order derivative in x.
     * @param dy    output image with first-order derivative in y.
     * @param ksize size of Sobel kernel. It must be 3. Only
     *              #BORDER_DEFAULT=#BORDER_REFLECT_101 and #BORDER_REPLICATE are
     *              supported.
     *              <p>
     *              SEE: Sobel
     */
    ...
    //
    // C++: void cv::sqrBoxFilter(Mat src, Mat& dst, int ddepth, Size ksize, Point
    // anchor = Point(-1, -1), bool normalize = true, int borderType =
    // BORDER_DEFAULT)
    //

    /**
     * Calculates the normalized sum of squares of the pixel values overlapping the
     * filter.
     * <p>
     * For every pixel \( (x, y) \) in the source image, the function calculates the
     * sum of squares of those neighboring pixel values which overlap the filter
     * placed over the pixel \( (x, y) \).
     * <p>
     ...
     *                   denotes that the anchor is at the kernel center.
     * @param normalize  flag, specifying whether the kernel is to be normalized by
     *                   it's area or not.
     * @param borderType border mode used to extrapolate pixels outside of the
     *                   image, see #BorderTypes. #BORDER_WRAP is not supported.
     *                   SEE: boxFilter
     ...
     *                  denotes that the anchor is at the kernel center.
     * @param normalize flag, specifying whether the kernel is to be normalized by
     *                  it's area or not. SEE: boxFilter
     */
    public static void sqrBoxFilter(Mat src, Mat dst, int ddepth, Size ksize, Point anchor, boolean normalize) {
     ...
     *               that the anchor is at the kernel center. SEE: boxFilter
     */
    ...
    //
    // C++: void cv::warpAffine(Mat src, Mat& dst, Mat M, Size dsize, int flags =
     ...
     * matrix:
     * <p>
     * \(\texttt{dst} (x,y) = \texttt{src} ( \texttt{M} _{11} x + \texttt{M} _{12} y
     ...
     *
     * @param src         input image.
     * @param dst         output image that has the size dsize and the same type as
     *                    src .
     * @param M           \(2\times 3\) transformation matrix.
     * @param dsize       size of the output image.
     * @param flags       combination of interpolation methods (see
     *                    #InterpolationFlags) and the optional flag
     *                    #WARP_INVERSE_MAP that means that M is the inverse
     *                    transformation ( \(\texttt{dst}\rightarrow\texttt{src}\)
     *                    ).
     * @param borderMode  pixel extrapolation method (see #BorderTypes); when
     *                    borderMode=#BORDER_TRANSPARENT, it means that the pixels
     *                    in the destination image corresponding to the "outliers"
     ...
     *                    0.
     *                    <p>
     *                    SEE: warpPerspective, resize, remap, getRectSubPix,
     *                    transform
     */
     ...
     *                   transformation ( \(\texttt{dst}\rightarrow\texttt{src}\) ).
     * @param borderMode pixel extrapolation method (see #BorderTypes); when
     *                   borderMode=#BORDER_TRANSPARENT, it means that the pixels in
     *                   the destination image corresponding to the "outliers" in
     ...
     * @param M     \(2\times 3\) transformation matrix.
     * @param dsize size of the output image.
     * @param flags combination of interpolation methods (see #InterpolationFlags)
     ...
     * @param dst   output image that has the size dsize and the same type as src .
     * @param M     \(2\times 3\) transformation matrix.
     * @param dsize size of the output image. flag #WARP_INVERSE_MAP that means that
     *              M is the inverse transformation (
    ...
    //
    // C++: void cv::warpPerspective(Mat src, Mat& dst, Mat M, Size dsize, int flags
    // = INTER_LINEAR, int borderMode = BORDER_CONSTANT, Scalar borderValue =
    // Scalar())
    //
     ...
     * matrix:
     * <p>
     * \(\texttt{dst} (x,y) = \texttt{src} \left ( \frac{M_{11} x + M_{12} y +
     * M_{13}}{M_{31} x + M_{32} y + M_{33}} , \frac{M_{21} x + M_{22} y +
     * M_{23}}{M_{31} x + M_{32} y + M_{33}} \right )\)
     * <p>
     * when the flag #WARP_INVERSE_MAP is set. Otherwise, the transformation is
     * first inverted with invert and then put in the formula above instead of M.
     ...
     *                    #INTER_NEAREST) and the optional flag #WARP_INVERSE_MAP,
     ...
     *                    equals 0.
     *                    <p>
     *                    SEE: warpAffine, resize, remap, getRectSubPix,
     ...
     * @param dst   output image that has the size dsize and the same type as src .
     * @param M     \(3\times 3\) transformation matrix.
     * @param dsize size of the output image. optional flag #WARP_INVERSE_MAP, that
     *              sets M as the inverse transformation (
     *              \(\texttt{dst}\rightarrow\texttt{src}\) ).
     *              <p>
     *              SEE: warpAffine, resize, remap, getRectSubPix,
    ...
    //
    // C++: void cv::warpPolar(Mat src, Mat& dst, Size dsize, Point2f center, double
    // maxRadius, int flags)
    //
     ...
     * <p>
     * polar_remaps_reference_image ![Polar remaps
     * reference](pics/polar_remap_doc.png)
     * <p>
     * Transform the source image using the following transformation: \( dst(\rho ,
     * \phi ) = src(x,y) \)
     * <p>
     * where \( \begin{array}{l} \vec{I} = (x - center.x, \;y - center.y) \\ \phi =
     * Kangle \cdot \texttt{angle} (\vec{I}) \\ \rho = \left\{\begin{matrix} Klin
     * \cdot \texttt{magnitude} (\vec{I}) &amp; default \\ Klog \cdot
     * log_e(\texttt{magnitude} (\vec{I})) &amp; if \; semilog \\
     * \end{matrix}\right. \end{array} \)
     * <p>
     * and \( \begin{array}{l} Kangle = dsize.height / 2\Pi \\ Klin = dsize.width /
     * maxRadius \\ Klog = dsize.width / log_e(maxRadius) \\ \end{array} \)
     * <p>
     * <p>
     * \par Linear vs semilog mapping
     * <p>
     * Polar mapping can be linear or semi-log. Add one of #WarpPolarMode to
     * {@code flags} to specify the polar mapping mode.
     * <p>
     ...
     *
     * <ul>
     * <li>if both values in {@code dsize &lt;=0 } (default), the destination image
     * will have (almost) same area of source bounding circle: \(\begin{array}{l}
     * dsize.area \leftarrow (maxRadius^2 \cdot \Pi) \\ dsize.width =
     * \texttt{cvRound}(maxRadius) \\ dsize.height = \texttt{cvRound}(maxRadius
     * \cdot \Pi) \\ \end{array}\)</li>
     * </ul>
     *
     *
     * <ul>
     * <li>if only {@code dsize.height &lt;= 0}, the destination image area will be
     * proportional to the bounding circle area but scaled by {@code Kx * Kx}:
     * \(\begin{array}{l} dsize.height = \texttt{cvRound}(dsize.width \cdot \Pi) \\
     * \end{array} \)</li>
     * </ul>
     *
     * <ul>
     * <li>if both values in {@code dsize &gt; 0 }, the destination image will have
     * the given size therefore the area of the bounding circle will be scaled to
     * {@code dsize}.</li>
     * </ul>
     * <p>
     * <p>
     ...
     * coordinate \((rho, phi)-&gt;(x, y)\): \snippet polar_transforms.cpp
     * InverseCoordinate
     *
     * @param src       Source image.
     ...
     *                  + #WarpPolarMode.
     *                  <ul>
     *                  <li>Add #WARP_POLAR_LINEAR to select linear polar mapping
     *                  (default)</li>
     ...
     *                  <li>To calculate magnitude and angle in degrees #cartToPolar
     *                  is used internally thus angles are measured from 0 to 360
     *                  with accuracy about 0.3 degrees.</li>
     *                  <li>This function uses #remap. Due to current implementation
     *                  limitations the size of an input and output images should be
     *                  less than 32767x32767.</li>
     *                  </ul>
     *                  <p>
     *                  SEE: cv::remap
     */
     ...
     *
     * <b>Note:</b> Any two neighbor connected components are not necessarily
     * separated by a watershed boundary (-1's pixels); for example, they can touch
     * each other in the initial marker image passed to the function.
     *
    ...
    // C++: Size getTextSize(const String& text, int fontFace, double fontScale, int
    ...
    private static native long createLineSegmentDetector_1(int _refine, double _scale, double _sigma_scale,
            double _quant, double _ang_th, double _log_eps, double _density_th);

    private static native long createLineSegmentDetector_2(int _refine, double _scale, double _sigma_scale,
            double _quant, double _ang_th, double _log_eps);

    private static native long createLineSegmentDetector_3(int _refine, double _scale, double _sigma_scale,
            double _quant, double _ang_th);

    private static native long createLineSegmentDetector_4(int _refine, double _scale, double _sigma_scale,
            double _quant);
    ...
    // C++: void cv::Canny(Mat dx, Mat dy, Mat& edges, double threshold1, double
    // threshold2, bool L2gradient = false)
    private static native void Canny_0(long dx_nativeObj, long dy_nativeObj, long edges_nativeObj, double threshold1,
            double threshold2, boolean L2gradient);

    private static native void Canny_1(long dx_nativeObj, long dy_nativeObj, long edges_nativeObj, double threshold1,
            double threshold2);
    ...
    private static native void Canny_4(long image_nativeObj, long edges_nativeObj, double threshold1,
            double threshold2);
    ...
    // threshold, double min_rho, double max_rho, double rho_step, double min_theta,
    // double max_theta, double theta_step)
    private static native void HoughLinesPointSet_0(long _point_nativeObj, long _lines_nativeObj, int lines_max,
