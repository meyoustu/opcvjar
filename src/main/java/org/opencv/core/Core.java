...
public class Core {
     ...
     * independently for each channel, and return it: \(\begin{array}{l} N = \sum
     * _{I: \; \texttt{mask} (I) \ne 0} 1 \\ M_c = \left ( \sum _{I: \;
     * \texttt{mask} (I) \ne 0}{ \texttt{mtx} (I)_c} \right )/N \end{array}\) When
     * all the mask elements are 0's, the function returns Scalar::all(0)
     *
     * @param src  input array that should have from 1 to 4 channels so that the
     *             result can be stored in Scalar_ .
     * @param mask optional operation mask. SEE: countNonZero, meanStdDev, norm,
     *             minMaxLoc
     ...
     *
     * @param src input array that should have from 1 to 4 channels so that the
     *            result can be stored in Scalar_ . SEE: countNonZero, meanStdDev,
     *            norm, minMaxLoc
     * @return automatically generated
     */
     ...
     *
     * @param src input array that must have from 1 to 4 channels. SEE:
     ...
     * <p>
     * The function cv::trace returns the sum of the diagonal elements of the matrix
     * mtx . \(\mathrm{tr} ( \texttt{mtx} ) = \sum _i \texttt{mtx} (i,i)\)
     *
    ...
    //
    // C++: String cv::samples::findFile(String relative_path, bool required = true,
     ...
     * Search directories:
     * <p>
     * 1. Directories passed via {@code addSamplesDataSearchPath()} 2.
     * OPENCV_SAMPLES_DATA_PATH_HINT environment variable 3.
     * OPENCV_SAMPLES_DATA_PATH environment variable If parameter value is not empty
     * and nothing is found then stop searching. 4. Detects build/install path based
     * on: a. current working directory (CWD) b. and/or binary module location
     * (opencv_core/opencv_world, doesn't work with static linkage) 5. Scan
     * {@code &lt;source&gt;/{,data,samples/data}} directories if build directory is
     * detected or the current directory is in source tree. 6. Scan
     * {@code &lt;install&gt;/share/OpenCV} directory if install directory is
     * detected.
     * <p>
     * SEE: cv::utils::findDataFile
     *
     * @param relative_path Relative path to data file
     * @param required      Specify "file not found" handling. If true, function
     *                      prints information message and raises cv::Exception. If
     *                      false, function returns empty result
     ...
     * SEE: cv::utils::findDataFile
     *
     * @param relative_path Relative path to data file If true, function prints
     *                      information message and raises cv::Exception. If false,
    ...
    //
    // C++: String cv::samples::findFileOrKeep(String relative_path, bool silentMode
    // = false)
    //
    ...
    //
    // C++: bool cv::checkRange(Mat a, bool quiet = true, _hidden_ * pos = 0, double
     ...
     * <p>
     * The function cv::checkRange checks that every array element is neither NaN
     * nor infinite. When minVal &gt;
     * <ul>
     * <li>DBL_MAX and maxVal &lt; DBL_MAX, the function also checks that each value
     * is between minVal and maxVal. In case of multi-channel arrays, each channel
     * is processed independently. If some values are out of range, position of the
     * first outlier is stored in pos (when pos != NULL). Then, the function either
     ...
     *               when the array elements are out of range or they throw an
     *               exception. elements.
     * @param minVal inclusive lower boundary of valid values range.
     * @param maxVal exclusive upper boundary of valid values range.</li>
     *               </ul>
     * @return automatically generated
     */
     ...
     *               when the array elements are out of range or they throw an
     *               exception. elements.
     * @param minVal inclusive lower boundary of valid values range.</li>
     *               </ul>
     * @return automatically generated
     */
     ...
     * eigenvectors of the symmetric matrix src: <code>
     * src*eigenvectors.row(i).t() = eigenvalues.at&lt;srcType&gt;(i)*eigenvectors.row(i).t()
     * </code>
     *
     * <b>Note:</b> Use cv::eigenNonSymmetric for calculation of real eigenvalues
     * and eigenvectors of non-symmetric matrix.
     *
     * @param src          input matrix that must have CV_32FC1 or CV_64FC1 type,
     *                     square size and be symmetrical (src ^T^ == src).
     ...
     *                     type as src; the eigenvectors are stored as subsequent
     *                     matrix rows, in the same order as the corresponding
     ...
     *                    same order as the corresponding eigenvalues. SEE:
     ...
     * #DECOMP_NORMAL ): \(\texttt{dst} = \arg \min _X \| \texttt{src1} \cdot
     * \texttt{X} - \texttt{src2} \|\)
     * <p>
     * If #DECOMP_LU or #DECOMP_CHOLESKY method is used, the function returns 1 if
     * src1 (or \(\texttt{src1}^T\texttt{src1}\) ) is non-singular. Otherwise, it
     * returns 0. In the latter case, dst is not valid. Other methods find a
     ...
     * @param flags solution (matrix inversion) method (#DecompTypes) SEE: invert,
     *              SVD, eigen
     * @return automatically generated
     */
     ...
     * between two vectors: \(d( \texttt{vec1} , \texttt{vec2} )=
     ...
     * metric in decibels (dB), between two input arrays src1 and src2. The arrays
     ...
     *
     * @param mtx input matrix that must have CV_32FC1 or CV_64FC1 type and square
     *            size. SEE: trace, invert, solve, eigen, REF: MatrixExpressions
     * @return automatically generated
     */
     ...
     * code computes the execution time in seconds: <code>
     ...
     * When the matrix src is singular or non-square, the function calculates the
     * pseudo-inverse matrix (the dst matrix) so that norm(src\*dst - I) is minimal,
     * where I is an identity matrix.
     * <p>
     * In case of the #DECOMP_LU method, the function returns non-zero value if the
     * inverse has been successfully calculated and 0 if src is singular.
     * <p>
     * In case of the #DECOMP_SVD method, the function returns the inverse condition
     ...
     *
     * @param src input floating-point M x N matrix.
     * @param dst output matrix of N x M size and the same type as src. SEE: solve,
     *            SVD
     * @return automatically generated
     */
    ...
    //
    // C++: double cv::kmeans(Mat data, int K, Mat& bestLabels, TermCriteria
     ...
     * cluster_count clusters and groups the input samples around the clusters. As
     * an output, \(\texttt{bestLabels}_i\) contains a 0-based cluster index for the
     * sample stored in the \(i^{th}\) row of the samples matrix.
     *
     * <b>Note:</b>
     * <ul>
     ...
     *                   float coordinates is needed. Examples of this array can be:
     *                   </li>
     ...
     * @param bestLabels Input/output integer array that stores the cluster indices
     *                   for every sample.
     * @param criteria   The algorithm termination criteria, that is, the maximum
     *                   number of iterations and/or the desired accuracy. The
     ...
     *                   returns the labels that yield the best compactness (see the
     *                   last function parameter).
     * @param flags      Flag that can take values of cv::KmeansFlags
     * @param centers    Output matrix of the cluster centers, one row per each
     *                   cluster center.
     * @return The function returns the compactness measure that is computed as
     *         \(\sum _i \| \texttt{samples} _i - \texttt{centers} _{
     *         \texttt{labels} _i} \| ^2\) after every attempt. The best (minimum)
     *         value is chosen and the corresponding labels and the compactness
     *         value are returned by the function. Basically, you can use only the
     *         core of the function, set the number of attempts to 1, initialize
     *         labels each time using a custom algorithm, pass them with the ( flags
     *         = #KMEANS_USE_INITIAL_LABELS ) flag, and then choose the best
     *         (most-compact) clustering.</li>
     *         </ul>
     */
    ...
    //
    // C++: double cv::norm(Mat src1, Mat src2, int normType = NORM_L2, Mat mask =
    // Mat())
    //
     ...
     * <p>
     * This version of cv::norm calculates the absolute difference norm or the
     * relative difference norm of arrays src1 and src2. The type of norm to
     ...
     * <p>
     * This version of #norm calculates the absolute norm of src1. The type of norm
     * to calculate is specified using #NormTypes.
     * <p>
     * As example for one array consider the function \(r(x)= \begin{pmatrix} x \\
     * 1-x \end{pmatrix}, x \in [-1;1]\). The \( L_{1}, L_{2} \) and \( L_{\infty}
     * \) norm for the sample value \(r(-1) = \begin{pmatrix} -1 \\ 2
     * \end{pmatrix}\) is calculated as follows \(align*} \| r(-1) \|_{L_1} &amp;=
     * |-1| + |2| = 3 \\ \| r(-1) \|_{L_2} &amp;= \sqrt{(-1)^{2} + (2)^{2}} =
     * \sqrt{5} \\ \| r(-1) \|_{L_\infty} &amp;= \max(|-1|,|2|) = 2 \) and for
     * \(r(0.5) = \begin{pmatrix} 0.5 \\ 0.5 \end{pmatrix}\) the calculation is
     * \(align*} \| r(0.5) \|_{L_1} &amp;= |0.5| + |0.5| = 1 \\ \| r(0.5) \|_{L_2}
     * &amp;= \sqrt{(0.5)^{2} + (0.5)^{2}} = \sqrt{0.5} \\ \| r(0.5) \|_{L_\infty}
     * &amp;= \max(|0.5|,|0.5|) = 0.5. \) The following graphic shows all values for
     * the three norm functions \(\| r(x) \|_{L_1}, \| r(x) \|_{L_2}\) and \(\| r(x)
     * \|_{L_\infty}\). It is notable that the \( L_{1} \) norm forms the upper and
     * the \( L_{\infty} \) norm forms the lower border for the example function \(
     * r(x) \). ![Graphs for the different norm functions from the above
     * example](pics/NormTypes_OneArray_1-2-INF.png)
     * <p>
     * When the mask parameter is specified and it is not empty, the norm is
     * <p>
     * If normType is not specified, #NORM_L2 is used. calculated only over the
     * region specified by the mask.
     * <p>
     * Multi-channel input arrays are treated as single-channel arrays, that is, the
     ...
     * equation: \(\texttt{coeffs} [n] x^{n} + \texttt{coeffs} [n-1] x^{n-1} + ... +
     * \texttt{coeffs} [1] x + \texttt{coeffs} [0] = 0\)
     *
     ...
     * <p>
     * The function cubeRoot computes \(\sqrt[3]{\texttt{val}}\). Negative arguments
     * are handled correctly. NaN and Inf are not handled. The accuracy approaches
     ...
     * The angle is measured in degrees and varies from 0 to 360 degrees. The
     * accuracy is about 0.3 degrees.
     *
     ...
     * extrapolation border mode. For example, if you use cv::BORDER_WRAP mode in
     * the horizontal direction, cv::BORDER_REFLECT_101 in the vertical direction
     * and want to compute value of the "virtual" pixel Point(-5, 100) in a
     * floating-point image img , it looks like: <code>
     * float val = img.at&lt;float&gt;(borderInterpolate(100, img.rows, cv::BORDER_REFLECT_101),
     * borderInterpolate(-5, img.cols, cv::BORDER_WRAP));
     * </code> Normally, the function is not called directly. It is used inside
     * filtering functions and also in copyMakeBorder.
     *
     * @param p          0-based coordinate of the extrapolated pixel along one of
     *                   the axes, likely &lt;0 or &gt;= len
     * @param len        Length of the array along the corresponding axis.
     * @param borderType Border type, one of the #BorderTypes, except for
     *                   #BORDER_TRANSPARENT and #BORDER_ISOLATED . When
     *                   borderType==#BORDER_CONSTANT , the function always returns
     *                   -1, regardless of p and len.
     *                   <p>
     *                   SEE: copyMakeBorder
     * @return automatically generated
     */
     ...
     * <p>
     * The function returns the number of non-zero elements in src : \(\sum _{I: \;
     * \texttt{src} (I) \ne0 } 1\)
     *
     * @param src single-channel array. SEE: mean, meanStdDev, norm, minMaxLoc,
     *            calcCovarMatrix
     * @return automatically generated
     */
     ...
     * OpenCV library:
     * <ul>
     * <li>{@code TBB} - The number of threads, that OpenCV will try to use for
     * parallel regions. If there is any tbb::thread_scheduler_init in user code
     * conflicting with OpenCV, then function returns default number of threads used
     * by TBB library.</li>
     * <li>{@code OpenMP} - An upper bound on the number of threads that could be
     * used to form a new team.</li>
     * <li>{@code Concurrency} - The number of threads, that OpenCV will try to use
     * for parallel regions.</li>
     * <li>{@code GCD} - Unsupported; returns the GCD thread pool limit (512) for
     * compatibility.</li>
     * <li>{@code C=} - The number of threads, that OpenCV will try to use for
     * parallel regions, if before called setNumThreads with threads &gt; 0,
     * otherwise returns the number of logical CPUs, available for the process. SEE:
     ...
     * greater than or equal to vecsize so that the DFT of a vector of size N can be
     * processed efficiently. In the current implementation N = 2 ^p^ \* 3 ^q^ \* 5
    ...
    // /**
    // * Returns the index of the currently executed thread within the current
    // parallel region. Always
    ...
    // * <ul>
    // * <li>
    // * {@code TBB} - Unsupported with current 4.1 TBB release. Maybe will be
    // supported in future.
    // * </li>
    // * <li>
    // * {@code OpenMP} - The thread number, within the current team, of the calling
    // thread.
    // * </li>
    // * <li>
    // * {@code Concurrency} - An ID for the virtual processor that the current
    // context is executing on (0
    // * for master thread and unique number for others, but not necessary
    // 1,2,3,...).
    // * </li>
    // * <li>
    // * {@code GCD} - System calling thread's ID. Never returns 0 inside parallel
    // region.
    // * </li>
    // * <li>
     ...
     * <ul>
     * <li>if coeffs is a 4-element vector: \(\texttt{coeffs} [0] x^3 +
     * \texttt{coeffs} [1] x^2 + \texttt{coeffs} [2] x + \texttt{coeffs} [3] = 0\)
     * </li>
     * <li>if coeffs is a 3-element vector: \(x^3 + \texttt{coeffs} [0] x^2 +
     * \texttt{coeffs} [1] x + \texttt{coeffs} [2] = 0\)</li>
     * </ul>
     * <p>
     ...
     * thread, from which getCPUTickCount is called, can be suspended and resumed at
     * another CPU with its own counter. So, theoretically (and practically) the
     ...
     * example, when the machine was turned on). It can be used to initialize RNG or
     * to measure a function execution time by reading the tick count before and
     ...
     * <p>
     * Returned value is a string containing space separated list of CPU features
     ...
     * processes each element of src as follows: \(\texttt{dst} (I) \leftarrow
     * \texttt{lut(src(I) + d)}\) where \(d = \fork{0}{if \(\texttt{src}\) has depth
     * \(\texttt{CV_8U}\)}{128}{if \(\texttt{src}\) has depth \(\texttt{CV_8S}\)}\)
     *
     * @param src input array of 8-bit elements.
     * @param lut look-up table of 256 elements; in case of multi-channel input
     *            array, the table should either have a single channel (in this case
     *            the same table is used for all channels) or the same number of
     *            channels as in the input array.
     * @param dst output array of the same size and number of channels as src, and
     *            the same depth as lut. SEE: convertScaleAbs, Mat::convertTo
     */
    ...
    //
    // C++: void cv::PCABackProject(Mat data, Mat mean, Mat eigenvectors, Mat&
    // result)
    //
    ...
    //
    // C++: void cv::PCACompute(Mat data, Mat& mean, Mat& eigenvectors, Mat&
    // eigenvalues, double retainedVariance)
    //
    ...
    //
    // C++: void cv::PCACompute(Mat data, Mat& mean, Mat& eigenvectors, Mat&
    // eigenvalues, int maxComponents = 0)
    //
    ...
    //
    // C++: void cv::PCACompute(Mat data, Mat& mean, Mat& eigenvectors, double
    // retainedVariance)
    //
    ...
    //
    // C++: void cv::PCACompute(Mat data, Mat& mean, Mat& eigenvectors, int
    // maxComponents = 0)
    //
     ...
     * when they have the same size and type: \(\texttt{dst}(I) = \texttt{saturate}
     * (| \texttt{src1}(I) - \texttt{src2}(I)|)\) Absolute difference between an
     * array and a scalar when the second array is constructed from Scalar or has as
     * many elements as the number of channels in {@code src1}: \(\texttt{dst}(I) =
     * \texttt{saturate} (| \texttt{src1}(I) - \texttt{src2} |)\) Absolute
     * difference between a scalar and an array when the first array is constructed
     * from Scalar or has as many elements as the number of channels in
     * {@code src2}: \(\texttt{dst}(I) = \texttt{saturate} (| \texttt{src1} -
     * \texttt{src2}(I) |)\) where I is a multi-dimensional index of array elements.
    ...
    //
    // C++: void cv::add(Mat src1, Mat src2, Mat& dst, Mat mask = Mat(), int dtype =
    // -1)
    //
     ...
     * <ul>
     * <li>Sum of two arrays when both input arrays have the same size and the same
     * number of channels: \(\texttt{dst}(I) = \texttt{saturate} ( \texttt{src1}(I)
     * + \texttt{src2}(I)) \quad \texttt{if mask}(I) \ne0\)</li>
     * <li>Sum of an array and a scalar when src2 is constructed from Scalar or has
     * the same number of elements as {@code src1.channels()}: \(\texttt{dst}(I) =
     * \texttt{saturate} ( \texttt{src1}(I) + \texttt{src2} ) \quad \texttt{if
     * mask}(I) \ne0\)</li>
     * <li>Sum of a scalar and an array when src1 is constructed from Scalar or has
     * the same number of elements as {@code src2.channels()}: \(\texttt{dst}(I) =
     * \texttt{saturate} ( \texttt{src1} + \texttt{src2}(I) ) \quad \texttt{if
     * mask}(I) \ne0\) where {@code I} is a multi-dimensional index of array
     * elements. In case of multi-channel arrays, each channel is processed
     * independently.</li>
     * </ul>
     * <p>
     ...
     * </code> The input arrays and the output array can all have the same or
     * different depths. For example, you can add a 16-bit unsigned array to a 8-bit
     * signed array and store the sum as a 32-bit floating-point array. Depth of the
     * output array is determined by the dtype parameter. In the second and third
     * cases above, as well as in the first case, when src1.depth() == src2.depth(),
     * dtype can be set to the default -1. In this case, the output array will have
     * the same depth as the input array, be it src1, src2 or both. <b>Note:</b>
     * Saturation is not applied when the output array has the depth CV_32S. You may
     ...
     * @param mask optional operation mask - 8-bit single channel array, that
     *             specifies elements of the output array to be changed. SEE:
     ...
     * @param dst  output array that has the same size and number of channels as the
     *             input array(s); the depth is defined by dtype or src1/src2.
     *             output array to be changed. SEE: subtract, addWeighted, scaleAdd,
     *             Mat::convertTo
     */
    ...
    //
    // C++: void cv::add(Mat src1, Scalar src2, Mat& dst, Mat mask = Mat(), int
    // dtype = -1)
    //
    ...
    //
    // C++: void cv::addWeighted(Mat src1, double alpha, Mat src2, double beta,
     ...
     * follows: \(\texttt{dst} (I)= \texttt{saturate} ( \texttt{src1} (I)*
     * \texttt{alpha} + \texttt{src2} (I)* \texttt{beta} + \texttt{gamma} )\) where
     * I is a multi-dimensional index of array elements. In case of multi-channel
     * arrays, each channel is processed independently. The function can be replaced
     * with a matrix expression: <code>
     * dst = src1*alpha + src2*beta + gamma;
     * </code> <b>Note:</b> Saturation is not applied when the output array has the
     * depth CV_32S. You may even get result of an incorrect sign in the case of
     * overflow.
     *
     * @param src1  first input array.
     ...
     *              to src1.depth(). SEE: add, subtract, scaleAdd, Mat::convertTo
     */
     ...
     * @param gamma scalar added to each sum.
     * @param dst   output array that has the same size and number of channels as
     *              the input arrays. can be set to -1, which will be equivalent to
     *              src1.depth(). SEE: add, subtract, scaleAdd, Mat::convertTo
     */
    ...
    //
    // C++: void cv::batchDistance(Mat src1, Mat src2, Mat& dist, int dtype, Mat&
    // nidx, int normType = NORM_L2, int K = 0, Mat mask = Mat(), int update = 0,
     ...
     * \(\texttt{dst} (I) = \texttt{src1} (I) \wedge \texttt{src2} (I) \quad
     * \texttt{if mask} (I) \ne0\) An array and a scalar when src2 is constructed
     * from Scalar or has the same number of elements as {@code src1.channels()}:
     * \(\texttt{dst} (I) = \texttt{src1} (I) \wedge \texttt{src2} \quad \texttt{if
     * mask} (I) \ne0\) A scalar and an array when src1 is constructed from Scalar
     * or has the same number of elements as {@code src2.channels()}: \(\texttt{dst}
     ...
     * case of multi-channel arrays, each channel is processed independently. In the
     * second and third cases above, the scalar is first converted to the array
     * type.
     *
     * @param src1 first input array or a scalar.
     * @param src2 second input array or a scalar.
     ...
     * input array: \(\texttt{dst} (I) = \neg \texttt{src} (I)\) In case of a
     * floating-point input array, its machine-specific bit representation (usually
     * IEEE754-compliant) is used for the operation. In case of multi-channel
     ...
     * \(\texttt{dst} (I) = \texttt{src1} (I) \vee \texttt{src2} (I) \quad
     * \texttt{if mask} (I) \ne0\) An array and a scalar when src2 is constructed
     * from Scalar or has the same number of elements as {@code src1.channels()}:
     * \(\texttt{dst} (I) = \texttt{src1} (I) \vee \texttt{src2} \quad \texttt{if
     * mask} (I) \ne0\) A scalar and an array when src1 is constructed from Scalar
     * or has the same number of elements as {@code src2.channels()}: \(\texttt{dst}
     ...
     * size: \(\texttt{dst} (I) = \texttt{src1} (I) \oplus \texttt{src2} (I) \quad
     * \texttt{if mask} (I) \ne0\) An array and a scalar when src2 is constructed
     * from Scalar or has the same number of elements as {@code src1.channels()}:
     * \(\texttt{dst} (I) = \texttt{src1} (I) \oplus \texttt{src2} \quad \texttt{if
     * mask} (I) \ne0\) A scalar and an array when src1 is constructed from Scalar
     * or has the same number of elements as {@code src2.channels()}: \(\texttt{dst}
     ...
     * representations (usually IEEE754-compliant) are used for the operation. In
     * case of multi-channel arrays, each channel is processed independently. In the
     * 2nd and 3rd cases above, the scalar is first converted to the array type.
     *
     * @param src1 first input array or a scalar.
     * @param src2 second input array or a scalar.
     * @param dst  output array that has the same size and type as the input arrays.
    ...
    //
    // C++: void cv::calcCovarMatrix(Mat samples, Mat& covar, Mat& mean, int flags,
    // int ctype = CV_64F)
    //
    ...
    //
    // C++: void cv::cartToPolar(Mat x, Mat y, Mat& magnitude, Mat& angle, bool
     ...
     * for every 2D vector (x(I),y(I)): \(\begin{array}{l} \texttt{magnitude} (I)=
     ...
     *
     * @param x              array of x-coordinates; this must be a single-precision
     *                       or double-precision floating-point array.
     * @param y              array of y-coordinates, that must have the same size
     *                       and same type as x.
     * @param magnitude      output array of magnitudes of the same size and type as
     *                       x.
     * @param angle          output array of angles that has the same size and type
     *                       as x; the angles are measured in radians (from 0 to
     *                       2\*Pi) or in degrees (0 to 360 degrees).
     * @param angleInDegrees a flag, indicating whether the angles are measured in
     *                       radians (which is by default), or in degrees. SEE:
     *                       Sobel, Scharr
     */
     ...
     * @param y         array of y-coordinates, that must have the same size and
     *                  same type as x.
     * @param magnitude output array of magnitudes of the same size and type as x.
     * @param angle     output array of angles that has the same size and type as x;
     *                  the angles are measured in radians (from 0 to 2\*Pi) or in
     *                  degrees (0 to 360 degrees). in radians (which is by
     ...
     * value.
     * <p>
     * The function compares: Elements of two arrays when src1 and src2 have the
     * same size: \(\texttt{dst} (I) = \texttt{src1} (I) \,\texttt{cmpop}\,
     * \texttt{src2} (I)\) Elements of src1 with a scalar src2 when src2 is
     * constructed from Scalar or has a single element: \(\texttt{dst} (I) =
     * \texttt{src1}(I) \,\texttt{cmpop}\, \texttt{src2}\) src1 with elements of
     * src2 when src1 is constructed from Scalar or has a single element:
     ...
     * matrix expressions: <code>
     * Mat dst1 = src1 &gt;= src2;
     * Mat dst2 = src1 &lt; 8;
     * ...
     * </code>
     *
     * @param src1  first input array or a scalar; when it is an array, it must have
     *              a single channel.
     ...
     *              same number of channels as the input arrays.
     * @param cmpop a flag, that specifies correspondence between the arrays
     *              (cv::CmpTypes) SEE: checkRange, min, max, threshold
     */
     ...
     * <ul>
     * <li>\(\texttt{m}_{ij}=\texttt{m}_{ji}\) for \(i &gt; j\) if
     * lowerToUpper=false</li>
     ...
     *                     upper half. Otherwise, the upper half is copied to the
     ...
     * </ul>
     *
     * @param m input-output floating-point square matrix. the upper half.
     *          Otherwise, the upper half is copied to the lower half. SEE: flip,
     *          transpose
     */
     ...
     * (half precision floating point). CV_16S format is used to represent FP16
     * data. There are two use modes (src -&gt; dst): CV_32F -&gt; CV_16S and CV_16S
     * -&gt; CV_32F. The input array has to have type of CV_32F or CV_16S to
     * represent the bit depth. If the input array is neither of them, the function
     * will raise an error. The format of half precision floating point is defined
     * in IEEE 754-2008.
     *
     * @param src input array.
    ...
    //
    // C++: void cv::convertScaleAbs(Mat src, Mat& dst, double alpha = 1, double
    // beta = 0)
    //
     ...
     * to an unsigned 8-bit type: \(\texttt{dst} (I)=
     * \texttt{saturate\_cast&lt;uchar&gt;} (| \texttt{src} (I)* \texttt{alpha} +
     * \texttt{beta} |)\) In case of multi-channel arrays, the function processes
     * each channel independently. When the output is not 8-bit, the operation can
     * be emulated by calling the Mat::convertTo method (or by using matrix
     * expressions) and then by calculating an absolute value of the result. For
     * example: <code>
     * Mat_&lt;float&gt; A(30,30);
     ...
     * @param beta  optional delta added to the scaled values. SEE: Mat::convertTo,
     *              cv::abs(const Mat&amp;)
     */
     ...
     * @param src   input array.
     * @param dst   output array.
     * @param alpha optional scale factor. SEE: Mat::convertTo, cv::abs(const
     *              Mat&amp;)
     */
    ...
    //
    // C++: void cv::copyMakeBorder(Mat src, Mat& dst, int top, int bottom, int
     ...
     * image will be filled with extrapolated pixels. This is not what filtering
     * functions based on it do (they extrapolate pixels on-fly), but what other
     * more complex functions, including your own, may do to simplify image boundary
     * handling.
     * <p>
     ...
     * ...
     * </code> <b>Note:</b> When the source image is a part (ROI) of a bigger image,
     * the function will try to use the pixels outside of the ROI to form a border.
     ...
     *                   the source image rectangle to extrapolate. For example,
     *                   top=1, bottom=1, left=1, right=1 mean that 1 pixel-wide
     ...
     * Copies the matrix to another one. When the operation mask is specified, if
     * the Mat::create call shown above reallocates the matrix, the newly allocated
     ...
     *             indicate which matrix elements need to be copied. The mask has to
     *             be of type CV_8U and can have 1 or multiple channels.
     */
     ...
     * <ul>
     * <li>Forward Cosine transform of a 1D vector of N elements: \(Y = C^{(N)}
     * \cdot X\) where \(C^{(N)}_{jk}= \sqrt{\alpha_j/N} \cos \left (
     * \frac{\pi(2k+1)j}{2N} \right )\) and \(\alpha_0=1\), \(\alpha_j=2\) for *j
     * &gt; 0*.</li>
     * <li>Inverse Cosine transform of a 1D vector of N elements: \(X = \left
     * (C^{(N)} \right )^{-1} \cdot Y = \left (C^{(N)} \right )^T \cdot Y\) (since
     * \(C^{(N)}\) is an orthogonal matrix, \(C^{(N)} \cdot \left(C^{(N)}\right)^T =
     * I\) )</li>
     * <li>Forward 2D Cosine transform of M x N matrix: \(Y = C^{(N)} \cdot X \cdot
     * \left (C^{(N)} \right )^T\)</li>
     * <li>Inverse 2D Cosine transform of M x N matrix: \(X = \left (C^{(N)} \right
     * )^T \cdot X \cdot C^{(N)}\)</li>
     * </ul>
     * <p>
     * The function chooses the mode of operation by looking at the flags and size
     * of the input array:
     * <ul>
     * <li>If (flags &amp; #DCT_INVERSE) == 0 , the function does a forward 1D or 2D
     * transform. Otherwise, it is an inverse 1D or 2D transform.</li>
     * <li>If (flags &amp; #DCT_ROWS) != 0 , the function performs a 1D transform of
     * each row.</li>
     ...
     * </ul>
     *
     * <b>Note:</b> Currently dct supports even-size arrays (2, 4, 6 ...). For data
     ...
     * size (see getOptimalDFTSize ). In the current implementation DCT of a vector
     * of size N is calculated via DFT of a vector of size N/2 . Thus, the optimal
     * DCT size N1 &gt;= N can be calculated as: <code>
     ...
     *
     * @param src input floating-point array.
     * @param dst output array of the same size and type as src . SEE: dft ,
     ...
     * <ul>
     * <li>Forward the Fourier transform of a 1D vector of N elements: \(Y = F^{(N)}
     * \cdot X,\) where \(F^{(N)}_{jk}=\exp(-2\pi i j k/N)\) and \(i=\sqrt{-1}\)
     * </li>
     * <li>Inverse the Fourier transform of a 1D vector of N elements:
     * \(\begin{array}{l} X'= \left (F^{(N)} \right )^{-1} \cdot Y = \left (F^{(N)}
     * \right )^* \cdot y \\ X = (1/N) \cdot X, \end{array}\) where
     * \(F^*=\left(\textrm{Re}(F^{(N)})-\textrm{Im}(F^{(N)})\right)^T\)</li>
     * <li>Forward the 2D Fourier transform of a M x N matrix: \(Y = F^{(M)} \cdot X
     * \cdot F^{(N)}\)</li>
     * <li>Inverse the 2D Fourier transform of a M x N matrix: \(\begin{array}{l}
     * X'= \left (F^{(M)} \right )^* \cdot Y \cdot \left (F^{(N)} \right )^* \\ X =
     * \frac{1}{M \cdot N} \cdot X' \end{array}\)</li>
     * </ul>
     * <p>
     ...
     * *CCS* spectrum looks: \(\begin{bmatrix} Re Y_{0,0} &amp; Re Y_{0,1} &amp; Im
     * Y_{0,1} &amp; Re Y_{0,2} &amp; Im Y_{0,2} &amp; \cdots &amp; Re Y_{0,N/2-1}
     * &amp; Im Y_{0,N/2-1} &amp; Re Y_{0,N/2} \\ Re Y_{1,0} &amp; Re Y_{1,1} &amp;
     * Im Y_{1,1} &amp; Re Y_{1,2} &amp; Im Y_{1,2} &amp; \cdots &amp; Re
     * Y_{1,N/2-1} &amp; Im Y_{1,N/2-1} &amp; Re Y_{1,N/2} \\ Im Y_{1,0} &amp; Re
     * Y_{2,1} &amp; Im Y_{2,1} &amp; Re Y_{2,2} &amp; Im Y_{2,2} &amp; \cdots &amp;
     * Re Y_{2,N/2-1} &amp; Im Y_{2,N/2-1} &amp; Im Y_{1,N/2} \\ \hdotsfor{9} \\ Re
     * Y_{M/2-1,0} &amp; Re Y_{M-3,1} &amp; Im Y_{M-3,1} &amp; \hdotsfor{3} &amp; Re
     * Y_{M-3,N/2-1} &amp; Im Y_{M-3,N/2-1}&amp; Re Y_{M/2-1,N/2} \\ Im Y_{M/2-1,0}
     * &amp; Re Y_{M-2,1} &amp; Im Y_{M-2,1} &amp; \hdotsfor{3} &amp; Re
     * Y_{M-2,N/2-1} &amp; Im Y_{M-2,N/2-1}&amp; Im Y_{M/2-1,N/2} \\ Re Y_{M/2,0}
     * &amp; Re Y_{M-1,1} &amp; Im Y_{M-1,1} &amp; \hdotsfor{3} &amp; Re
     ...
     * the input array:
     * <ul>
     * <li>If #DFT_ROWS is set or the input array has a single row or single column,
     * the function performs a 1D forward or inverse transform of each row of a
     * matrix when #DFT_ROWS is set. Otherwise, it performs a 2D transform.</li>
     * <li>If the input array is real and #DFT_INVERSE is not set, the function
     * performs a forward 1D or 2D transform:
     * <ul>
     * <li>When #DFT_COMPLEX_OUTPUT is set, the output is a complex matrix of the
     * same size as input.</li>
     * <li>When #DFT_COMPLEX_OUTPUT is not set, the output is a real matrix of the
     ...
     * the matrix above. In case of multiple 1D transforms (when using the #DFT_ROWS
     * flag), each row of the output matrix looks like the first row of the matrix
     * above.</li>
     * </ul>
     * <li>If the input array is complex and either #DFT_INVERSE or #DFT_REAL_OUTPUT
     ...
     * DFT_INVERSE and DFT_ROWS.</li>
     * <li>When #DFT_INVERSE is set and the input array is real, or it is complex
     ...
     * <ul>
     * <li>Since nonzeroRows != 0 is passed to the forward transform calls and since
     * A and B are copied to the top-left corners of tempA and tempB, respectively,
     * it is not necessary to clear the whole tempA and tempB. It is only necessary
     * to clear the tempA.cols - A.cols ( tempB.cols - B.cols) rightmost columns of
     * the matrices.</li>
     ...
     * split the output array C into multiple tiles. For each tile, estimate which
     * parts of A and B are required to calculate convolution in this tile. If the
     ...
     * are too big, the temporary arrays tempA and tempB become too big and there is
     * also a slowdown because of bad cache locality. So, there is an optimal tile
     * size somewhere in the middle.</li>
     * <li>If different tiles in C can be calculated in parallel and, thus, the
     ...
     * to "flip" the second convolution operand B vertically and horizontally using
     * flip . <b>Note:</b>
     * <ul>
     * <li>An example using the discrete fourier transform can be found at
     * opencv_source_code/samples/cpp/dft.cpp</li>
     ...
     * </li>
     * <li>(Python) An example rearranging the quadrants of a Fourier image can be
     ...
     *                    (#DFT_INVERSE is not set) or only the first nonzeroRows of
     *                    the output array (#DFT_INVERSE is set) contain non-zeros,
     *                    thus, the function can handle the rest of the rows more
     *                    efficiently and save some time; this technique is very
     *                    useful for calculating array cross-correlation or
     *                    convolution using DFT. SEE: dct , getOptimalDFTSize ,
     *                    mulSpectrums, filter2D , matchTemplate , flip ,
     *                    cartToPolar , magnitude , phase</li>
     *                    </ul>
     */
     ...
     * @param dst   output array whose size and type depends on the flags .
     * @param flags transformation flags, representing a combination of the
     *              #DftFlags nonzeroRows rows of the input array (#DFT_INVERSE is
     *              not set) or only the first nonzeroRows of the output array
     *              (#DFT_INVERSE is set) contain non-zeros, thus, the function can
     *              handle the rest of the rows more efficiently and save some time;
     *              this technique is very useful for calculating array
     *              cross-correlation or convolution using DFT. SEE: dct ,
     *              getOptimalDFTSize , mulSpectrums, filter2D , matchTemplate ,
     *              flip , cartToPolar , magnitude , phase</li>
     *              </ul>
     */
     ...
     * @param src input array that could be real or complex.
     * @param dst output array whose size and type depends on the flags .
     *            nonzeroRows rows of the input array (#DFT_INVERSE is not set) or
     *            only the first nonzeroRows of the output array (#DFT_INVERSE is
     *            set) contain non-zeros, thus, the function can handle the rest of
     *            the rows more efficiently and save some time; this technique is
     *            very useful for calculating array cross-correlation or convolution
     *            using DFT. SEE: dct , getOptimalDFTSize , mulSpectrums, filter2D ,
     *            matchTemplate , flip , cartToPolar , magnitude , phase</li>
     *            </ul>
     */
    ...
    //
    // C++: void cv::divide(Mat src1, Mat src2, Mat& dst, double scale = 1, int
    // dtype = -1)
    //
     ...
     * saturate(src1(I)*scale/src2(I))}\) or a scalar by an array when there is no
     * src1 : \(\texttt{dst(I) = saturate(scale/src2(I))}\)
     * <p>
     * Different channels of multi-channel arrays are processed independently.
     * <p>
     * For integer types when src2(I) is zero, dst(I) will also be zero.
     *
     * <b>Note:</b> In case of floating point data there is no special defined
     * behavior for zero src2(I) values. Regular floating-point division is used.
     ...
     *              src2.depth(), but in case of an array-by-array division, you can
     *              only pass -1 when src1.depth()==src2.depth(). SEE: multiply,
     *              add, subtract
     */
     ...
     * @param src2  second input array of the same size and type as src1.
     * @param scale scalar factor.
     * @param dst   output array of the same size and type as src2. case of an
    ...
    //
    // C++: void cv::divide(Mat src1, Scalar src2, Mat& dst, double scale = 1, int
    // dtype = -1)
    //
    ...
    /**
     * Calculates eigenvalues and eigenvectors of a non-symmetric matrix (real
     * eigenvalues only).
     *
     ...
     *                     src). The eigenvectors are stored as subsequent matrix
     ...
     * array: \(\texttt{dst} [I] = e^{ src(I) }\)
     * <p>
     * The maximum relative error is about 7e-6 for single-precision input and less
     * than 1e-10 for double-precision input. Currently, the function converts
     * denormalized values to zeros on output. Special values (NaN, Inf) are not
     * handled.
     *
     * @param src input array.
     * @param dst output array of the same size and type as src. SEE: log ,
     *            cartToPolar , polarToCart , phase , pow , sqrt , magnitude
     */
     ...
     * <p>
     * Given a binary matrix (likely returned from an operation such as threshold(),
     * compare(), &gt;, ==, etc, return all of the non-zero indices as a cv::Mat or
     * std::vector&lt;cv::Point&gt; (x,y) For example: <code>
     ...
     * <p>
     * The function cv::flip flips the array in one of three different ways (row and
     * column indices are 0-based): \(\texttt{dst} _{ij} = \left\{ \begin{array}{l
     * l} \texttt{src} _{\texttt{src.rows}-i-1,j} &amp; if\; \texttt{flipCode} = 0
     * \\ \texttt{src} _{i, \texttt{src.cols} -j-1} &amp; if\; \texttt{flipCode}
     * &gt; 0 \\ \texttt{src} _{ \texttt{src.rows} -i-1, \texttt{src.cols} -j-1}
     * &amp; if\; \texttt{flipCode} &lt; 0 \\ \end{array} \right.\) The example
     * scenarios of using the function are the following: Vertical flipping of the
     * image (flipCode == 0) to switch between top-left and bottom-left image
     * origin. This is a typical operation in video processing on Microsoft
     * Windows\* OS. Horizontal flipping of the image with the subsequent horizontal
     * shift and absolute difference calculation to check for a vertical-axis
     * symmetry (flipCode &gt; 0). Simultaneous horizontal and vertical flipping of
     * the image with the subsequent shift and absolute difference calculation to
     * check for a central symmetry (flipCode &lt; 0). Reversing the order of point
     * arrays (flipCode &gt; 0 or flipCode == 0).
     *
     * @param src      input array.
     * @param dst      output array of the same size and type as src.
     * @param flipCode a flag to specify how to flip the array; 0 means flipping
     *                 around the x-axis and positive value (for example, 1) means
     *                 flipping around y-axis. Negative value (for example, -1)
     *                 means flipping around both axes. SEE: transpose , repeat ,
     *                 completeSymm
     */
    ...
    //
    // C++: void cv::gemm(Mat src1, Mat src2, double alpha, Mat src3, double beta,
    // Mat& dst, int flags = 0)
    //
     ...
     * <p>
     * The function cv::gemm performs generalized matrix multiplication similar to
     * the gemm functions in BLAS level 3. For example,
     * {@code gemm(src1, src2, alpha, src3, beta, dst, GEMM_1_T + GEMM_3_T)}
     * corresponds to \(\texttt{dst} = \texttt{alpha} \cdot \texttt{src1} ^T \cdot
     ...
     * call can be replaced with: <code>
     ...
     * @param beta  weight of src3.
     * @param dst   output matrix; it has the proper size and the same type as input
     *              matrices. SEE: mulTransposed , transform
     */
     ...
     *            the same number of rows and the same depth.
     ...
     *
     * @param src input floating-point single-channel array.
     * @param dst output array of the same size and type as src. SEE: dct, dft,
     *            idft, getOptimalDFTSize
     */
     ...
     * <b>Note:</b> None of dft and idft scales the result by default. So, you
     * should pass #DFT_SCALE to one of dft or idft explicitly to make these
     * transforms mutually inverse. SEE: dft, dct, idct, mulSpectrums,
     * getOptimalDFTSize
     *
     ...
     *                    undefined content (see the convolution sample in dft
     *                    description.
     */
     ...
     * @param src   input floating-point real or complex array.
     * @param dst   output array whose size and type depend on the flags.
     * @param flags operation flags (see dft and #DftFlags). the convolution sample
     *              in dft description.
     */
     ...
     *
     * @param src input floating-point real or complex array.
     * @param dst output array whose size and type depend on the flags. the
     ...
     * <ul>
     * <li>For every element of a single-channel input array: \(\texttt{dst} (I)=
     * \texttt{lowerb} (I)_0 \leq \texttt{src} (I)_0 \leq \texttt{upperb} (I)_0\)
     * </li>
     * <li>For two-channel arrays: \(\texttt{dst} (I)= \texttt{lowerb} (I)_0 \leq
     * \texttt{src} (I)_0 \leq \texttt{upperb} (I)_0 \land \texttt{lowerb} (I)_1
     * \leq \texttt{src} (I)_1 \leq \texttt{upperb} (I)_1\)</li>
     * <li>and so forth.</li>
     * </ul>
     * <p>
     ...
     * input array: \(\texttt{dst} (I) = \log (\texttt{src}(I)) \)
     * <p>
     * Output on zero, negative and special (NaN, Inf) values is undefined.
     *
     * @param src input array.
     * @param dst output array of the same size and type as src . SEE: exp,
     *            cartToPolar, polarToCart, phase, pow, sqrt, magnitude
     */
     ...
     * the corresponding elements of x and y arrays: \(\texttt{dst} (I) =
     * \sqrt{\texttt{x}(I)^2 + \texttt{y}(I)^2}\)
     *
     * @param x         floating-point array of x-coordinates of the vectors.
     * @param y         floating-point array of y-coordinates of the vectors; it
     *                  must have the same size as x.
     * @param magnitude output array of the same size and type as x. SEE:
     ...
     * \(\texttt{dst} (I)= \max ( \texttt{src1} (I), \texttt{src2} (I))\) or array
     * and a scalar: \(\texttt{dst} (I)= \max ( \texttt{src1} (I), \texttt{value}
     * )\)
     *
     * @param src1 first input array.
     * @param src2 second input array of the same size and type as src1 .
     * @param dst  output array of the same size and type as src1. SEE: min,
     *             compare, inRange, minMaxLoc, REF: MatrixExpressions
     */
    ...
    //
    // C++: void cv::meanStdDev(Mat src, vector_double& mean, vector_double& stddev,
    // Mat mask = Mat())
    //
     ...
     * output parameters: \(\begin{array}{l} N = \sum _{I, \texttt{mask} (I) \ne 0}
     * 1 \\ \texttt{mean} _c = \frac{\sum_{ I: \; \texttt{mask}(I) \ne 0}
     * \texttt{src} (I)_c}{N} \\ \texttt{stddev} _c = \sqrt{\frac{\sum_{ I: \;
     * \texttt{mask}(I) \ne 0} \left ( \texttt{src} (I)_c - \texttt{mean} _c \right
     * )^2}{N}} \end{array}\) When all the mask elements are 0's, the function
     * returns mean=stddev=Scalar::all(0). <b>Note:</b> The calculated standard
     * deviation is only the diagonal of the complete normalized covariance matrix.
     * If the full matrix is needed, you can reshape the multi-channel array M x N
     * to the single-channel array M\*N x mtx.channels() (only possible when the
     ...
     * @param mask   optional operation mask. SEE: countNonZero, mean, norm,
     ...
     *               results can be stored in Scalar_ 's.
     * @param mean   output parameter: calculated mean value.
     * @param stddev output parameter: calculated standard deviation. SEE:
     ...
     * \(\texttt{dst} (I)= \min ( \texttt{src1} (I), \texttt{src2} (I))\) or array
     * and a scalar: \(\texttt{dst} (I)= \min ( \texttt{src1} (I), \texttt{value}
     * )\)
     *
     * @param src1 first input array.
     * @param src2 second input array of the same size and type as src1.
     * @param dst  output array of the same size and type as src1. SEE: max,
     *             compare, inRange, minMaxLoc
     */
     ...
     *               have the same size and the same depth.
     * @param dst    output array or vector of matrices; all the matrices <b>must be
     *               allocated</b>; their size and depth must be the same as in
     *               src[0].
     ...
     *               src, fromTo[k\*2+1] is an index of the output channel in dst;
     *               the continuous channel numbering is used: the first input image
     *               channels are indexed from 0 to src[0].channels()-1, the second
     *               input image channels are indexed from src[0].channels() to
     *               src[0].channels() + src[1].channels()-1, and so on, the same
     *               scheme is used for the output image channels; as a special
     *               case, when fromTo[k\*2] is negative, the corresponding output
     *               channel is filled with zero .
     */
    ...
    //
    // C++: void cv::mulSpectrums(Mat a, Mat b, Mat& c, int flags, bool conjB =
    // false)
    //
     ...
     *              cv::DFT_ROWS, which indicates that each row of src1 and src2 is
     *              an independent 1D Fourier spectrum. If you do not want to use
     *              this flag, then simply add a {@code 0} as value.
     ...
     *              cv::DFT_ROWS, which indicates that each row of src1 and src2 is
     *              an independent 1D Fourier spectrum. If you do not want to use
     *              this flag, then simply add a {@code 0} as value. or not (false).
     */
    ...
    //
    // C++: void cv::mulTransposed(Mat src, Mat& dst, bool aTa, Mat delta = Mat(),
     ...
     * transposition: \(\texttt{dst} = \texttt{scale} ( \texttt{src} -
     * \texttt{delta} )^T ( \texttt{src} - \texttt{delta} )\) if aTa=true , and
     * \(\texttt{dst} = \texttt{scale} ( \texttt{src} - \texttt{delta} ) (
     * \texttt{src} - \texttt{delta} )^T\) otherwise. The function is used to
     ...
     *              multiplication. When the matrix is empty ( delta=noArray() ), it
     *              is assumed to be zero, that is, nothing is subtracted. If it has
     *              the same size as src , it is simply subtracted. Otherwise, it is
     *              "repeated" (see repeat ) to cover the full src and then
     *              subtracted. Type of the delta matrix, when it is not empty, must
     *              be the same as the type of created output matrix. See the dtype
     *              parameter description below.
     * @param scale Optional scale factor for the matrix product.
     * @param dtype Optional type of the output matrix. When it is negative, the
     *              output matrix will have the same type as src . Otherwise, it
     *              will be type=CV_MAT_DEPTH(dtype) that should be either CV_32F or
     ...
     *              be the same as the type of created output matrix. See the dtype
     *              parameter description below.
     * @param scale Optional scale factor for the matrix product. the output matrix
     *              will have the same type as src . Otherwise, it will be
     ...
     *              subtracted. Type of the delta matrix, when it is not empty, must
     *              be the same as the type of created output matrix. See the dtype
     *              parameter description below. the output matrix will have the
     *              same type as src . Otherwise, it will be
     *              type=CV_MAT_DEPTH(dtype) that should be either CV_32F or CV_64F
     *              . SEE: calcCovarMatrix, gemm, repeat, reduce
     */
     ...
     * @param dst output square matrix.
     * @param aTa Flag specifying the multiplication ordering. See the description
     *            below. multiplication. When the matrix is empty ( delta=noArray()
     *            ), it is assumed to be zero, that is, nothing is subtracted. If it
     *            has the same size as src , it is simply subtracted. Otherwise, it
     *            is "repeated" (see repeat ) to cover the full src and then
     *            subtracted. Type of the delta matrix, when it is not empty, must
     *            be the same as the type of created output matrix. See the dtype
     *            parameter description below. the output matrix will have the same
     *            type as src . Otherwise, it will be type=CV_MAT_DEPTH(dtype) that
     *            should be either CV_32F or CV_64F . SEE: calcCovarMatrix, gemm,
     *            repeat, reduce
     */
    ...
    //
    // C++: void cv::multiply(Mat src1, Mat src2, Mat& dst, double scale = 1, int
    // dtype = -1)
    //
     ...
     * <p>
     * \(\texttt{dst} (I)= \texttt{saturate} ( \texttt{scale} \cdot \texttt{src1}
     * (I) \cdot \texttt{src2} (I))\)
     * <p>
     ...
     * @param dtype optional depth of the output array SEE: add, subtract, divide,
     *              scaleAdd, addWeighted, accumulate, accumulateProduct,
     ...
     * @param src2  second input array of the same size and the same type as src1.
     * @param dst   output array of the same size and type as src1.
     * @param scale optional scale factor. SEE: add, subtract, divide, scaleAdd,
     ...
     * @param src1 first input array.
     * @param src2 second input array of the same size and the same type as src1.
     * @param dst  output array of the same size and type as src1. SEE: add,
     *             subtract, divide, scaleAdd, addWeighted, accumulate,
    ...
    //
    // C++: void cv::multiply(Mat src1, Scalar src2, Mat& dst, double scale = 1, int
    // dtype = -1)
    //
    ...
    //
    // C++: void cv::normalize(Mat src, Mat& dst, double alpha = 1, double beta = 0,
     ...
     * elements so that \(\| \texttt{dst} \| _{L_p}= \texttt{alpha}\) (where p=Inf,
     * 1 or 2) when normType=NORM_INF, NORM_L1, or NORM_L2, respectively; or so that
     * \(\min _I \texttt{dst} (I)= \texttt{alpha} , \, \, \max _I \texttt{dst} (I)=
     * \texttt{beta}\)
     * <p>
     * when normType=NORM_MINMAX (for dense arrays only). The optional mask
     * specifies a sub-array to be normalized. This means that the norm or min-n-max
     * are calculated over the sub-array, and then this sub-array is modified to be
     * normalized. If you want to only use the mask to calculate the norm or min-max
     * but modify the whole array, you can use norm and Mat::convertTo.
     * <p>
     ...
     *                  case of the range normalization.
     * @param beta      upper range boundary in case of the range normalization; it
     *                  is not used for the norm normalization.
     ...
     * @param mask      optional operation mask. SEE: norm, Mat::convertTo,
     *                  SparseMat::convertTo
     */
     ...
     * @param dtype     when negative, the output array has the same type as src;
     *                  otherwise, it has the same number of channels as src and the
     *                  depth =CV_MAT_DEPTH(dtype). SEE: norm, Mat::convertTo,
     *                  SparseMat::convertTo
     */
     ...
     * @param beta      upper range boundary in case of the range normalization; it
     *                  is not used for the norm normalization.
     * @param norm_type normalization type (see cv::NormTypes). number of channels
     *                  as src and the depth =CV_MAT_DEPTH(dtype). SEE: norm,
     ...
     *              not used for the norm normalization. number of channels as src
     *              and the depth =CV_MAT_DEPTH(dtype). SEE: norm, Mat::convertTo,
     *              SparseMat::convertTo
     */
     ...
     * @param dst   output array of the same size as src .
     * @param alpha norm value to normalize to or the lower range boundary in case
     *              of the range normalization. normalization. number of channels as
     *              src and the depth =CV_MAT_DEPTH(dtype). SEE: norm,
     *              Mat::convertTo, SparseMat::convertTo
     */
     ...
     *
     * @param src input array.
     * @param dst output array of the same size as src . normalization.
     *            normalization. number of channels as src and the depth
     *            =CV_MAT_DEPTH(dtype). SEE: norm, Mat::convertTo,
     *            SparseMat::convertTo
     */
     ...
     * treating it as a 2D or 3D vector, in the following way: \((x, y, z)
     * \rightarrow (x'/w, y'/w, z'/w)\) where \((x', y', z', w') = \texttt{mat}
     * \cdot \begin{bmatrix} x &amp; y &amp; z &amp; 1 \end{bmatrix}\) and \(w =
     * \fork{w'}{if \(w' \ne 0\)}{\infty}{otherwise}\)
     * <p>
     ...
     * compute the most probable perspective transformation out of several pairs of
     * corresponding points, you can use getPerspectiveTransform or findHomography .
     *
     * @param src input two-channel or three-channel floating-point array; each
     *            element is a 2D/3D vector to be transformed.
     * @param dst output array of the same size and type as src.
     * @param m   3x3 or 4x4 floating-point transformation matrix. SEE: transform,
     *            warpPerspective, getPerspectiveTransform, findHomography
     */
     ...
     * is formed from the corresponding elements of x and y : \(\texttt{angle} (I) =
     * \texttt{atan2} ( \texttt{y} (I), \texttt{x} (I))\)
     * <p>
     * The angle estimation accuracy is about 0.3 degrees. When x(I)=y(I)=0 , the
     * corresponding angle(I) is set to 0.
     *
     * @param x              input floating-point array of x-coordinates of 2D
     *                       vectors.
     * @param y              input array of y-coordinates of 2D vectors; it must
     ...
     *              type as x . degrees, otherwise, they are measured in radians.
     */
    ...
    //
    // C++: void cv::polarToCart(Mat magnitude, Mat angle, Mat& x, Mat& y, bool
    // angleInDegrees = false)
    //
     ...
     * \(\begin{array}{l} \texttt{x} (I) = \texttt{magnitude} (I) \cos (
     * \texttt{angle} (I)) \\ \texttt{y} (I) = \texttt{magnitude} (I) \sin (
     * \texttt{angle} (I)) \\ \end{array}\)
     * <p>
     * The relative accuracy of the estimated coordinates is about 1e-6.
     *
     * @param magnitude      input floating-point array of magnitudes of 2D vectors;
     *                       it can be an empty matrix (=Mat()), in this case, the
     *                       function assumes that all the magnitudes are =1; if it
     *                       is not empty, it must have the same size and type as
     *                       angle.
     * @param angle          input floating-point array of angles of 2D vectors.
     * @param x              output array of x-coordinates of 2D vectors; it has the
     *                       same size and type as angle.
     * @param y              output array of y-coordinates of 2D vectors; it has the
     *                       same size and type as angle.
     * @param angleInDegrees when true, the input angles are measured in degrees,
     *                       otherwise, they are measured in radians. SEE:
     ...
     *                  assumes that all the magnitudes are =1; if it is not empty,
     *                  it must have the same size and type as angle.
     * @param angle     input floating-point array of angles of 2D vectors.
     * @param x         output array of x-coordinates of 2D vectors; it has the same
     *                  size and type as angle.
     * @param y         output array of y-coordinates of 2D vectors; it has the same
     *                  size and type as angle. degrees, otherwise, they are
     *                  measured in radians. SEE: cartToPolar, magnitude, phase,
     *                  exp, log, pow, sqrt
     */
     ...
     * \(\texttt{dst} (I) = \fork{\texttt{src}(I)^{power}}{if \(\texttt{power}\) is
     ...
     * root of array src shows: <code>
     * Mat mask = src &lt; 0;
     * pow(src, 1./5, dst);
     * subtract(Scalar::all(0), dst, dst, mask);
     * </code> For some values of power, such as integer values, 0.5 and -0.5,
     ...
     * @param dst   output array of the same size and type as src. SEE: sqrt, exp,
     *              log, cartToPolar, polarToCart
     */
     ...
     *                   operations (see the details below). instead. SEE: RNG, sort
     */
     ...
     * generated random numbers are clipped to fit the value range of the output
     * array data type.
     *
     ...
     *               either a vector (in which case a diagonal standard deviation
     *               matrix is assumed) or a square matrix. SEE: RNG, randu
     */
     ...
     * uniformly-distributed random numbers from the specified range: \(\texttt{low}
     * _c \leq \texttt{dst} (I)_c &lt; \texttt{high} _c\)
     *
     * @param dst  output array of random numbers; the array must be pre-allocated.
     * @param low  inclusive lower boundary of the generated random numbers.
     * @param high exclusive upper boundary of the generated random numbers. SEE:
     *             RNG, randn, theRNG
     */
     ...
     * In case of #REDUCE_MAX and #REDUCE_MIN , the output image should have the
     * same type as the source one. In case of #REDUCE_SUM and #REDUCE_AVG , the
     ...
     *              the matrix is reduced to a single row. 1 means that the matrix
     *              is reduced to a single column.
     * @param rtype reduction operation that could be one of #ReduceTypes
     * @param dtype when negative, the output vector will have the same type as the
     *              input matrix, otherwise, its type will be
     ...
     *              the matrix is reduced to a single row. 1 means that the matrix
     *              is reduced to a single column.
     * @param rtype reduction operation that could be one of #ReduceTypes otherwise,
     *              its type will be CV_MAKE_TYPE(CV_MAT_DEPTH(dtype),
     *              src.channels()). SEE: repeat
     */
     ...
     * <p>
     * The function cv::repeat duplicates the input array one or more times along
     * each of the two axes: \(\texttt{dst} _{ij}= \texttt{src} _{i\mod src.rows, \;
     * j\mod src.cols }\) The second variant of the function is more convenient to
     * use with REF: MatrixExpressions.
     *
     * @param src input array to replicate.
     * @param ny  Flag to specify how many times the {@code src} is repeated along
     *            the vertical axis.
     * @param nx  Flag to specify how many times the {@code src} is repeated along
     *            the horizontal axis.
    ...
    /**
     * Rotates a 2D array in multiples of 90 degrees. The function cv::rotate
     * rotates the array in one of three different ways: Rotate by 90 degrees
     * clockwise (rotateCode = ROTATE_90_CLOCKWISE). Rotate by 180 degrees clockwise
     * (rotateCode = ROTATE_180). Rotate by 270 degrees clockwise (rotateCode =
     * ROTATE_90_COUNTERCLOCKWISE).
     *
     * @param src        input array.
     * @param dst        output array of the same type as src. The size is the same
     *                   with ROTATE_180, and the rows and cols are switched for
     ...
     *                   #RotateFlags SEE: transpose , repeat , completeSymm, flip,
     *                   RotateFlags
     */
     ...
     * calculates the sum of a scaled array and another array: \(\texttt{dst} (I)=
     * \texttt{scale} \cdot \texttt{src1} (I) + \texttt{src2} (I)\) The function can
     * also be emulated with a matrix expression, for example: <code>
     * Mat A(3, 3, CV_64F);
     * ...
     ...
     * @param src2  second input array of the same size and type as src1.
     * @param dst   output array of the same size and type as src1. SEE: add,
     *              addWeighted, subtract, Mat::dot, Mat::convertTo
     */
     ...
     * @param s   value to assign to diagonal elements. SEE: Mat::zeros, Mat::ones,
     ...
     * </code>
     *
     * @param mtx matrix to initialize (not necessarily square). SEE: Mat::zeros,
     *            Mat::ones, Mat::setTo, Mat::operator=
     */
     ...
     * system default. This function must be called outside of parallel region.
     * <p>
     * OpenCV will try to run its functions with specified threads number, but some
     * behaviour differs from framework:
     * <ul>
     * <li>{@code TBB} - User-defined parallel constructions will run with the same
     * threads number, if another is not specified. If later on user creates his own
     * scheduler, OpenCV will use it.</li>
     * <li>{@code OpenMP} - No special defined behaviour.</li>
     * <li>{@code Concurrency} - If threads == 1, OpenCV will disable threading
     * optimizations and run its functions sequentially.</li>
     * <li>{@code GCD} - Supports only values &lt;= 0.</li>
     * <li>{@code C=} - No special defined behaviour.
     *
     * @param nthreads Number of threads used by OpenCV. SEE: getNumThreads,
     *                 getThreadNum</li>
     *                 </ul>
     */
     ...
     * custom value.
     *
     * @param seed new state for default random number generator SEE: RNG, randu,
     *             randn
     */
     ...
     * lexicographically, you can use STL std::sort generic function with the proper
     * comparison predicate.
     *
     * @param src   input single-channel array.
     * @param dst   output array of the same size and type as src.
     * @param flags operation flags, a combination of #SortFlags SEE: sortIdx,
     *              randShuffle
     */
     ...
     * the indices of sorted elements in the output array. For example: <code>
     ...
     * In case of multi-channel arrays, each channel is processed independently. The
    ...
    //
    // C++: void cv::subtract(Mat src1, Mat src2, Mat& dst, Mat mask = Mat(), int
    // dtype = -1)
    //
     ...
     * <ul>
     * <li>Difference between two arrays, when both input arrays have the same size
     * and the same number of channels: \(\texttt{dst}(I) = \texttt{saturate} (
     * \texttt{src1}(I) - \texttt{src2}(I)) \quad \texttt{if mask}(I) \ne0\)</li>
     * <li>Difference between an array and a scalar, when src2 is constructed from
     * Scalar or has the same number of elements as {@code src1.channels()}:
     * \(\texttt{dst}(I) = \texttt{saturate} ( \texttt{src1}(I) - \texttt{src2} )
     * \quad \texttt{if mask}(I) \ne0\)</li>
     * <li>Difference between a scalar and an array, when src1 is constructed from
     * Scalar or has the same number of elements as {@code src2.channels()}:
     * \(\texttt{dst}(I) = \texttt{saturate} ( \texttt{src1} - \texttt{src2}(I) )
     * \quad \texttt{if mask}(I) \ne0\)</li>
     * <li>The reverse difference between a scalar and an array in the case of
     * {@code SubRS}: \(\texttt{dst}(I) = \texttt{saturate} ( \texttt{src2} -
     * \texttt{src1}(I) ) \quad \texttt{if mask}(I) \ne0\) where I is a
     * multi-dimensional index of array elements. In case of multi-channel arrays,
     * each channel is processed independently.</li>
     * </ul>
     * <p>
     * The first function in the list above can be replaced with matrix expressions:
     * <code>
     ...
     * </code> The input arrays and the output array can all have the same or
     * different depths. For example, you can subtract to 8-bit unsigned arrays and
     * store the difference in a 16-bit signed array. Depth of the output array is
     * determined by dtype parameter. In the second and third cases above, as well
     * as in the first case, when src1.depth() == src2.depth(), dtype can be set to
     * the default -1. In this case the output array will have the same depth as the
     * input array, be it src1, src2 or both. <b>Note:</b> Saturation is not applied
     * when the output array has the depth CV_32S. You may even get result of an
     ...
     * @param dtype optional depth of the output array SEE: add, addWeighted,
     ...
     *             the input array.
     * @param mask optional operation mask; this is an 8-bit single channel array
     *             that specifies elements of the output array to be changed. SEE:
     ...
     * @param src2 second input array or a scalar.
     * @param dst  output array of the same size and the same number of channels as
     *             the input array. of the output array to be changed. SEE: add,
    ...
    //
    // C++: void cv::subtract(Mat src1, Scalar src2, Mat& dst, Mat mask = Mat(), int
    // dtype = -1)
    //
     ...
     * element of the array src and stores the results in dst : \(\texttt{dst} (I) =
     * \texttt{m} \cdot \texttt{src} (I)\) (when m.cols=src.channels() ), or
     * \(\texttt{dst} (I) = \texttt{m} \cdot [ \texttt{src} (I); 1]\) (when
     * m.cols=src.channels()+1 )
     * <p>
     * Every element of the N -channel array src is interpreted as N -element vector
     * that is transformed using the M x N or M x (N+1) matrix m to M-element vector
     * - the corresponding element of the output array dst .
     * <p>
     ...
     *
     * @param src input array that must have as many channels (1 to 4) as m.cols or
     *            m.cols-1.
     * @param dst output array of the same size and depth as src; it has as many
     *            channels as m.rows.
     * @param m   transformation 2x2 or 2x3 floating-point matrix. SEE:
     *            perspectiveTransform, getAffineTransform, estimateAffine2D,
     ...
     * Transposes a matrix.
     * <p>
     * The function cv::transpose transposes the matrix src : \(\texttt{dst} (i,j) =
     * \texttt{src} (j,i)\) <b>Note:</b> No complex conjugation is done in case of a
     * complex matrix. It should be done separately if needed.
     *
     * @param src input array.
     ...
     *
     * @param src input array or vector of matrices. all of the matrices must have
     *            the same number of cols and the same depth
     ...
     * <p>
     * Use this only to override default behavior Passed paths are used in LIFO
     * order.
     *
     ...
     * ({@code &lt;opencv_contrib&gt;/modules/&lt;name&gt;/samples/data} -&gt;
     * {@code &lt;name&gt;/samples/data} +
     * {@code modules/&lt;name&gt;/samples/data}). Passed subdirectories are used in
     * LIFO order.
     *
    ...
    // C++: minMaxLoc(Mat src, double* minVal, double* maxVal=0, Point* minLoc=0,
